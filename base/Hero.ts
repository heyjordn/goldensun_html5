...
export class Hero extends ControllableChar {
  ...
  set_speed_factors(
    ...
    if (this.climbing) {
      ...
      if (!this.sliding_on_ice) {
        if (
          this.colliding_directions_mask &
            get_direction_mask(desired_direction) ||
          ...
          if ((desired_direction & 1) === 1) {
            ...
            if (
              this.colliding_directions_mask &
              get_direction_mask(desired_direction - 1)
            ) {
        ...
        this.sliding_on_ice &&
        this.colliding_directions_mask &
          get_direction_mask(this.ice_slide_direction)
      ) {
   ...
   */
  initialize() {
    const hero_sprite_base =
      this.data.info.main_char_list[this.key_name].sprite_base;
    this.set_sprite(
  ...
  update() {
    ...
    const force_squat = this.data.info.main_char_list[
      this.key_name
    ].has_permanent_status(permanent_status.DOWNED);
  ...
  update_general_counter() {
    if (this.data.map.get_current_zones()) {
      const incr = this.dashing
        ? Hero.GENERAL_COUNTER_INCR_DASH
        : Hero.GENERAL_COUNTER_INCR_WALK;
      ...
      if (this.general_counter >= Hero.GENERAL_COUNTER_LIMIT) {
        ...
        const djinni_to_recover = Djinn.djinn_in_recover.length
          ? Djinn.djinn_in_recover[0]
          : null;
        if (djinni_to_recover) {
          this.data.info.djinni_list[djinni_to_recover].set_status(
            djinn_status.SET
          );
        }
