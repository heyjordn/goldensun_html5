...
export class DjinnListWindow {
  ...
  constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.chars_sprites = {};
    this.djinns_sprites = [];
    this.djinn_description = this.base_window.set_text_in_position(
      "",
      DJINN_DESCRIPTION_X,
      DJINN_DESCRIPTION_Y
    );
    ...
    this.djinn_status_change_header_window = new DjinnModeHeaderWindow(
      this.game,
      this.data
    );
    this.djinn_char_stats_window_left = new DjinnCharStatsWindow(this.game);
    this.djinn_char_stats_window_right = new DjinnCharStatsWindow(
      this.game,
      DJINN_CHAR_WIN_STATS_RIGHT_X
    );
  ...
  next_character() {
    ...
    if (
      ...
      (this.selected_char_index + 1 === this.setting_djinn_status_char_index ||
        (this.sizes[this.selected_char_index + 1] <
          this.sizes[this.setting_djinn_status_char_index] &&
  ...
  next_djinni() {
    ...
    if (
      ...
      !this.action_text_selected &&
      this.sizes[this.selected_char_index] <
        this.sizes[this.setting_djinn_status_char_index]
    )
      this.select_action_text();
    else {
  ...
  select_action_text() {
    ...
    this.move_cursor(
      CURSOR_TEXT_X + COL_GAP * this.selected_char_index,
      CURSOR_TEXT_Y
    );
  }
  ...
  init_djinn_sprites() {
    for (let i = 0; i < CHARS_PER_PAGE; ++i) {
      ...
      for (let key in elements) {
        ...
        if (elem === elements.NO_ELEMENT || elem === elements.ALL_ELEMENTS)
          continue;

        const x = DJINN_SPRITE_X + i * CHAR_X_BETWEEN;
        const sprite_base_key = Djinn.sprite_base_key(elem);
        const sprite_key = this.data.info.npcs_sprite_base_list[
          sprite_base_key
        ].getSpriteKey(djinn_status.SET);
        this.djinns_sprites[i][elem] = this.chars_sprites_group.create(
          x,
          CHAR_Y_PADDING,
          sprite_key
        );
  ...
  init_djinni_status_texts() {
    ...
    for (let i = 0; i < CHARS_PER_PAGE; ++i) {
      ...
      const y = 16;
      this.djinni_status_texts.push(
        this.base_window.set_text_in_position("", x, y)
      );
    }
  }
  ...
  set_djinn_sprite(tween: boolean = true) {
    ...
    if (
      this.setting_djinn_status &&
      this.selected_djinn_index === this_char.djinni.length
    )
      return;

    const this_djinn =
      this.data.info.djinni_list[this_char.djinni[this.selected_djinn_index]];
    ...
    const this_sprite =
      this.djinns_sprites[this.selected_char_index][this_djinn.element];
    ...
    const action =
      this_djinn.status === djinn_status.RECOVERY
        ? djinn_status.STANDBY
        : this_djinn.status;
    const djinn_sprite_base =
      this.data.info.npcs_sprite_base_list[
        Djinn.sprite_base_key(this_djinn.element)
      ];
    djinn_sprite_base.setAnimation(this_sprite, action);
    const anim_key = djinn_sprite_base.getAnimationKey(
      action,
  ...
  load_page() {
    ...
    for (let i = 0; i < CHARS_PER_PAGE; ++i) {
      ...
      if (!(char_key_name in this.chars_sprites)) {
        const action_key = this_char.sprite_base.getSpriteKey(
          base_actions.IDLE
        );
        this.chars_sprites[char_key_name] = this.chars_sprites_group.create(
          0,
          0,
          action_key
        );
        this.chars_sprites[char_key_name].anchor.setTo(0.5, 1.0);
        this_char.sprite_base.setAnimation(
          this.chars_sprites[char_key_name],
          base_actions.IDLE
        );
      }
  ...
  update_djinn_list(char_index: number) {
    ...
    for (let j = 0; j < char_djinni.length; ++j) {
      ...
      stars.push(
        this.base_window.create_at_group(star_x, star_y, "stars", {
  ...
  set_highlight_bar() {
    if (this.setting_djinn_status && this.action_text_selected) {
      ...
      this.page_number_bar_highlight.x =
        HIGHLIGHT_X_PADDING + this.selected_char_index * DJINN_NAME_BETWEEN;
      this.page_number_bar_highlight.y =
        HIGHLIGHT_Y_PADDING + this.selected_djinn_index * numbers.FONT_SIZE;
    }
    if (this.setting_djinn_status) {
      this.on_action_bar_highlight.visible = true;
      this.on_action_bar_highlight.x =
        HIGHLIGHT_X_PADDING +
        this.setting_djinn_status_char_index * DJINN_NAME_BETWEEN;
      this.on_action_bar_highlight.y =
        HIGHLIGHT_Y_PADDING +
        this.setting_djinn_status_djinn_index * numbers.FONT_SIZE;
    } else {
      this.on_action_bar_highlight.visible = false;
    }
  }
  ...
  update_djinn_description() {
    if (
      this.setting_djinn_status &&
      this.selected_djinn_index ===
        this.data.info.party_data.members[this.selected_char_index].djinni
          .length
    ) {
      ...
      this.base_window.update_text(
        this_djinn.description,
        this.djinn_description
      );
    }
  }
  ...
  grant_control(
    ...
    const controls: Control[] = [
      {
        buttons: Button.LEFT,
        on_down: this.previous_character.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.RIGHT,
        on_down: this.next_character.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.UP,
        on_down: this.previous_djinni.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.DOWN,
        on_down: this.next_djinni.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.A,
  ...
  darken_font_color(darken = true) {
    const this_char =
      this.data.info.party_data.members[this.setting_djinn_status_char_index];

    for (
      let i = 0;
      i < this.djinn_names[this.setting_djinn_status_char_index].length;
      ++i
    ) {
      ...
      this.base_window.update_text_color(
        color,
        this.djinn_names[this.setting_djinn_status_char_index][i]
      );
    }
  }
  ...
  set_djinn_operation() {
    ...
    const this_djinn =
      this.data.info.djinni_list[
        this_char.djinni[this.setting_djinn_status_djinn_index]
      ];
    ...
    if (this.setting_djinn_status_char_index !== this.selected_char_index) {
      ...
      if (this.action_text_selected) {
        this_statuses = [
          this_djinn.status === djinn_status.STANDBY
            ? djinn_status.ANY
            : djinn_status.STANDBY,
        ];
        next_statuses = [
        ...
        next_djinn =
          this.data.info.djinni_list[
            next_char.djinni[this.selected_djinn_index]
          ];
        this_statuses = [
          next_djinn.status === djinn_status.STANDBY
            ? djinn_status.ANY
            : next_djinn.status,
          this_djinn.status === djinn_status.STANDBY
            ? djinn_status.ANY
        ...
        () => {
          ...
          switch (this.view_state) {
            case VIEW_STATES.STATS:
              ...
              this.djinn_char_stats_window_left.base_window.show(
                undefined,
                false
              );
              this.djinn_char_stats_window_right.base_window.update_position({
              ...
              this.djinn_status_change_header_window.set_action_info_text(
                `: ${this_char.name}'s Psy`
              );
              break;
            case VIEW_STATES.THIS_CHAR:
              this.djinn_psynergy_window.update_info(
                this_char,
                this_djinni,
                this_statuses
              );
              this.djinn_psynergy_window.base_window.show(undefined, false);
              this.djinn_char_stats_window_right.base_window.close(
                undefined,
                false
              );
              this.djinn_status_change_header_window.set_action_info_text(
                `: ${next_char.name}'s Psy`
              );
              break;
            case VIEW_STATES.NEXT_CHAR:
              ...
              this.djinn_char_stats_window_left.base_window.close(
                undefined,
                false
              );
              this.djinn_char_stats_window_right.base_window.show(
                undefined,
                false
              );
              this.djinn_char_stats_window_right.base_window.update_position({
  ...
  change_all_djinn_status() {
    ...
    const this_char =
      this.data.info.party_data.members[this.selected_char_index];
    const this_djinn =
      this.data.info.djinni_list[this_char.djinni[djinn_index]];
  ...
  on_choose() {
    ...
    this.djinn_action_window.set_action_for_specific_djinn(
      this_char,
      this_djinn
    );
    this.chars_quick_info_window.set_l_button_visibility(false);
