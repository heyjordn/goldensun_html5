...
export class DjinnModeHeaderWindow {
  ...
  constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.ok_msg_text = this.base_window.set_text_in_position(
      "Is this OK?",
      OK_MSG_X,
      OK_MSG_Y
    );
    this.djinn_status_text = this.base_window.set_text_in_position(
      "",
      DJINN_STATUS_X,
      DJINN_STATUS_Y
    );
    ...
    this.djinn_name_after_text = this.base_window.set_text_in_position(
      "",
      DJINN_NAME_AFTER_X,
      DJINN_NAME_AFTER_Y
    );
  ...
  init_arrow_blinks() {
    ...
    this.djinn_status_arrow_blink_timer.loop(90, () => {
      this.djinn_status_arrow.visible = this.djinn_status_arrow.visible
        ? false
        : true;
    });
  ...
  mount_window() {
    ...
    if (this.chars.length === 1) {
      ...
      this.base_window.update_text_color(
        djinn_font_colors[this.djinni[0].status],
        this.djinn_name_before_text
      );
      this.base_window.update_text_position(
      ...
      this.sprites.push(
        this.base_window.create_at_group(STAR_AFTER_X, STAR_AFTER_Y, "stars", {
          frame: this.djinni[0].element,
        })
      );
      ...
      this.base_window.update_text(
        `: ${this.chars[0].name}'s Psy`,
        this.action_info_text
      );
      this.base_window.update_text_position(
        { x: OK_MSG_X_2, y: OK_MSG_Y_2 },
        this.ok_msg_text
      );
      this.base_window.update_text(
        this.djinni[0].name,
      ...
      if (this.action === djinn_actions.TRADE) {
        ...
        this.base_window.update_text_position(
          {
            x: DJINN_NAME_BEFORE_X - 5,
            y: DJINN_NAME_BEFORE_Y + numbers.FONT_SIZE,
          },
          this.djinn_name_before_text
        );
        this.sprites.push(
  ...
  set_char_and_djinn_sprite() {
    for (let i = 0; i < this.chars.length; ++i) {
      ...
      const char_sprite = this.base_window.create_at_group(
        CHARS_X[i],
        CHARS_Y[i],
        action_key
      );
