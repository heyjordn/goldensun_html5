...
export class DjinnPsynergyWindow {
  ...
  grant_control() {
    ...
    const controls = [
      ...
      {
        buttons: Button.R,
        on_down: this.next_state_callback,
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.A,
  ...
  set_abilities_list() {
    ...
    for (let i = 0; i < this.abilities.length; ++i) {
      ...
      this.icon_sprites_in_window.push(
        this.base_window.create_at_group(icon_x, icon_y, "abilities_icons", {
          frame: key_name,
        })
      );
  ...
  open(
    ...
    this.char = char;
    this.djinni = djinni;
    this.elem_per_page = this.djinni
      ? DEFAULT_ELEM_PER_PAGE
      : DEFAULT_ELEM_PER_PAGE + 1;
    this.next_djinni_status = next_djinni_status;
