...
export class DjinnCharStatsWindow {
  ...
  constructor(game, win_x = 0) {
    ...
    this.char_name_text = this.base_window.set_text_in_position(
      "",
      CHAR_NAME_X,
      CHAR_NAME_Y
    );
    this.base_window.set_text_in_position("Lv", LV_X, LV_Y);
    this.level_number_text = this.base_window.set_text_in_position(
      "",
      LV_NUMBER_X,
      LV_NUMBER_Y,
      {
    ...
    this.class_text = this.base_window.set_text_in_position(
      "",
      CLASS_X,
      CLASS_Y
    );
    this.new_class_text = this.base_window.set_text_in_position(
      "",
      NEW_CLASS_X,
      NEW_CLASS_Y
    );
    ...
    elements_list.forEach((element, i) => {
      ...
      this.djinn_number_texts[element] = this.base_window.set_text_in_position(
        "",
        x,
        DJINN_NUMBER_Y,
        {
    ...
    stats.forEach((stat, i) => {
      ...
      this.stats_next_texts[stat] = this.base_window.set_text_in_position(
        "",
        STATS_NEXT_X,
        y,
        {
  ...
  init_arrow_blinks() {
    ...
    this.class_name_arrow_blink_timer.loop(90, () => {
      this.class_name_arrow.visible = this.class_name_arrow.visible
        ? false
        : true;
    });
  ...
  mount_window() {
    ...
    this.base_window.update_text(
      this.char.level.toString(),
      this.level_number_text
    );
    ...
    elements_list.forEach((element, i) => {
      ...
      const x =
        ...
        char_width -
        1 -
        i * DJINN_NUMBER_SLOT_WIDTH;
      const star_sprite = this.base_window.create_at_group(
        x,
        DJINN_NUMBER_Y + 1,
        "stars",
        { frame: element }
    ...
    stats.forEach((stat, i) => {
      ...
      if (is_hp_pp) {
        ...
        if (!preview_values) {
          current_stat =
            this.char[current_main_stat_map[ordered_main_stats[i]]];
      ...
      this.base_window.update_text(
        isNaN(next_stat) ? "" : next_stat.toString(),
        this.stats_next_texts[stat]
      );
