...
export class ShopItemQuantityWindow {
  ...
  constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.coins_val_text = this.window.set_text_in_position(
      "",
      COINS_VALUE_END_X,
      COINS_VALUE_Y,
      {
    ...
    this.coins_label_text = this.window.set_text_in_position(
      "Coins",
      COINS_LABEL_X,
      COINS_LABEL_Y
    );
  ...
  on_change(quantity: number) {
    ...
    this.window.update_text(
      String(this.base_price * this.chosen_quantity),
      this.coins_val_text
    );
  }
  ...
  grant_control(on_cancel: Function, on_select: Function) {
    ...
    const controls = [
      {
        buttons: Button.LEFT,
        on_down: this.decrease_amount.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.RIGHT,
        on_down: this.increase_amount.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.A,
        on_down: on_select,
        sfx: { down: "menu/positive_4" },
      },
      {
        buttons: Button.B,
        on_down: on_cancel,
    ...
    this.data.control_manager.add_controls(controls, {
      loop_config: {
        horizontal: true,
        horizontal_time: ITEM_COUNTER_LOOP_TIME,
      },
    });
  }
  ...
  open(
    shop_item_obj: ShopItem,
    char_item_obj?: ItemSlot,
    use_coins: boolean = false,
    open_callback?: () => void
  ) {
    this.data.cursor_manager.move_to(
      ...
      () => {
        this.base_price =
          this.data.info.items_list[shop_item_obj.key_name].price;
        ...
        let available_quantity =
          shop_item_obj.quantity === -1 ? 30 : shop_item_obj.quantity;
        if (available_quantity + owned > 30) available_quantity = 30 - owned;
        if (
          use_coins &&
          this.base_price * available_quantity > this.data.info.party_data.coins
        ) {
          available_quantity =
            (this.data.info.party_data.coins / this.base_price) | 0;
        }
