...
export class EquipCompare {
  ...
  constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.text_group = this.window.define_internal_group("texts", {
      x: TXT_GROUP_X,
      y: TXT_GROUP_Y,
    });
    this.arrow_group = this.window.define_internal_group("arrows", {
      x: ARROW_X,
      y: ARROW_Y,
    });
    ...
    this.agi_label_text = this.init_text_sprite(
      "AGL",
      0,
      2 * LINE_SHIFT,
      false
    ...
    this.curr_def_text = this.init_text_sprite(
      "",
      CURR_STAT_END_X,
      LINE_SHIFT,
      true
    );
    this.curr_agi_text = this.init_text_sprite(
      "",
      CURR_STAT_END_X,
      2 * LINE_SHIFT,
      true
    ...
    this.new_def_text = this.init_text_sprite(
      "",
      NEW_STAT_END_X,
      LINE_SHIFT,
      true
    );
    this.new_agi_text = this.init_text_sprite(
      "",
      NEW_STAT_END_X,
      2 * LINE_SHIFT,
      true
  ...
  init_text_sprite(text: string, x: number, y: number, right_align: boolean) {
    let txt = this.window.set_text_in_position(text, x, y, {
  ...
  make_arrow(diff: number, line: number) {
    ...
    if (dead_arrows.length > 0)
      (dead_arrows[0] as Phaser.Sprite).reset(arrow_x, arrow_y);
    else
      this.window.create_at_group(arrow_x, arrow_y, "menu", {
        frame: key,
        internal_group_key: "arrows",
      });
  }
  ...
  compare_items(
    ...
    let eq_effects = {};
    if (equipped) {
      eq_effects = _.mapKeys(
        this.data.info.items_list[equipped].effects,
        (effect) => effect.type
      );
    }
    let nitem_effects = _.mapKeys(
      this.data.info.items_list[new_item].effects,
      (effect) => effect.type
    );
  ...
  show_stat_compare() {
    if (
      !this.data.info.items_list[this.selected_item].equipable_chars.includes(
    ...
    for (let i = 0; i < eq_types.length; i++) {
      if (
        selected_item_type === item_types[eq_types[i]] &&
        eq_slots[slot_types[i]]
      )
        char_current_item =
          this.data.info.items_list[eq_slots[slot_types[i]].key_name].key_name;
    }
  ...
  open(char_key: string, item: string, open_callback?: () => void) {
    this.selected_char = this.data.info.party_data.members.filter(
      (c: MainChar) => {
