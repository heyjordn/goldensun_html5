...
export class BuySelectMenu {
  ...
  set_text(text: string, index: number) {
    ...
    let dead_shadows = this.text_group.children.filter(
      (s: Phaser.BitmapText) => {
    ...
    if (dead_texts.length > 0 && dead_shadows.length > 0) {
      ...
      shadow = this.game.add.bitmapText(
        index * LINE_SHIFT,
        0,
        "gs-shop-bmp-font",
        text
      );
      sprite = this.game.add.bitmapText(
        index * LINE_SHIFT,
        0,
        "gs-shop-bmp-font",
        text
    ...
    if (dead_text_bgs.length > 0) {
      ...
      (dead_text_bgs[0] as Phaser.Graphics).drawRect(
        0,
        0,
        -(sprite.height + 1),
        -(sprite.width + 1)
  ...
  set_sprites(page: number) {
    for (let i = 0; i < this.pages[page].length; i++) {
      ...
      if (dead_items.length > 0 && dead_backgrounds.length > 0) {
        ...
        (dead_items[0] as Phaser.Sprite).frameName =
          this.pages[page][i].key_name;
        dead_items[0].scale.x = 1;
        dead_items[0].scale.y = 1;
        (dead_items[0] as Phaser.Sprite).reset(i * LINE_SHIFT, 0);
      } else {
        this.sprite_group
          .create(i * LINE_SHIFT, 0, "menu", "item_border")
      ...
      if (dead_tags.length > 0)
        (dead_tags[0] as Phaser.Sprite).reset(i * LINE_SHIFT, 0);
  ...
  change_page(page: number, force_index?: number) {
    ...
    if (force_index !== undefined) {
      ...
      this.selected_index >= this.pages[this.current_page].length
    ) {
  ...
  next_page(force_index?: number) {
    if (this.pages.length === 1 || this.current_page + 1 === this.pages.length)
      return;
  ...
  /*Returns the item with the next index
    ...
    Used as a callback for controls*/
  next_item() {
    if (this.pages[this.current_page].length === 1 && this.pages.length === 1)
      return;
    ...
    if (this.selected_index - 1 < 0) {
      if (this.current_page - 1 < 0) {
        if (this.pages.length === 1)
          this.change_item(this.pages[this.current_page].length - 1);
        else
          this.change_page(
            this.pages.length - 1,
            this.pages[this.pages.length - 1].length - 1
          );
  ...
  is_last(page: number, index: number) {
    if (page === this.pages.length - 1 && index === this.pages[page].length - 1)
      return true;
    else return false;
  }
  ...
  open(
    items: { [key_name: string]: ShopItem },
    index: number = 0,
    page: number = 0,
    open_callback?: () => void
  ) {
    ...
    this.make_pages();
    this.selected_index =
      index < this.pages[this.current_page].length ? index : 0;
