...
export class BuyArtifactsMenu {
  ...
  sell_old_equip(old_item: Item, slot: ItemSlot) {
    ...
    if (old_item.rare_item) {
      ...
      for (let i = 0; i < shop_list.length; i++) {
        if (shop_list[i].key_name === old_item.key_name) {
          exists = true;
          this.data.info.shops_list[this.parent.shop_key].item_list[
            i
          ].quantity += 1;
        }
      }
    ...
    this.data.control_manager.add_simple_controls(
      this.check_game_ticket.bind(this)
    );
  }
  ...
  async show_cursed_msg() {
    ...
    cursed_msg_win.set_text_in_position(
      "You've been cursed!",
      undefined,
      undefined,
      { italic: true }
  ...
  async equip_new_item() {
    const item_type =
      this.data.info.items_list[this.selected_item.key_name].type;
    const char_slots = this.selected_character.equip_slots;

    if (
      this.data.info.items_list[this.selected_item.key_name]
    ...
    if (!this.old_item) {
        ...
        this.yesno_action.open(
          {
            ...
            no: () => {
              let msg_key = this.old_item.rare_item
      ...
      this.data.control_manager.add_simple_controls(
        after_compliment.bind(this)
      );
    }
  }

  on_purchase_success(
    equip_ask: boolean = false,
    game_ticket: boolean = false
  ) {
    let quantity = 1;
    let key_name = game_ticket ? "game_ticket" : this.selected_item.key_name;
    let item_to_add = this.data.info.items_list[key_name];

    if (this.quant_win.is_open && !game_ticket)
      quantity = this.quant_win.chosen_quantity;
    ...
    if (
      ...
      let exec = () => {
        this.data.control_manager.add_simple_controls(
          this.open_buy_select.bind(this)
        );
      };
      ...
      let process_purchase = () => {
        ...
        if (this.selected_item.global_artifact) {
          const global_item = this.data.info.artifacts_global_list.find(
            (item_data) => {
          ...
          if (global_item) {
            if (global_item.quantity > quantity) {
              ...
              this.data.info.artifacts_global_list =
                this.data.info.artifacts_global_list.filter((item_data) => {
                  return item_data.key_name !== item.key_name;
                });
            }
          }
        ...
        if (!game_ticket) {
          ...
          for (let i = 0; i < shop_list.length; i++) {
            if (
              shop_list[i].key_name === this.selected_item.key_name &&
              shop_list[i].quantity !== -1
            ) {
              this.data.info.shops_list[this.parent.shop_key].item_list[
                i
              ].quantity -= quantity;
              if (
                this.data.info.shops_list[this.parent.shop_key].item_list[i]
                  .quantity === 0
              )
                cursor_back = true;
            }
          ...
          const slot_type = item_to_add.equipable
            ? item_equip_slot[item_to_add.type]
            : null;
          const slot_key_name = slot_type
            ? this.selected_character.equip_slots[slot_type]?.key_name
            : null;
          ...
          if (equip_ask && !has_cursed_slot) {
            let equip_now = () => {
              ...
              text = this.npc_dialog.replace_text(
                text,
                this.selected_character.name
              );
              this.npc_dialog.update_dialog(text, false, false);

              this.yesno_action.open(
                {
                  yes: this.equip_new_item.bind(this),
  ...
  on_buy_equip_select() {
    ...
    if (this.selected_character.items.length === MAX_INVENTORY_SIZE) {
      ...
      if (
        ...
        ].equipable_chars.includes(this.selected_character.key_name)
      ) {
        ...
        this.yesno_action.open(
          {
            yes: this.on_purchase_success.bind(this, false, false),
            no: this.open_equip_compare.bind(this),
          },
          { x: YESNO_X, y: YESNO_Y }
        );
      } else {
  ...
  on_buy_item_select(game_ticket: boolean = false) {
    ...
    if (this.selected_character.items.length === MAX_INVENTORY_SIZE) {
      ...
      this.char_display.grant_control(
        game_ticket
          ? this.on_cancel_game_ticket.bind(this)
          : this.on_cancel_char_select.bind(this),
      ...
      if (game_ticket) this.on_purchase_success(false, game_ticket);
        ...
        if (
          ...
          let shop_item = {
            ...
            quantity:
              shop_item_match.quantity === -1 ? 30 : shop_item_match.quantity,
          };
          ...
          if (!this.quant_win.is_open)
            this.quant_win.open(shop_item, char_item, true);
  ...
  open_equip_compare() {
    this.buy_select_pos = {
      ...
      is_last: this.buy_select.is_last(
        this.buy_select.current_page,
    ...
    this.close_windows(close_windows, () => {
      ...
      let open_windows = [
        {
          name: WindowNames.CHAR_DISPLAY,
          arguments: [this.selected_char_index, CharsMenuModes.SHOP],
        },
        {
          name: WindowNames.EQ_COMPARE,
          arguments: [char_key, this.selected_item.key_name],
        },
      ];
  ...
  on_buy_select() {
    this.selected_item =
      this.buy_select.pages[this.buy_select.current_page][
        this.buy_select.selected_index
      ];
    this.data.control_manager.reset();

    if (this.data.info.items_list[this.selected_item.key_name].equipable)
    ...
    if (Object.keys(this.item_list).length === 0) this.close_menu();
      ...
      this.close_windows(close_windows, () => {
        let open_windows = [
          {
            ...
            arguments: [
              this.item_list,
              this.buy_select_pos.index,
              this.buy_select_pos.page,
            ],
          },
        ...
        this.show_windows(open_windows, () => {
          ...
          this.buy_select.grant_control(
            this.close_menu.bind(this),
            this.on_buy_select.bind(this)
          );
        });
      });
    }
  ...
  open_menu(is_artifacts_menu: boolean, close_callback?: Function) {
    ...
    this.active = true;
    this.item_list = this.is_artifacts_menu
      ? this.parent.artifact_list
      : this.parent.normal_item_list;
    ...
    if (is_artifacts_menu) {
      if (Object.keys(this.item_list).length === 0) {
        ...
        this.data.control_manager.add_simple_controls(
          this.open_buy_select.bind(this, "buy_select"),
          {
            reset_on_press: true,
          }
        );
