...
export class InventoryWindow {
  ...
  constructor(game: Phaser.Game, data: GoldenSun, on_change: Function) {
    ...
    this.sprite_group = this.window.define_internal_group(SPRITE_GROUP_KEY, {
      x: ITEM_X,
      y: ITEM_Y,
    });
  ...
  check_expand(expand: boolean) {
    ...
    this.window.update_size({
      height: this.window.height + modifier * EXPAND_DIFF,
    });
    this.window.update_position({
      y: this.window.y - modifier * EXPAND_DIFF,
    });
  ...
  set_text() {
    ...
    if (!found) this.window.update_text(MESSAGE_NO_ITEM, this.text);
      ...
      this.window.update_text(
        MESSAGE_HAVE_ITEM + item_match.quantity,
        this.text
      );
  ...
  make_item_grid() {
    ...
    for (
      let line = 0;
      line < Math.ceil(char_items.length / MAX_PER_LINE);
      line++
    ) {
      const this_line = [];
      for (let col = 0; col < MAX_PER_LINE; col++) {
  ...
  kill_item_at(line: number, col: number) {
    ...
    if (this.item_grid[line][col].broken) {
      let broken_icons = this.sprite_group.children.filter(
        (b: Phaser.Sprite) => {
          return (
            b.alive === true &&
    ...
    if (this.item_grid[line][col].equipped) {
      let equipped_icons = this.icon_group.children.filter(
        (e: Phaser.Sprite) => {
          return (
            e.alive === true &&
  ...
  grant_control(on_cancel: Function, on_select: Function) {
    const controls = [
      {
        buttons: Button.LEFT,
        on_down: this.previous_col.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.RIGHT,
        on_down: this.next_col.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.UP,
  ...
  next_col() {
    if (
      this.item_grid.length === 1 &&
      this.item_grid[this.cursor_pos.line].length === 1
    )
      return;
  ...
  previous_col() {
    ...
    if (this.cursor_pos.col > 0) {
      ...
      if (this.cursor_pos.line === 0) {
        this.set_cursor(
          this.item_grid.length - 1,
          this.item_grid[this.item_grid.length - 1].length - 1
        );
      } else {
  ...
  next_line() {
    ...
    if (this.cursor_pos.line === this.item_grid.length - 1) {
      ...
      if (
        this.cursor_pos.col >
        this.item_grid[this.cursor_pos.line + 1].length - 1
      )
        this.set_cursor(
  ...
  set_sprites() {
    ...
    for (let line = 0; line < this.item_grid.length; line++) {
      for (let col = 0; col < max_per_line; col++) {
        ...
        let this_item =
          this.data.info.items_list[this.item_grid[line][col].key_name];

        let dead_items = this.sprite_group.children.filter(
          (s: Phaser.Sprite) => {
            return s.alive === false && s.key === ITEMS_IMG_KEY;
          }
        );
        let dead_backgrounds = this.sprite_group.children.filter(
          (s: Phaser.Sprite) => {
        ...
        if (dead_items.length > 0 && dead_backgrounds.length > 0) {
          (dead_backgrounds[0] as Phaser.Sprite).reset(
            col * ICON_SIZE,
            line * ICON_SIZE
          );
          (dead_items[0] as Phaser.Sprite).reset(
            col * ICON_SIZE,
            line * ICON_SIZE
          );
        ...
        if (this.item_grid[line][col].broken) {
          let dead_broken = this.sprite_group.children.filter(
            (b: Phaser.Sprite) => {
              return b.alive === false && b.frameName === BROKEN_IMG_KEY;
            }
          );
          if (dead_broken.length > 0)
            (dead_broken[0] as Phaser.Sprite).reset(
              col * ICON_SIZE,
              line * ICON_SIZE
            );
          else
        ...
        if (this.item_grid[line][col].equipped) {
          let dead_icons = this.icon_group.children.filter(
            (e: Phaser.Sprite) => {
              return e.alive === false && e.frameName === EQUIPPED_IMG_KEY;
            }
          );
          if (dead_icons.length > 0)
            (dead_icons[0] as Phaser.Sprite).reset(
              col * ICON_SIZE,
              line * ICON_SIZE
            );
          else
        ...
        if (this.item_grid[line][col].quantity > 1) {
          let dead_text = this.icon_group.children.filter(
            (t: Phaser.BitmapText) => {
              return t.alive === false && t.text !== undefined;
            }
          );
          if (dead_text.length > 0) {
            (dead_text[0] as Phaser.BitmapText).text =
              this.item_grid[line][col].quantity.toString();
            (dead_text[0] as Phaser.BitmapText).reset(
              col * ICON_SIZE,
              line * ICON_SIZE
            );
            dead_text[0].x +=
              SUB_TEXT_X_SHIFT - (dead_text[0] as Phaser.BitmapText).width;
          } else {
