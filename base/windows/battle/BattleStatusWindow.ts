...
export class BattleStatusWindow {
  ...
  public constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.window = new Window(
      ...
      0,
      0,
      BattleStatusWindow.WINDOW.WIDTH,
      BattleStatusWindow.WINDOW.HEIGHT
    );
    this.window.define_internal_group(BattleStatusWindow.GROUP_KEY, {
      x: 0,
      y: 0,
    });
    ...
    this.window.group.bringToTop(
      this.window.get_internal_group(BattleStatusWindow.GROUP_KEY)
    );
    this.init_text();
  }
  ...
  private update_info() {
    ...
    this.window.update_text(
      char_index >= Battle.MAX_CHARS_IN_BATTLE ? "In the back" : "",
      this.in_the_back
    );
  ...
  private update_effects() {
    ...
    for (let index in status_effects) {
      ...
      if (status_effects[index] === temporary_status.DEATH_CURSE) {
        ...
        effect.properties.turns =
          this.selected_char.get_effect_turns_count(main_char_effect);
      }
    ...
    for (let index in buffs_debuffs) {
      ...
      if (
        buffs_debuffs[index].stat === effect_types.RESIST ||
        buffs_debuffs[index].stat === effect_types.POWER
      ) {
  ...
  private get_buffs_debuffs() {
    const effects: {
      stat: effect_types;
      value: BattleStatusEffect["properties"]["value"];
    }[] = [];
    ...
    for (let i = 0; i < stat_keys.length; i++) {
      const val =
        ...
        this.selected_char.preview_stat_without_abilities_effect(
          effect_type_stat[stat_keys[i]]
        );
      if (val !== 0) {
    ...
    const elemental_base =
      this.selected_char.preview_elemental_stats_without_abilities_effect();

    const power_value: BattleStatusEffect["properties"]["value"] =
      ordered_elements.reduce((obj, elem) => {
        obj[elem] =
          this.selected_char.current_power[elem] - elemental_base[elem].power;
        return obj;
      }, {});
    const resist_value: BattleStatusEffect["properties"]["value"] =
      ordered_elements.reduce((obj, elem) => {
        obj[elem] =
          this.selected_char.current_resist[elem] - elemental_base[elem].resist;
        return obj;
      }, {});
      ...
      return _.sortBy(
        (
          [
            ...this.data.info.main_char_list[this.selected_char.key_name]
              .temporary_status,
          ] as (permanent_status | temporary_status)[]
        ).concat([
          ...this.data.info.main_char_list[this.selected_char.key_name]
            .permanent_status,
        ]),
  ...
  private set_sprites() {
    ...
    const sprite_key = this.selected_char.sprite_base.getSpriteKey(
      base_actions.BATTLE
    );
    const sprite_base =
      this.data.info.main_char_list[this.selected_char.key_name].sprite_base;
    ...
    this.battle_sprite.animations.play(
      sprite_base.getAnimationKey(base_actions.BATTLE, "idle_back")
    );
    ...
    if (this.battle_effects.length > 0) {
      for (let index in this.battle_effects) {
        ...
        const x_pos =
          BattleStatusWindow.EFFECTS.X +
          parseInt(index) * BattleStatusWindow.EFFECTS.SHIFT;
  ...
  private previous_char() {
    ...
    this.change_character(
      party[(char_index + party.length - 1) % party.length]
    );
  }
  ...
  public trigger_state_change() {
    if (this.current_state === BattleStatusStates.ITEMS)
      this.current_state = BattleStatusStates.STATISTICS;
  ...
  private draw_seprartor() {
    ...
    const separator_y =
      BattleStatusWindow.SEPARATOR.Y +
      (shift ? BattleStatusWindow.SEPARATOR.SHIFT : 0);
    const separator_width = BattleStatusWindow.SEPARATOR.WIDTH;

    this.window.draw_separator(
      separator_x,
      separator_y,
      separator_x + separator_width,
      separator_y,
      false
    );
