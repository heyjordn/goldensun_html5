...
export class ItemPsynergyChooseWindow {
  ...
  constructor(
    ...
    this.char_select_controls_sprites = [
      this.window.create_at_group(9, 97, "keyboard_buttons", {
        color: 0x0,
        frame: "l_button",
      }),
      this.window.create_at_group(8, 96, "keyboard_buttons", {
        frame: "l_button",
      }),
      this.window.create_at_group(24, 97, "keyboard_buttons", {
        color: 0x0,
        frame: "r_button",
      }),
      this.window.create_at_group(23, 96, "keyboard_buttons", {
        frame: "r_button",
      }),
    ];
    ...
    if (this.is_psynergy_window) {
      list_length = this.char.abilities.filter((elem_key_name) => {
        ...
        return (
          ...
          (ability.is_field_psynergy || ability.effects_outside_battle)
        );
      }).length;
    } else {
    ...
    if (this.is_psynergy_window) {
      this.elements = this.char.abilities
        .filter((elem_key_name) => {
          ...
          return (
            elem_key_name in this.element_list &&
            (ability.is_field_psynergy ||
              (!this.setting_shortcut && ability.effects_outside_battle))
          );
        })
        .slice(
    ...
    for (let i = 0; i < this.elements.length; ++i) {
      ...
      if (this.is_psynergy_window) {
        this.icon_sprites_in_window.push(
          this.window.create_at_group(icon_x, icon_y, this.element_sprite_key, {
            frame: elem_key_name,
          })
        );
        ...
        this.window.make_item_obj(
          ...
          { x: 0, y: 0 },
          {
            ...
            quantity: item.carry_up_to_30
              ? this.item_objs[i].quantity
              : undefined,
  ...
  clear_sprites() {
    for (let i = 0; i < this.icon_sprites_in_window.length; ++i) {
      if (this.is_psynergy_window) {
        ...
        this.window.destroy_internal_group(
          this.icon_sprites_in_window[i].data.internal_group_key
        );
      }
    }
    this.icon_sprites_in_window = [];
  ...
  grant_control(
    ...
    const controls = [
      ...
      {
        buttons: Button.UP,
        on_down: this.previous_element.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.DOWN,
        on_down: this.next_element.bind(this),
        sfx: { down: "menu/move" },
      },
      {
        buttons: Button.A,
  ...
  set_description_window_text(
    ...
    if (tween_if_necessary && pratical_text_width > numbers.GAME_WIDTH) {
      ...
      description_window.tween_text_horizontally(
  ...
  cast_ability(
    ...
    if (ability.type === ability_types.HEALING) {
      ...
      const current_prop = ability.affects_pp
        ? main_stats.CURRENT_PP
        : main_stats.CURRENT_HP;
      const max_prop = ability.affects_pp
        ? main_stats.MAX_PP
        : main_stats.MAX_HP;
      if (dest_char[max_prop] > dest_char[current_prop]) {
        dest_char.current_hp = _.clamp(
          dest_char[current_prop] - value,
          0,
          dest_char[max_prop]
        );
        if (dest_char[max_prop] === dest_char[current_prop]) {
          ...
          case effect_types.PERMANENT_STATUS:
            if (!effect_obj.add_status) {
              if (dest_char.has_permanent_status(effect_obj.status_key_name)) {
                ...
                status_removed.removed.push(
                  status_label_map[effect_obj.status_key_name]
                );
              } else {
                status_removed.not_removed.push(
                  status_label_map[effect_obj.status_key_name]
                );
              }
            }
      ...
      if (status_removed.removed.length || status_removed.not_removed.length) {
        if (status_removed.removed.length) {
          status_description = `${dest_char.name} is not ${_.uniq(
          ...
          if (!stats_boosted.length) {
            this.set_description_window_text(
              description_window,
              description_window_text,
              `${dest_char.name} is not ${_.uniq(
                status_removed.not_removed
              ).join("/")}.`
            );
            return false;
          }
