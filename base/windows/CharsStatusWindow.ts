...
export class CharsStatusWindow {
  ...
  constructor(
    ...
    const chars_number = _.clamp(
      this.data.info.party_data.members.length,
      MAX_CHARS_NUMBER
    );
    this.status_win_width =
      chars_number * WIDTH_PER_CHAR + (INITIAL_PADDING_X >> 1);
    this.status_win_x =
      numbers.GAME_WIDTH -
      this.status_win_width -
      numbers.INSIDE_BORDER_WIDTH -
      numbers.OUTSIDE_BORDER_WIDTH;
    ...
    if (this.djinni_info) {
      ...
      for (let i = 0; i < ordered_elements.length; ++i) {
        ...
        this.standby_count_text[element] =
          this.status_window.set_text_in_position(
            "",
            STANDBY_COUNT_X[x],
  ...
  set_chars_info() {
    const chars_list = this.data.info.party_data.members.slice(
      0,
      MAX_CHARS_NUMBER
    );
    for (let i = 0; i < chars_list.length; ++i) {
      ...
      info_sprites_obj.group =
        this.status_window.define_internal_group(group_key);
      ...
      info_sprites_obj.hp_bar_graphics = this.game.add.graphics(
        base_x_pos,
        y_pos_bar
      );
      info_sprites_obj.hp_bar_graphics.beginFill(STATUS_BAR_COLOR_GOOD, 1);
      info_sprites_obj.hp_bar_graphics.drawRect(
        0,
        0,
        STATUS_BAR_WIDTH,
        STATUS_BAR_HEIGHT
      ...
      info_sprites_obj.pp_bar_graphics = this.game.add.graphics(
        base_x_pos,
        y_pos_bar
      );
      info_sprites_obj.pp_bar_graphics.beginFill(STATUS_BAR_COLOR_GOOD, 1);
      info_sprites_obj.pp_bar_graphics.drawRect(
        0,
        0,
        STATUS_BAR_WIDTH,
        STATUS_BAR_HEIGHT
  ...
  check_chars_in_party() {
    const current_members = new Set(
      this.data.info.party_data.members.map((m) => m.key_name)
    );
    ...
    const add_chars = [...current_members].filter(
      (m) => !current_info.has(m)
    ).length;
    if (add_chars) {
      this.set_chars_info();
    }
  ...
  update_chars_info() {
    ...
    if (this.djinni_info) {
      ...
      if (show_djinn_info) {
        ...
        for (let i = 0; i < ordered_elements.length; ++i) {
          const element = ordered_elements[i];
          const text =
            element in this.standby_djinni
              ? this.standby_djinni[element].toString()
              : "0";
    ...
    this.status_win_width =
      chars_number * WIDTH_PER_CHAR +
      (INITIAL_PADDING_X >> 1) +
      (show_djinn_info ? DJINN_INFO_WIDTH : 0);
    this.status_win_x =
      numbers.GAME_WIDTH -
      this.status_win_width -
      numbers.INSIDE_BORDER_WIDTH -
    ...
    for (let i = 0; i < chars_number; ++i) {
      ...
      this.status_window.update_text_position(
        { x: base_x_pos },
        info_sprite.hp_header
      );
      this.status_window.update_text_position(
        { x: base_x_pos },
        info_sprite.pp_header
      );

      info_sprite.hp_bar_graphics.x = base_x_pos;
      info_sprite.pp_bar_graphics.x = base_x_pos;

      const hp_damage_bar_width =
        (STATUS_BAR_WIDTH * (1 - char.current_hp / char.max_hp)) | 0;
      const hp_damage_bar_x =
        base_x_pos + STATUS_BAR_WIDTH - hp_damage_bar_width;
      ...
      const pp_damage_bar_width =
        (STATUS_BAR_WIDTH * (1 - char.current_pp / char.max_pp)) | 0;
      const pp_damage_bar_x =
        base_x_pos + STATUS_BAR_WIDTH - pp_damage_bar_width;
      ...
      if (i !== 0 || show_djinn_info) {
        this.status_window.draw_separator(
          base_x_pos - 5,
          3,
          base_x_pos - 5,
          this.status_win_height - 1
        );
      }
    }
