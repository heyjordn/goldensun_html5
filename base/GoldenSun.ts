...
export class GoldenSun {
  ...
  private render_loading() {
    ...
    this.game.debug.text(
      `${this.loading_progress}% loading ${this.loading_what}...`,
      5,
      15,
      "#00ff00"
    );
  ...
  private async initialize_game() {
    ...
    const sanctum_data =
      snapshot?.last_visited_town_with_sanctum ??
      this.dbs.init_db.last_visited_town_with_sanctum;
    if (sanctum_data) {
    ...
    this.map = await map.mount_map(
      ...
      snapshot?.map_data.encounter_cumulator
    );
  ...
  private initialize_psynergy_controls() {
    const quick_ability = (index: number) => {
      ...
      this.info.field_abilities_list[shortcut.ability].cast(
        this.hero,
        shortcut.adept
      );
    };

    const fire_psynergy_shortcut = (
      button: AdvanceButton.L | AdvanceButton.R
    ) => {
      if (!this.hero_movement_allowed(false)) return;
  ...
  private update() {
    ...
    if (this.hero_movement_allowed()) {
      ...
      if (this.menu_open && this.main_menu.is_active) {
        ...
        this.healer_open &&
        this.healer_menu.horizontal_menu.menu_active
      ) {
  ...
  private check_fps() {
    if (!this.showing_fps_banner && this.game.time.fps < numbers.MIN_FPS) {
      (document.querySelector("#fps-warning") as HTMLElement).style.display =
        "block";
      this.showing_fps_banner = true;
    } else if (
      this.showing_fps_banner &&
      this.game.time.fps >= numbers.MIN_FPS
    ) {
      (document.querySelector("#fps-warning") as HTMLElement).style.display =
        "none";
