...
export class Classes {
  ...
  static choose_right_class(
    ...
    return Classes.choose_class_by_type(
      classes_list,
      current_level,
      class_type
    );
  ...
  private static choose_class_by_type(
    ...
    let classes = Object.values(classes_list).filter(
      (this_class) => this_class.class_type === class_type
    );
    ...
    return _.sortBy(classes, [
      (this_class) => _.sum(Object.values(this_class.required_level)),
    ]).reverse()[0];
  }
  ...
  static choose_class_type_by_element_afinity(
    ...
    const no_secondary = secondary_elements.every(
