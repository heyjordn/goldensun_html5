...
export class Footsteps {
  ...
  constructor(game, data, char) {
    ...
    this.footsteps_sprite_base =
      this.data.info.misc_sprite_base_list[FOOTSTEPS_KEY_NAME];
    this.char = char;
  }
  ...
  create_step(direction, action) {
    ...
    this.update_foot();
    const footsteps_type =
      this.current_action === base_actions.IDLE ? "double" : "single";
    const animation_name = this.footsteps_sprite_base.getAnimationKey(
      FOOTSTEPS_KEY_NAME,
      footsteps_type
    );
    ...
    const animation_obj =
      footsteps_sprite.animations.getAnimation(animation_name);
  ...
  update_foot() {
    this.footsteps_time_interval =
      this.current_action === base_actions.WALK
        ? WALKING_TIME_INTERVAL
        : RUNNING_TIME_INTERVAL;
    if (this.current_action === base_actions.IDLE) {
      this.foot_forward = foot_forward_types.NONE;
    } else {
      this.foot_forward =
        this.foot_forward === foot_forward_types.LEFT
          ? foot_forward_types.RIGHT
          : foot_forward_types.LEFT;
    }
  }
