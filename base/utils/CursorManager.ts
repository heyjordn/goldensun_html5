...
export class CursorManager {
  ...
  public init_tween(config: TweenConfig, callback?: Function) {
    ...
    switch (config.type) {
      case CursorManager.WIGGLE.KEY:
        ...
        let wiggle_time = config.time
          ? config.time
          : CursorManager.WIGGLE.DEFAULT_TIME;

        this.active_tween = this.game.add
          .tween(this.cursor)
          .to(
            {
              x: this.cursor.x + wiggle_x1,
              y: this.cursor.y + wiggle_y1,
            },
            wiggle_time,
          ...
          .to(
            {
              x: this.cursor.x + wiggle_x2,
              y: this.cursor.y + wiggle_y2,
            },
            wiggle_time,
            Phaser.Easing.Linear.None
          )
      ...
      case CursorManager.POINT.KEY:
        ...
        let point_time = config.time
          ? config.time
          : CursorManager.POINT.DEFAULT_TIME;
        ...
        this.active_tween = this.game.add
          ...
          .to(
            {
              x: this.cursor.x + point_x,
              y: this.cursor.y + point_y,
            },
            point_time,
            Phaser.Easing.Linear.None
          )
  ...
  public move_to(
    ...
    this.cursor_default_pos = {
      x: pos.x + this.game.camera.x,
      y: pos.y + this.game.camera.y,
    };
