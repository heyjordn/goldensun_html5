...
export class ControlManager {
  ...
  add_simple_controls(callback: Function, params?: SimpleControlParams) {
    ...
    return params
      ? this.add_controls(controls, {
          ...
          no_initial_reset: params?.no_initial_reset,
        })
      : this.add_controls(controls);
  }
  ...
  private apply_control_params(controls: Control[], params: any) {
    ...
    if (options?.vertical || options?.vertical_time) {
      edits.push({
        button: Button.UP,
        loop_time: options?.vertical_time,
      });
      edits.push({
        button: Button.DOWN,
        loop_time: options?.vertical_time,
    ...
    if (options?.horizontal || options?.horizontal_time) {
      edits.push({
        button: Button.LEFT,
        loop_time: options?.horizontal_time,
      });
      edits.push({
        button: Button.RIGHT,
        loop_time: options?.horizontal_time,
    ...
    controls.forEach((control) => {
      ...
      const binding_callback = (is_down: boolean, sfx: string) => {
        ...
        if (Array.isArray(control.buttons)) {
          if (!this.check_bt_sequence_is_down(control.buttons as Button[]))
            return;
        }
        ...
        if (control.halt) {
          if (Array.isArray(gamepad_button)) {
            gamepad_button.forEach((bt) =>
              (is_down ? bt.on_down : bt.on_up).halt()
            );
          } else {
      ...
      if (control.on_up) {
        ...
        if (call_once) {
          signal_binding = last_gamepad_bt.on_up.addOnce(
            binding_callback.bind(this, false, control.sfx?.up)
          );
        } else {
      ...
      if (control.on_down) {
        ...
        if (loop_time && !call_once) {
          const last_bt =
            control.buttons[(control.buttons as Button[]).length - 1];
          const b1 = last_gamepad_bt.on_down.add((event) => {
            ...
            if (opposite_button && this.gamepad.is_down(opposite_button)) {
              const opposite_gamepad_bt = this.gamepad.get_button(
                opposite_button
              ) as GamepadButton;
