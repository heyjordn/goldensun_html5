...
export const effect_type_extra_stat: {
  [effect_type in effect_types]?: main_stats;
} = {
...
export const effect_type_elemental_stat: {
  [effect_type in effect_types]?: elemental_stats;
} = {
...
export const on_catch_status_msg = {
  [temporary_status.DELUSION]: (target) =>
    `${target.name} is wrapped in delusion!`,
  [temporary_status.STUN]: (target) => `${target.name} has been stunned!`,
  [temporary_status.SLEEP]: (target) => `${target.name} falls asleep!`,
  [temporary_status.SEAL]: (target) =>
    `${target.name}'s Psynergy has been sealed!`,
  [temporary_status.DEATH_CURSE]: (target) =>
    `The Spirit of Death embraces ${target.name}!`,
  [permanent_status.DOWNED]: (target) => {
    return target.fighter_type === fighter_types.ALLY
      ? `${target.name} was downed...`
      : `You felled ${target.name}!`;
  },
  [permanent_status.POISON]: (target) =>
    `${target.name} is infected with poison!`,
  [permanent_status.VENOM]: (target) =>
    `${target.name} is infected with deadly poison!`,
  [permanent_status.HAUNT]: (target) => `An evil spirit grips ${target.name}!`,
};

export const on_remove_status_msg = {
  [temporary_status.DELUSION]: (target) =>
    `${target.name} sees clearly once again!`,
  ...
  [temporary_status.SEAL]: (target) =>
    `${target.name}'s Psynergy seal is gone!`,
  [permanent_status.DOWNED]: (target) => `${target.name}'s has been revived!`,
  [permanent_status.POISON]: (target) =>
    `The poison is purged from ${target.name}!`,
  [permanent_status.VENOM]: (target) =>
    `The venom is purged from ${target.name}!`,
};
...
export abstract class Player {
  ...
  public effect_turns_count: {
    [effect: string]: number | { [element: string]: number };
  };
