...
export class Djinn {
  ...
  set_status(status: djinn_status) {
    if (status === djinn_status.RECOVERY) {
      ...
      Djinn.djinn_in_recover = Djinn.djinn_in_recover.filter(
        (key_name) => key_name !== this.key_name
      );
      this.recovery_turn = 0;
    }
  ...
  static has_standby_djinn(djinni_list, members) {
    return _.some(
      members
        ...
        .map((djinn_keys) => {
          return djinn_keys.filter(
            (key) => djinni_list[key].status === djinn_status.STANDBY
          ).length;
        })
    );
  }

  static get_standby_djinni(
