...
export class Audio {
  ...
  private alter_volume(delta: number) {
    return (this.game.sound.volume = _.clamp(
      this.game.sound.volume + delta,
      0,
      1
    ));
  }
  ...
  async init_se() {
    ...
    const load_se_promise = new Promise(
      (resolve) => (load_se_promise_resolve = resolve)
    );
  ...
  private initialize_se(se_data, load_promise_resolve) {
    ...
    for (let se_key in se_data) {
      ...
      const loader = this.game.load.audiosprite(
        se_key,
        audio_data.audio,
        null,
        audio_data.json
  ...
  play_se(
    ...
    const audio = this.se_data[key_parts[0]].play(
      key_parts[1],
      volume,
      position_shift
    );
