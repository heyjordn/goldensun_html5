...
export class Map {
  ...
  private _interactable_objects_label_map: {
  ...
  constructor(
    ...
    this._colorize_filter = this.game.add.filter(
      "Colorize"
  ...
  sort_sprites() {
    ...
    const send_to_back_list = new Array(
      this.data.middlelayer_group.children.length
    );
    const send_to_front_list = new Array(
      this.data.middlelayer_group.children.length
    );
    const has_sort_function = new Array(
      this.data.middlelayer_group.children.length
    );
    const has_sort_end_function = new Array(
      this.data.middlelayer_group.children.length
    );
    this.data.middlelayer_group.children.forEach(
      (sprite: PIXI.DisplayObjectContainer, index) => {
    ...
    this.data.middlelayer_group.customSort(
      (a: PIXI.DisplayObjectContainer, b: PIXI.DisplayObjectContainer) => {
  ...
  update() {
    ...
    this.collision_sprite.body.velocity.y =
      this.collision_sprite.body.velocity.x = 0;
  ...
  private update_map_rotation() {
    if (this.is_world_map) {
      const value_check =
        Math.abs(this.mode7_filter.angle) <
        Map.MAX_CAMERA_ROTATION * Math.abs(this.data.hero.current_speed.x);
      const sign_check =
        Math.sign(this.mode7_filter.angle) === this.data.hero.current_speed.x;
      if (this.data.hero.current_speed.x && (value_check || sign_check)) {
        this.mode7_filter.angle -=
          Math.sign(this.data.hero.current_speed.x) * Map.CAMERA_ROTATION_STEP;
      } else if (
        !this.data.hero.current_speed.x &&
        Math.abs(this.mode7_filter.angle) > 0
      ) {
        this.mode7_filter.angle -=
          Math.sign(this.mode7_filter.angle) * Map.CAMERA_ROTATION_STEP;
      }
    }
  }
  ...
  load_map_assets(force_load: boolean, on_complete?: () => void) {
    ...
    promises.push(
      new Promise((resolve) => (load_tilemap_promise_resolve = resolve))
    );
    this.game.load
      .tilemap(
        this.key_name,
        this.tileset_json_url,
        null,
    ...
    promises.push(
      new Promise((resolve) => (load_image_promise_resolve = resolve))
    );
    this.game.load
      .image(this.key_name, this.tileset_image_url)
      .onLoadComplete.addOnce(load_image_promise_resolve);

    if (this.bgm_key) {
      let load_bgm_promise_resolve;
      promises.push(
        new Promise((resolve) => (load_bgm_promise_resolve = resolve))
      );
      this.game.load
        .audio(this.bgm_key, [this.bgm_url])
    ...
    for (let i = 0; i < this.physics_names.length; ++i) {
      ...
      promises.push(
        new Promise((resolve) => (load_physics_promise_resolve = resolve))
      );
      this.game.load
  ...
  config_body(collision_layer: number) {
    ...
    if (this.collision_embedded) {
      ...
      if (this.processed_polygons[this.collision_layer]) {
        for (
          let i = 0;
          i < this.processed_polygons[this.collision_layer].length;
          ++i
        ) {
          ...
          const shape: p2.Convex =
            this.collision_sprite.body.data.shapes[
      ...
      const collision_layer_objects =
        this.sprite.objects[this.collision_layer]?.objectsData ?? [];
      for (let i = 0; i < collision_layer_objects.length; ++i) {
        ...
        if (collision_object.properties) {
          sensor_active =
            collision_object.properties.affected_by_reveal &&
            collision_object.properties.collide_on_reveal;
        }
        if (collision_object.rectangle) {
          ...
          const shape = this.body.addRectangle(
            width,
            height,
            x + (width >> 1),
            y + (height >> 1)
          ...
          const shape = this.collision_sprite.body.addCircle(
            ...
            Math.round(collision_object.x) +
              (Math.round(collision_object.width) >> 1),
            Math.round(collision_object.y) +
              (Math.round(collision_object.height) >> 1)
          );
          if (collision_object.properties) {
            shape.properties = collision_object.properties;
            shape.sensor = sensor_active;
    ...
    this.collision_sprite.body.setCollisionGroup(
  ...
  private pre_processor_polygons() {
    if (this.polygons_processed) {
      for (let collision_layer in this.processed_polygons) {
        for (
          let i = 0;
          i < this.processed_polygons[collision_layer].length;
          ++i
        ) {
    ...
    for (let j = 0; j < this.collision_layers_number; ++j) {
      ...
      const collision_layer_objects =
        this.sprite.objects[collision_layer]?.objectsData ?? [];
      ...
      for (let i = 0; i < collision_layer_objects.length; ++i) {
        ...
        if (
          collision_object.polygon ||
          collision_object.rectangle ||
          collision_object.ellipse
        ) {
  ...
  set_collision_in_tile(
    ...
    if (collision_layer in this.processed_polygons) {
      ...
      if (this.collision_layer === collision_layer) {
        if (
          this.collision_layer in this._shapes &&
          location_key in this._shapes[this.collision_layer]
        ) {
  ...
  body_in_map(instance: NPC | InteractableObjects) {
    ...
    if (instance.base_collision_layer >= 0) {
      const instances =
        this._bodies_positions[instance.base_collision_layer][location_key];
  ...
  remove_body_tile(
    ...
    if (collision_layer in this._bodies_positions) {
      ...
      this._bodies_positions[collision_layer][location_key] =
        this._bodies_positions[collision_layer][location_key].filter(
          (inst) => inst !== instance
        );
  ...
  get_current_tile(controllable_char: ControllableChar, layer?: Map["layers"]) {
    if (layer !== undefined) {
      return this.sprite.getTile(
        controllable_char.tile_x_pos,
        controllable_char.tile_y_pos,
        layer
      );
    } else {
      return this.layers
  ...
  private create_tile_event(property_key: string, raw_property: string) {
    try {
      ...
      const event =
        this.data.tile_event_manager.get_event_instance(property_info);
  ...
  private create_npc(
    ...
    try {
      ...
      const snapshot_info =
        this.data.snapshot_manager.snapshot?.map_data.npcs[npc_index];
      ...
      const actual_action =
        (npc_db.actions && initial_action in npc_db.actions) ||
        !npc_db.action_aliases
          ? initial_action
          : npc_db.action_aliases[initial_action];
      initial_action = snapshot_info?.action ?? initial_action;
      let initial_animation =
        property_info.animation ??
        npc_db.actions[actual_action].initial_animation;
      initial_animation = snapshot_info?.animation ?? initial_animation;
      const interaction_pattern =
        property_info.interaction_pattern ?? npc_db.interaction_pattern;
      const ignore_physics =
        property_info.ignore_physics ?? npc_db.ignore_physics;
      const voice_key = property_info.voice_key ?? npc_db.voice_key;
      const ignore_world_map_scale =
        property_info.ignore_world_map_scale ?? npc_db.ignore_world_map_scale;
      const enable_footsteps =
        property_info.enable_footsteps ?? this._show_footsteps;
      ...
      const step_max_variation =
        property_info.step_max_variation ?? npc_db.step_max_variation;
      ...
      const anchor_x =
        snapshot_info?.anchor.x ?? property_info.anchor_x ?? npc_db.anchor_x;
      const anchor_y =
        snapshot_info?.anchor.y ?? property_info.anchor_y ?? npc_db.anchor_y;
      const scale_x =
        snapshot_info?.scale.x ?? property_info.scale_x ?? npc_db.scale_x;
      const scale_y =
        snapshot_info?.scale.y ?? property_info.scale_y ?? npc_db.scale_y;
      const base_collision_layer =
        snapshot_info?.base_collision_layer ??
        property_info.base_collision_layer;
      const visible = snapshot_info?.visible ?? property_info.visible;
      const movement_type =
        snapshot_info?.movement_type ?? property_info.movement_type;
      ...
      const internal_storage_keys =
        this._internal_map_objs_storage_keys.npcs[map_index] ?? {};
      ...
      if (npc.label) {
        if (npc.label in this._npcs_label_map) {
          console.warn(
            `NPC with '${npc.label}' label is already set in this map.`
          );
        } else {
      ...
      console.warn(
        `NPC "${property_key}" is not a valid JSON or does not exist in db file.`
      );
      return null;
    }
  }
  ...
  private create_interactable_object(
    ...
    try {
      ...
      const snapshot_info =
        this.data.snapshot_manager.snapshot?.map_data.interactable_objects[
          io_index
        ];
      const interactable_object_db =
        this.data.dbs.interactable_objects_db[property_info.key_name];
      ...
      const anchor_x =
        snapshot_info?.anchor?.x ??
        property_info.anchor_x ??
        interactable_object_db.anchor_x;
      const anchor_y =
        snapshot_info?.anchor?.y ??
        property_info.anchor_y ??
        interactable_object_db.anchor_y;
      const scale_x =
        snapshot_info?.scale?.x ??
        property_info.scale_x ??
        interactable_object_db.scale_x;
      const scale_y =
        snapshot_info?.scale?.y ??
        property_info.scale_y ??
        interactable_object_db.scale_y;
      const has_shadow =
        property_info.has_shadow ?? interactable_object_db.has_shadow;
      const psynergies_info = _.merge(
        interactable_object_db.psynergies_info ?? {},
        property_info.psynergies_info ?? {}
      );
      const action =
        snapshot_info?.action ??
        property_info.action ??
        interactable_object_db.initial_action;
      let animation;
      if (interactable_object_db.actions) {
        if (property_info.animation) {
          ...
          if ("same_as" in interactable_object_db.actions[action]) {
            ...
            animation =
              interactable_object_db.actions[same_as_key].initial_animation;
          } else {
            animation =
              interactable_object_db.actions[action].initial_animation;
          }
        }
      }
      ...
      const entangled_by_bush =
        snapshot_info?.entangled_by_bush ?? property_info.entangled_by_bush;
      const internal_storage_keys =
        this._internal_map_objs_storage_keys.interactable_objects[map_index] ??
        {};
      const storage_keys = Object.assign(
        internal_storage_keys,
        property_info.storage_keys
      ...
      if (interactable_object.label) {
        if (interactable_object.label in this.interactable_objects_label_map) {
          ...
          this.interactable_objects_label_map[interactable_object.label] =
            interactable_object;
        }
      }
    } catch {
  ...
  private config_interactable_object() {
    for (let i = 0; i < this.interactable_objects.length; ++i) {
      const snapshot_info =
        this.data.snapshot_manager.snapshot?.map_data.interactable_objects[i];
      ...
      if (
        (!snapshot_info &&
          interactable_object.base_collision_layer in this._bodies_positions) ||
        snapshot_info?.body_in_map
      ) {
        const bodies_positions =
          this._bodies_positions[interactable_object.base_collision_layer];
        const location_key = IntegerPairKey.get_key(
  ...
  private config_npc() {
    for (let i = 0; i < this.npcs.length; ++i) {
      ...
      const snapshot_info =
        this.data.snapshot_manager.snapshot?.map_data.npcs[i];
      npc.init_npc(this, snapshot_info);
      if (
        (!snapshot_info &&
          npc.base_collision_layer in this._bodies_positions) ||
        snapshot_info?.body_in_map
      ) {
        const bodies_positions =
          this._bodies_positions[npc.base_collision_layer];
        const location_key = IntegerPairKey.get_key(
          npc.tile_x_pos,
          npc.tile_y_pos
        );
  ...
  private process_tiled_layers() {
    ...
    this.sprite.objects = _.mapKeys(
      this.sprite.objects,
      (objs: any, layer_name: string) => {
          ...
          objs.objectsData.forEach((this_obj) => {
            ...
            try {
              this.encounter_zones.push({
                rectangle: zone,
                base_rate:
                  this_obj.properties?.base_rate ??
                  objs.properties?.base_rate ??
                  0,
                parties: this_obj.properties?.parties
                  ? JSON.parse(this_obj.properties.parties)
                  : [],
                background_key: this_obj.properties?.background_key ?? null,
              });
            } catch {
              console.warn(
                `Parties data is not a valid JSON in ${layer_name} layer.`
              );
            }
          });
          return layer_name;
    ...
    for (let layer in layers_to_join) {
      ...
      this.sprite.objects[layer].objectsData = this.sprite.objects[
        layer
      ].objectsData.concat(layers_to_join[layer]);
    }
  }
    ...
    for (let i = 0; i < this.layers.length; ++i) {
      ...
      if (!reorganize) {
        ...
        if (layer_obj.properties.blend_mode !== undefined) {
          layer_sprite.blendMode = parse_blend_mode(
            layer_obj.properties.blend_mode
          );
        }
      ...
      if (layer_obj.properties.over !== undefined) {
        target_collision_layer = this.parse_over_prop(
          layer_obj.properties.over
        );
      }
  ...
  private zone_check() {
    ...
    if (this.start_battle_encounter(base_rate)) {
      ...
      const current_tiles = this.get_current_tile(
        this.data.hero
      ) as Phaser.Tile[];
  ...
  add_generic_sprite(
    ...
    const generic_sprite = this.game.add.sprite(
      x,
      y,
      sprite_key,
      options?.frame,
    ...
    generic_sprite.base_collision_layer =
      options?.collision_layer ?? this.collision_layer;
    if (options?.play) {
  ...
  remove_event(location_key: number, event_id: number) {
    this.events[location_key] = this.events[location_key].filter(
  ...
  initialize_map_name_window() {
    ...
    this._map_name_window = new Window(
      this.game,
      window_x,
      window_y,
      window_width,
    ...
    this.map_name_window.group.transformCallback = () => {
      if (this.map_name_window.open) {
        this.map_name_window.group.x +=
          this.map_name_window.x - this.map_name_window.group.worldPosition.x;
        this.map_name_window.group.y +=
          this.map_name_window.y - this.map_name_window.group.worldPosition.y;
      }
    };
  }
  ...
  async mount_map(collision_layer: number = 0, encounter_cumulator?: number) {
    if (!this.assets_loaded) {
      ...
      const load_promise = new Promise(
        (resolve) => (load_promise_resolve = resolve)
      );
    ...
    if (this.sprite.properties?.retreat_data) {
      try {
        ...
        this._retreat_data.direction =
          directions[parsed_data.direction as string];
      } catch {
  ...
  private config_world_map() {
    ...
    if (this.is_world_map) {
      ...
      this.npcs.forEach((npc) => {
        if (!npc.ignore_world_map_scale) {
          npc.sprite.scale.setTo(
            numbers.WORLD_MAP_SPRITE_SCALE_X,
            numbers.WORLD_MAP_SPRITE_SCALE_Y
          );
        }
      ...
      this.interactable_objects.forEach(
        (obj) => (obj.sprite.data.mode7 = true)
      );
    ...
    if (this.data.hero && next_body_radius !== this.data.hero.body_radius) {
      ...
      if (this.is_world_map) {
        ...
        this.data.hero.shadow.scale.setTo(
          numbers.WORLD_MAP_SPRITE_SCALE_X,
          numbers.WORLD_MAP_SPRITE_SCALE_Y
        );
        if (this.is_world_map) {
  ...
  set_internal_storage_key(
    ...
    if (is_npc) {
      ...
      this._internal_map_objs_storage_keys.npcs[obj_index][property] =
        storage_key;
    } else {
      if (
   ...
   */
  initialize_internal_storage_key(
    internal_map_objs_storage_keys: Map["internal_map_objs_storage_keys"]
  ) {
  ...
  manage_filter(filter: Phaser.Filter, set: boolean) {
    ...
    if (set) {
      if (test_sprite.filters && !test_sprite.filters.includes(filter)) {
        this.layers.forEach(
          (l) => (l.sprite.filters = [...l.sprite.filters, filter])
        );
