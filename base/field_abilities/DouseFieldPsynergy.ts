...
export class DouseFieldPsynergy extends FieldAbilities {
  ...
  init_cloud() {
    const cloud_sprite_base =
      this.data.info.misc_sprite_base_list[DouseFieldPsynergy.CLOUD_KEY_NAME];
    const sprite_key = cloud_sprite_base.getSpriteKey(
      DouseFieldPsynergy.CLOUD_KEY_NAME
    );
    this.cloud_sprite = this.data.overlayer_group.create(0, 0, sprite_key);
    cloud_sprite_base.setAnimation(
      this.cloud_sprite,
      DouseFieldPsynergy.CLOUD_KEY_NAME
    );
    ...
    this.cloud_sprite.y =
      this.controllable_char.y - this.controllable_char.height + 10;
  }
  ...
  async move_cloud_out() {
    ...
    if (this.target_object) {
      ...
      dest_y =
        ...
        DouseFieldPsynergy.DOUSE_HEIGHT;
    }
