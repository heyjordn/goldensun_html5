...
export class FrostFieldPsynergy extends FieldAbilities {
  ...
  init_snowflakes() {
    ...
    for (let i = 0; i < FrostFieldPsynergy.SNOWFLAKES_COUNT; ++i) {
      const snowflake_sprite = this.data.overlayer_group.create(
        0,
        0,
        "frost_snowflake"
      );
      ...
      const spiral_angle = {
        rad: FrostFieldPsynergy.TOTAL_TURNS_SNOWFLAKES,
      };
  ...
  static set_permanent_blink(
    game: Phaser.Game,
    target_object: FrostFieldPsynergy["target_object"]
  ) {
