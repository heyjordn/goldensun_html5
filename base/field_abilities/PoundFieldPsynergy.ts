...
export class PoundFieldPsynergy extends FieldAbilities {
  ...
  init_hand() {
    ...
    this.hand_tween(
      ...
      150,
      this.change_hand_frame.bind(this)
    );
  }
    ...
    timer.add(850, () => {
      ...
      this.hand_tween(this.hand_translate_x, this.hand_translate_y, 300, () => {
        this.game.add
          ...
          .to({ width: 0, height: 0 }, 300, Phaser.Easing.Linear.None, true);
  ...
  init_stars() {
    for (let i = 0; i < PoundFieldPsynergy.STAR_MAX_COUNT; ++i) {
      const initial_x =
        this.hand_sprite.centerX + Math.cos((Math.PI + numbers.degree60) * i);
      const initial_y =
        this.hand_sprite.centerY + Math.sin((Math.PI + numbers.degree60) * i);
      const star_sprite = this.data.overlayer_group.create(
        initial_x,
        initial_y,
        "star"
      );
      ...
      const this_angle =
        ((Math.PI + numbers.degree90) * i) / (12 - 1) + numbers.degree30;
  ...
  init_wave(x_pos, y_pos) {
    ...
    const wave_animation = this.game.add
      .tween(wave)
      .to({ size: size }, 400, Phaser.Easing.Linear.None);
    const wave_phase = this.game.add
      .tween(wave)
      .to(
        { phase: -3.26, transparent_radius: 30 },
        400,
        Phaser.Easing.Linear.None
      );
