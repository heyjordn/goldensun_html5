...
export class LashFieldPsynergy extends FieldAbilities {
  ...
  init_hand() {
    ...
    const sprite_key = this._hand_sprite_base.getSpriteKey(
      LashFieldPsynergy.LASH_HAND_KEY_NAME
    );
    ...
    this._hand_sprite_base.setAnimation(
      this._hand_sprite,
      LashFieldPsynergy.LASH_HAND_KEY_NAME
    );
    const close_hand_key = this._hand_sprite_base.getAnimationKey(
      LashFieldPsynergy.LASH_HAND_KEY_NAME,
      "close"
    );
  ...
  goto_dock() {
    ...
    if (this.target_object) {
      ...
      target_y =
        this.target_object.sprite.centerY - this.controllable_char.height;
    } else {
      target_x =
        this.controllable_char.x +
        LashFieldPsynergy.LASH_MAX_RANGE * front_pos.x;
      target_y =
        this.controllable_char.y +
    ...
    to_up_tween.onComplete.addOnce(() => {
      const open_hand_key = this._hand_sprite_base.getAnimationKey(
        LashFieldPsynergy.LASH_HAND_KEY_NAME,
        "open"
      );
      ...
      let timer_event = this.game.time.events.add(180, () => {
        ...
        to_down_tween.onComplete.addOnce(() => {
          if (
            this.target_object &&
            this._rope_dock.is_starting_dock &&
            !this._rope_dock.tied
          ) {
            this.tie();
          } else {
  ...
  start_final_emitter(x: number, y: number) {
    const sprite_key =
      this._psynergy_particle_base.getSpriteKey("psynergy_particle");
    ...
    const anim_key = this._psynergy_particle_base.getAnimationKey(
      "psynergy_particle",
      "vanish"
    );
    final_emitter.forEach((particle: Phaser.Sprite) => {
  ...
  async tie() {
    const dest_x_px = get_centered_pos_in_px(
      this._rope_dock.dest_x,
      this.data.map.tile_width
    );
    const dest_y_px = get_centered_pos_in_px(
      this._rope_dock.dest_y,
    ...
    const shake_promise = new Promise(
      (resolve) => (shake_promise_resolve = resolve)
    );
    ...
    const to_dock_promise = new Promise(
      (resolve) => (to_dock_promise_resolve = resolve)
    );
    if (!constains_dest) {
    ...
    const to_dest_promise = new Promise(
      (resolve) => (to_dest_promise_resolve = resolve)
    );
    ...
    const timer_event = this.game.time.events.add(100, async () => {
      ...
      if (!constains_dest) {
        final_promises.push(
          this.data.camera.follow(this.controllable_char, 700)
        );
      }

      let to_out_promise_resolve;
      final_promises.push(
        new Promise((resolve) => (to_out_promise_resolve = resolve))
      );
  ...
  update_rope_frags(completeness: number) {
    const target_index =
      (this._rope_dock.rope_fragments_group.children.length * completeness) | 0;
