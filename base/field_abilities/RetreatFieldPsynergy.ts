...
export class RetreatFieldPsynergy extends FieldAbilities {
  ...
  check_if_can_retreat() {
    ...
    const ability_name =
      this.data.info.abilities_list[RetreatFieldPsynergy.ABILITY_KEY_NAME].name;
    dialog.next_dialog(
      `${ability_name}...`,
      () => {
        next = true;
  ...
  async init() {
    ...
    const promise_tween_pos = new Promise(
      (resolve) => (tween_pos_resolve = resolve)
    );
    this.game.add
    ...
    const promise_tween_scale = new Promise(
      (resolve) => (tween_scale_resolve = resolve)
    );
    this.game.add
    ...
    emitter.destroy();
    this.data.particle_manager.clearData(
  ...
  start_particles_emitter() {
    ...
    this.data.particle_manager.addData(
      RetreatFieldPsynergy.EMITTER_DATA_NAME,
      out_data
    );
  ...
  async move_char() {
    ...
    this.controllable_char.body.x = get_centered_pos_in_px(
      retreat_info.x,
      this.data.map.tile_width
    );
    this.controllable_char.body.y = get_centered_pos_in_px(
      retreat_info.y,
