...
export class AvoidFieldPsynergy extends FieldAbilities {
  ...
  constructor(game, data) {
    super(
      game,
      data,
      AvoidFieldPsynergy.ABILITY_KEY_NAME,
      AvoidFieldPsynergy.ACTION_KEY_NAME,
      false,
      false
    );
  ...
  async init() {
    ...
    dialog.next_dialog(
      "Monsters won't attack so often now.",
  ...
  async aura(total_time: number) {
    ...
    const start_y =
      this.controllable_char.y - (this.controllable_char.height << 1) + 10;
    const end_x = start_x;
    ...
    for (let i = 0; i < auras_count; ++i) {
      ...
      for (let j = 0; j < 2; ++j) {
        const y_delta = +(j === 0);
        const aura = this.data.overlayer_group.create(
          start_x,
          start_y + y_delta,
          "psynergy_aura"
        );
