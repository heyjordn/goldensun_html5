...
export class WhirlwindFieldPsynergy extends FieldAbilities {
  ...
  init() {
    ...
    const sprite_key = this._whirlwind_sprite_base.getSpriteKey(
      WhirlwindFieldPsynergy.ABILITY_KEY_NAME
    );
    ...
    this._whirlwind_sprite_base.setAnimation(
      this._whirlwind_sprite,
      WhirlwindFieldPsynergy.ABILITY_KEY_NAME
    );
    const blow_key = this._whirlwind_sprite_base.getAnimationKey(
      WhirlwindFieldPsynergy.ABILITY_KEY_NAME,
      "blow"
    );
    ...
    if (this.target_object) {
      ...
      target_x =
        this.controllable_char.sprite.centerX +
        front_pos.x * WhirlwindFieldPsynergy.WHIRLWIND_MISS_SHIFT;
      target_y =
        this.controllable_char.sprite.centerY +
        front_pos.y * WhirlwindFieldPsynergy.WHIRLWIND_MISS_SHIFT;
    }
    this.game.add
  ...
  async rotate_leaves() {
    this._previous_angles = new Array(WhirlwindFieldPsynergy.LEAVES_COUNT).fill(
      0
    );
    this._blow_gradient = {
      gradient: WhirlwindFieldPsynergy.BLOW_RO_STEP_SCALE_MIN,
    };
    ...
    const promise_rotation = new Promise(
      (resolve) => (promise_rotation_resolve = resolve)
    );
    const shifts: { x: number; y: number }[] = new Array(
      WhirlwindFieldPsynergy.LEAVES_COUNT
    );
    for (let i = 0; i < WhirlwindFieldPsynergy.LEAVES_COUNT; ++i) {
      const frame_name = `leaves/blowing/0${_.random(5)}`;
      const leaf = this._leaves.create(
        0,
        0,
        "bush",
        frame_name
    ...
    const initial_gradient_factor =
      this._blow_gradient.gradient * gradient_discount;
    rotate_tween.onUpdateCallback(() => {
      ...
      for (let i = 0; i < WhirlwindFieldPsynergy.LEAVES_COUNT; ++i) {
        ...
        leaf.centerX =
          WhirlwindFieldPsynergy.LEAVES_CIRCLE_RAD * Math.cos(this_angle) +
          shifts[i].x;
        leaf.centerY =
          WhirlwindFieldPsynergy.LEAVES_CIRCLE_RAD * Math.sin(this_angle) +
          shifts[i].y;
        if (this_angle > dest_angle_fraction) {
          let new_angle =
            this._previous_angles[i] +
            WhirlwindFieldPsynergy.BLOW_PHI_STEP_SCALE * random_normal();
    ...
    const promise_blowing = new Promise(
      (resolve) => (promise_blowing_resolve = resolve)
    );
    this._blowing = true;
    ...
    const timer_event = this.game.time.events.add(
      WhirlwindFieldPsynergy.BLOW_DURATION,
      promise_blowing_resolve
    );
  ...
  miss_target() {
    const timer_event = this.game.time.events.add(
      WhirlwindFieldPsynergy.MISS_DURATION,
      () => {
        this.finish_whirlwind();
      }
    );
  ...
  finish_whirlwind() {
    const end_key = this._whirlwind_sprite_base.getAnimationKey(
      WhirlwindFieldPsynergy.ABILITY_KEY_NAME,
      "end"
    );
