...
export class RevealFieldPsynergy extends FieldAbilities {
  ...
  constructor(game, data) {
    super(
      ...
      RevealFieldPsynergy.ACTION_KEY_NAME,
      false,
      false
    );
  ...
  update() {
    if (this.controllable_char?.on_reveal) {
      ...
      const limit_sqr_dist =
        Math.pow(limit_point.x - this.casting_point.x, 2) +
        Math.pow(limit_point.y - this.casting_point.y, 2);
  ...
  show_wave() {
    ...
    this.data.super_group.mask = this.game.add.graphics(
      this.reveal_wave.x,
      this.reveal_wave.y
    );
    this.data.super_group.mask.beginFill(0xffffff, 1);
    this.data.super_group.mask.drawRect(
      0,
      0,
      numbers.GAME_WIDTH,
      numbers.GAME_HEIGHT
