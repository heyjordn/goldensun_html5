...
export class MindReadFieldPsynergy extends FieldAbilities {
  ...
  async init() {
    ...
    if (
      !this.target_object ||
      this.target_object.interaction_pattern ===
        interaction_patterns.NO_INTERACTION ||
      !this.target_object.active ||
      this.target_object.base_collision_layer !==
    ...
    this.fire_next_step = this.dialog_manager.next.bind(
      this.dialog_manager,
  ...
  set_arrows() {
    ...
    const arrow_angle = Math.atan2(
      char_head_pos.y - target_head_pos.y,
      char_head_pos.x - target_head_pos.x
    );
