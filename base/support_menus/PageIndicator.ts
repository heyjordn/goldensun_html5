...
export class PageIndicator {
  ...
  public constructor(
    ...
    this.window.define_internal_group(PageIndicator.GROUP_KEY, {
      x: 0,
      y: 0,
    });
  ...
  public initialize(
    ...
    for (let i = 1; i <= this.page_count; ++i) {
      ...
      this.page_numbers.push(
        this.window.set_text_in_position(i.toString(), x, y, {
          is_center_pos: true,
        })
      );
    }
  ...
  public select_page(page_index: number) {
    if (this.mode === PageIndicatorModes.HIGHLIGHT) {
      this.number_bar_highlight.x =
        this.number_bar.x + page_index * PageIndicator.NUMBER_WIDTH;
      ...
      this.page_numbers.forEach((n, index) => {
        n.text.tint =
          index === page_index ? DEFAULT_FONT_COLOR : INACTIVE_FONT_COLOR;
      });
  ...
  private set_arrows() {
    ...
    this.default_arrow_pos = {
      right: this.right_arrow.x,
      left: this.left_arrow.x,
    };
