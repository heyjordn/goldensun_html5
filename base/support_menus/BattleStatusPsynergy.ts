...
export class BattleStatusPsynergy extends StatusComponent {
  ...
  public select_option() {
    const highlight = {
      ...
      y:
        BattleStatusPsynergy.HIGHLIGHT.Y +
        BattleStatusPsynergy.SHIFT * this.current_line,
  ...
  public on_change() {
    ...
    const chosen_ability =
      this.battle_abilities[this.current_col][this.current_line];
  ...
  public on_up() {
    ...
    if (this.current_line === 0) {
      if (this.current_col === 0) {
        ...
        this.current_line =
          this.battle_abilities[this.battle_abilities.length - 1].length - 1;
      } else {
  ...
  public on_down() {
    ...
    if (
      this.current_line + 1 ===
      this.battle_abilities[this.current_col].length
    ) {
  ...
  public initialize() {
    ...
    if (!this.battle_abilities[this.current_col])
      this.current_col = this.battle_abilities.length - 1;
    ...
    abilities.forEach((ability, index) => {
      ...
      let y_pos =
        BattleStatusPsynergy.PSYNERGY.ICON_Y +
        index * BattleStatusPsynergy.SHIFT;
      ...
      y_pos =
        BattleStatusPsynergy.PSYNERGY.NAME_Y +
        index * BattleStatusPsynergy.SHIFT;
      ...
      y_pos =
        BattleStatusPsynergy.PSYNERGY.PP_LABEL_Y +
        index * BattleStatusPsynergy.SHIFT;
      ...
      y_pos =
        BattleStatusPsynergy.PSYNERGY.PP_VALUE_Y +
        index * BattleStatusPsynergy.SHIFT;
      ...
      y_pos =
        BattleStatusPsynergy.PSYNERGY.STAR_Y +
        index * BattleStatusPsynergy.SHIFT;
      ...
      y_pos =
        BattleStatusPsynergy.PSYNERGY.RANGE_Y +
        index * BattleStatusPsynergy.SHIFT;
  ...
  private update_abilities() {
    ...
    all_abilities.forEach((key_name) => {
      ...
      if (!this.data.info.abilities_list[key_name]) {
        console.warn(
          "Ability " + '"' + key_name + '"' + " does not exist in the database."
        );
