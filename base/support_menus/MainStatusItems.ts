...
export class MainStatusItems extends StatusComponent {
  ...
  public select_option() {
    const highlight = {
      ...
      y:
        MainStatusItems.HIGHLIGHT.Y + MainStatusItems.SHIFT * this.current_line,
    ...
    const cursor_y =
      MainStatusItems.CURSOR.Y + MainStatusItems.SHIFT * this.current_line;
  ...
  public on_change() {
    ...
    const eq_slot =
      item_equip_slot[this.data.info.items_list[chosen_item.key_name].type];
    const slots = [
      equip_slots.WEAPON,
      equip_slots.BODY,
      equip_slots.CHEST,
    ...
    if (slots.includes(eq_slot) && chosen_item.equipped) {
      const eq_highlight_shift =
        MainStatusItems.EQ_HIGHLIGHT.SHIFT *
        MainStatusItems.EQ_HIGHLIGHT.POS[eq_slot];
    ...
    this.update_description(
      this.data.info.items_list[chosen_item.key_name].description
    );
  }
  ...
  public on_up() {
    ...
    if (this.current_line === 0) {
      if (this.current_col === 0) {
        ...
        this.current_line =
          this.char_items[this.char_items.length - 1].length - 1;
      } else {
  ...
  public initialize() {
    ...
    if (!this.char_items[this.current_col])
      this.current_col = this.char_items.length - 1;
