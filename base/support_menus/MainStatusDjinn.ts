...
export class MainStatusDjinn extends StatusComponent {
  ...
  public on_left() {
    ...
    this.current_page =
      (this.current_page + this.djinn_pages.length - 1) %
  ...
  private load_page() {
    ...
    for (let i = 0; i < ordered_elements.length; i++) {
      ...
      const x_pos =
        MainStatusDjinn.SPRITES.CENTER_X + i * MainStatusDjinn.SPRITES.SHIFT;
  ...
  private make_pages() {
    ...
    const highest_count = (
      _.maxBy(
        Object.values(djinn_list),
        (list: Djinn[]) => list.length
      ) as Djinn[]
    ).length;
    ...
    for (let i = 0; i < n_pages; i++) {
      const page: DjinnList = ordered_elements.reduce((result, elem) => {
        result[elem] = djinn_list[elem].slice(
          max_per_page * i,
          max_per_page * (i + 1) - 1
        );
        return result;
  ...
  private get_djinn_by_element() {
    const djinn = ordered_elements.reduce((result, elem) => {
      const char_djinn = this.data.info.party_data.members
        .map((char) => char.djinn_by_element[elem])
        .flat();
