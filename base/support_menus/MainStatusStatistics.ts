...
import {
  BattleStatusEffect,
  BattleStatusWindow,
} from "../windows/battle/BattleStatusWindow";
...
export class MainStatusStatistics extends StatusComponent {
  ...
  public static readonly MenuDescriptions = {
    [MainStatistics.NAME]: {
      line1: "Your name and level. Experience",
      line2: "points increase your level.",
    },
    [MainStatistics.EXP]: {
    ...
    [MainStatistics.HP]: {
      line1: "Your current and max Hit Points.",
      line2: "At zero HP you cannot fight.",
    },
    [MainStatistics.PP]: {
      line1: "Your Psynergy Points. They",
      line2: "recover as you travel.",
    },
    [MainStatistics.DJINN]: {
      line1: "Your Djinn. This also shows",
      line2: "which ones are set.",
    },
    [MainStatistics.ELEM_LEVELS]: {
      line1: "Elemental Levels indicate the skill",
      line2: "level of the elemental attribute.",
    },
    [MainStatistics.ELEM_POWER]: {
      line1: "Power reflects the damage you",
      line2: "can do with each element.",
    },
  ...
  public select_option() {
    ...
    if (this.current_col !== 0) {
      ...
      highlight.height =
        MainStatusStatistics.HIGHLIGHT[this.current_line].HEIGHT;
      ...
      highlight.y =
        MainStatusStatistics.HIGHLIGHT.EFFECT.Y +
        highlight_shift * this.current_line;
      ...
      cursor_y =
        MainStatusStatistics.CURSOR.EFFECT.Y + cursor_shift * this.current_line;
    }
    ...
    this.data.cursor_manager.move_to(
      ...
      { animate: false, tween_config: cursor_tween }
    );
  }
  ...
  public on_change() {
    ...
    if (this.current_col === 0) {
      if (this.battle_status_effects.length === 0)
        msg_obj = MainStatusStatistics.MenuStatusMsgs.normal;
      else
        msg_obj =
          MainStatusStatistics.MenuStatusMsgs[
            this.battle_status_effects[this.current_line].key
          ];
    } else {
      msg_obj = { line1: null, line2: null };
      msg_obj.line1 =
        MainStatusStatistics.MenuDescriptions[this.current_line].line1;

      if (this.current_line === MainStatistics.EXP) {
        const exp =
          this.selected_char.exp_curve[this.selected_char.level] -
          this.selected_char.current_exp;
        msg_obj.line2 =
          MainStatusStatistics.MenuDescriptions[this.current_line].line2(exp);
      } else
        msg_obj.line2 =
          MainStatusStatistics.MenuDescriptions[this.current_line].line2;
    }
  ...
  public on_up() {
    ...
    if (this.current_col === 0) {
      ...
      this.current_line =
        (this.current_line + effects_count - 1) % effects_count;
    } else
      this.current_line =
        (this.current_line + MainStatusStatistics.LINES - 1) %
        MainStatusStatistics.LINES;
  ...
  public on_down() {
    ...
    if (this.current_col === 0) {
      ...
      this.current_line = (this.current_line + 1) % MainStatusStatistics.LINES;
  ...
  public initialize() {
    ...
    this.djinn_group.y =
      this.game.camera.y + MainStatusStatistics.DJINN_GROUP_Y_OFFSET;

    this.state = (this.manager as MainStatusMenu).state;

    for (let i = 0; i < ordered_elements.length; i++) {
      const x_pos =
        MainStatusStatistics.ELEM.STARS.X +
        i * MainStatusStatistics.ELEM.STARS_SHIFT;
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      let y_pos =
        MainStatusStatistics.ELEM.LABEL.Y +
        i * MainStatusStatistics.ELEM.LABEL_SHIFT;
    ...
    for (let i = 0; i < ordered_elements.length; i++) {
      ...
      const x_pos =
        MainStatusStatistics.ELEM.NUMBERS.END_X +
        i * MainStatusStatistics.ELEM.NUMBERS.X_SHIFT;
      ...
      for (let i = 0; i < ordered_elements.length; i++) {
        ...
        const x_pos =
          MainStatusStatistics.ELEM.DJINN.CENTER_X +
          i * MainStatusStatistics.ELEM.DJINN.X_SHIFT;
        let y_pos = MainStatusStatistics.ELEM.DJINN.Y;

        if (this.state === MainStatusStates.CHARACTERS)
          y_pos += MainStatusStatistics.ELEM.DOWN_SHIFT;

        const djinni_sprite = this.get_djinni_sprite(elem, this.djinn_group, {
          x: x_pos,
          y: y_pos,
        });
  ...
  init_status_effects() {
    if (this.battle_status_effects.length === 0) {
      ...
      for (let i = 0; i < this.battle_status_effects.length; i++) {
        ...
        let y_pos =
          MainStatusStatistics.EFFECTS.NAME_Y +
          i * MainStatusStatistics.EFFECTS.SHIFT;
        ...
        y_pos =
          MainStatusStatistics.EFFECTS.ICON_Y +
          i * MainStatusStatistics.EFFECTS.SHIFT;
