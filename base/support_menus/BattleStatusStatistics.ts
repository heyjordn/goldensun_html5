...
export class BattleStatusStatistics extends StatusComponent {
  ...
  public static readonly BattleDescriptions = {
    [BattleStatistics.NAME]: {
      line1: "Use the L & R Buttons to",
      line2: "switch between characters.",
    },
    [BattleStatistics.EXP]: {
      line1: "Current experience points.",
      line2: (exp: number) => `${exp} to next level.`,
    },
    [BattleStatistics.HP]: {
      line1: "Your current and maximum HP.",
      line2: "Affected by Djinn and equipment.",
    },
    [BattleStatistics.PP]: {
      line1: "Your current and maximum PP.",
      line2: "Affected by Djinn and equipment.",
    },
    [BattleStatistics.CLASS]: {
      line1: "Your current class. Your",
      line2: "class changes when you set Djinn.",
    },
  ...
  private static readonly ELEM_NUMBERS = {
    END_X: 69,
    Y: 80,
    X_SHIFT: 32,
    Y_SHIFT: 8,
  ...
  public select_option() {
    ...
    if (this.current_col === 0) {
      ...
      highlight.width =
        BattleStatusStatistics.HIGHLIGHT[this.current_line].WIDTH;
      highlight.height =
        BattleStatusStatistics.HIGHLIGHT[this.current_line].HEIGHT;
      ...
      if (this.battle_status_effects.length === 0) {
        ...
        highlight.x =
          BattleStatusStatistics.HIGHLIGHT.EFFECT.X +
          highlight_shift * (this.current_col - 1);
      ...
      cursor_x =
        BattleStatusStatistics.CURSOR.EFFECT.X +
        cursor_shift * (this.current_col - 1);
  ...
  public on_change() {
    ...
    if (this.current_col === 0) {
      const msgs = {
        line1:
          BattleStatusStatistics.BattleDescriptions[this.current_line].line1,
        line2:
          BattleStatusStatistics.BattleDescriptions[this.current_line].line2,
      };
      ...
      if (this.battle_status_effects.length === 0) {
        ...
        let msgs = null;
        if (
          ordered_status_battle.includes(
            effect.key as temporary_status | permanent_status
          )
        ) {
          msgs = {
          ...
          for (let i = 0; i < elems_to_show.length; i++) {
            if (elems_to_show[i].value >= 0)
              elems_to_show[i].value = "+" + elems_to_show[i].value;
          ...
          msgs = {
            line1:
              BattleStatusStatistics.BattleBuffMsgs[effect.properties.modifier][
                effect.key
              ].line1,
            line2:
              BattleStatusStatistics.BattleBuffMsgs[effect.properties.modifier][
  ...
  public on_left() {
    ...
    if (effects_count === 0) this.current_col = this.current_col === 0 ? 1 : 0;
      ...
      this.current_col =
        (this.current_col + (effects_count + 1) - 1) % (effects_count + 1);
  ...
  public on_up() {
    if (this.current_col !== 0) this.current_col = 0;
      ...
      this.current_line =
        (this.current_line + BattleStatusStatistics.LINES - 1) %
        BattleStatusStatistics.LINES;
  ...
  public on_down() {
    if (this.current_col !== 0) this.current_col = 0;
      ...
      this.current_line =
        (this.current_line + 1) % BattleStatusStatistics.LINES;
  ...
  public initialize() {
    for (let i = 0; i < ordered_elements.length; i++) {
      const x_pos =
        BattleStatusStatistics.STARS.X + i * BattleStatusStatistics.STARS_SHIFT;
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      const y_pos =
        BattleStatusStatistics.LABEL.Y + i * BattleStatusStatistics.LABEL_SHIFT;
    ...
    for (let i = 0; i < ordered_elements.length; i++) {
      ...
      const x_pos =
        BattleStatusStatistics.ELEM_NUMBERS.END_X +
        i * BattleStatusStatistics.ELEM_NUMBERS.X_SHIFT;
