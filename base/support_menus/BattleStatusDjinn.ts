...
export class BattleStatusDjinn extends StatusComponent {
  ...
  public select_option() {
    const highlight = {
      x:
        BattleStatusDjinn.HIGHLIGHT.X +
        BattleStatusDjinn.SHIFT_X * this.current_col,
      y:
        BattleStatusDjinn.HIGHLIGHT.Y +
        BattleStatusDjinn.SHIFT_Y * this.current_line,
    ...
    const cursor_x =
      BattleStatusDjinn.CURSOR.X + BattleStatusDjinn.SHIFT_X * this.current_col;
    const cursor_y =
      BattleStatusDjinn.CURSOR.Y +
      BattleStatusDjinn.SHIFT_Y * this.current_line;
  ...
  public on_change() {
    if (!this.char_djinn[this.current_col])
    ...
    const msg =
      chosen_djinn.status === djinn_status.STANDBY
        ? "Ready to summon."
        : chosen_djinn.description;
  ...
  public on_up() {
    ...
    if (this.current_line === 0) {
      if (this.current_col === 0) {
        this.current_col = this.char_djinn.length - 1;
        this.current_line =
          this.char_djinn[this.char_djinn.length - 1].length - 1;
      } else {
  ...
  public initialize() {
    ...
    this.char_djinn.forEach((col, col_index) => {
      col.forEach((djinn, line_index) => {
        ...
        let x_pos =
          BattleStatusDjinn.DJINN.STAR_X +
          col_index * BattleStatusDjinn.SHIFT_X;
        let y_pos =
          BattleStatusDjinn.DJINN.STAR_Y +
          line_index * BattleStatusDjinn.SHIFT_Y;
        ...
        if (status === djinn_status.RECOVERY) {
          ...
          x_pos =
            BattleStatusDjinn.DJINN.RECOVERY_END_X +
            col_index * BattleStatusDjinn.SHIFT_X;
          y_pos =
            BattleStatusDjinn.DJINN.RECOVERY_Y +
            line_index * BattleStatusDjinn.SHIFT_Y;
        ...
        x_pos =
          BattleStatusDjinn.DJINN.NAME_X +
          col_index * BattleStatusDjinn.SHIFT_X;
        y_pos =
          BattleStatusDjinn.DJINN.NAME_Y +
          line_index * BattleStatusDjinn.SHIFT_Y;
