...
export async function initialize_game_data(game: Phaser.Game, data: GoldenSun) {
  ...
  data.info.maps_list = initialize_maps(
    game,
    data,
    data.dbs.maps_db,
    snapshot,
    load_maps_promise_resolve
  );
  ...
  const load_enemies_sprites_promise = new Promise(
    (resolve) => (load_enemies_sprites_promise_resolve = resolve)
  );
  data.info.enemies_list = initialize_enemies(
    game,
    data,
    data.dbs.enemies_db,
    load_enemies_sprites_promise_resolve
  );
  ...
  const load_abilities_promise = new Promise(
    (resolve) => (load_abilities_promise_resolve = resolve)
  );
  data.info.abilities_list = initialize_abilities(
    game,
    data,
    data.dbs.abilities_db,
    load_abilities_promise_resolve
  );
  await load_abilities_promise;

  // Initializes ITEMS
  let load_items_promise_resolve;
  const load_items_promise = new Promise(
    (resolve) => (load_items_promise_resolve = resolve)
  );
  data.info.items_list = initialize_items(
    game,
    data,
    data.dbs.items_db,
    load_items_promise_resolve
  );
  ...
  data.info.party_data = {
    ...
    visited_shops: snapshot?.visited_shops
      ? new Set<string>(snapshot.visited_shops)
      : new Set<string>(),
    psynergies_shortcuts: {
  ...
  const load_chars_promise = new Promise(
    (resolve) => (load_chars_promise_resolve = resolve)
  );
  ...
  const load_misc_promise = new Promise(
    (resolve) => (load_misc_promise_resolve = resolve)
  );
  ...
  const load_iter_objs_promise = new Promise(
    (resolve) => (load_iter_objs_promise_resolve = resolve)
  );
  ...
  const load_npcs_promise = new Promise(
    (resolve) => (load_npcs_promise_resolve = resolve)
  );
  data.info.npcs_sprite_base_list = initialize_npcs_data(
    game,
    data,
    data.dbs.npc_db,
    load_npcs_promise_resolve
  );
  ...
  data.info.artifacts_global_list =
    snapshot?.artifacts_global_list ?? data.dbs.init_db.artifacts_global_list;
  data.info.artifacts_global_list.forEach(
    (item_data) => (item_data.global_artifact = true)
  );
}
