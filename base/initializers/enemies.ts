...
export function initialize_enemies(
  ...
  for (let i = 0; i < enemies_db.length; ++i) {
    ...
    info.sprite_base = new SpriteBase(info.data.key_name, [
      base_actions.BATTLE,
    ]);
    ...
    if (action !== undefined) {
      info.sprite_base.setActionSpritesheet(
        base_actions.BATTLE,
        action.spritesheet_img,
        action.spritesheet
      );
      info.sprite_base.setActionAnimations(
        base_actions.BATTLE,
        action.positions,
        action.frames_number
      );
      info.sprite_base.setActionFrameRate(
        base_actions.BATTLE,
        action.frame_rate
      );
