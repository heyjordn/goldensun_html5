...
export function initialize_main_chars(
  ...
  for (let i = 0; i < main_chars_db.length; ++i) {
    ...
    char_snapshot_data?.permanent_status?.forEach((status) =>
      main_char.add_permanent_status(status)
    );

    if (char_snapshot_data?.learnt_abilities) {
      char_snapshot_data.learnt_abilities.forEach((ability) =>
        main_char.learn_ability(ability, false)
      );
    ...
    main_char.current_hp =
      char_snapshot_data?.current_hp ?? main_char.current_hp;
    main_char.current_pp =
      char_snapshot_data?.current_pp ?? main_char.current_pp;
    ...
    if (
      Object.keys(char_data.weapons_sprites).length &&
      base_actions.BATTLE in char_db.actions
    ) {
      ...
      for (let weapon_type in char_data.weapons_sprites) {
        ...
        weapons_sprite_base.setActionAnimations(
          weapon_type,
          action.animations,
          action.frames_count
        );
