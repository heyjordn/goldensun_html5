  ...
  for (let npc_key in npc_db) {
    ...
    for (let i = 0; i < actions.length; ++i) {
      ...
      npc_sprite_info.setActionSpritesheet(
        action_key,
        action_obj.spritesheet.image,
        action_obj.spritesheet.json
      );
      npc_sprite_info.setActionAnimations(
        action_key,
        action_obj.animations,
        action_obj.frames_count
      );
  ...
  game.load.onLoadComplete.addOnce(() => {
    for (let npc_key in npc_db) {
      ...
      for (let action_alias in npc_info.action_aliases) {
        npc_sprite_info.setActionAlias(
          game,
          action_alias,
          npc_info.action_aliases[action_alias]
        );
      }
    }
