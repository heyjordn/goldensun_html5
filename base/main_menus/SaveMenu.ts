...
export class SaveMenu {
  ...
  constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.button = this.window.create_at_group(30, 16, "buttons", {
      frame: "save_game",
    });
    this.button.anchor.setTo(0.5, 0.5);

    this.window.set_text_in_position("Save your quest", 48, 10, {
      italic: true,
    });
  }
  ...
  set_controls() {
    const controls = [
      {
        buttons: Button.A,
        on_down: this.download_snapshot.bind(this),
        sfx: { down: "menu/positive" },
      },
      {
        buttons: Button.B,
        on_down: this.close_menu.bind(this),
  ...
  set_button_tween() {
    ...
    this.button_tween = this.game.add
      ...
      .to(
        { x: SaveMenu.MAX_SCALE, y: SaveMenu.MAX_SCALE },
        125,
