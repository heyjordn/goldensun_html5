...
export class MainItemMenu {
  ...
  constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.item_change_stats_window = new StatsOrClassCheckWithItemWindow(
      this.game,
      this.data
    );
  ...
  config_arrange_window() {
    this.arrange_window.create_at_group(9, 9, "keyboard_buttons", {
      frame: "l_button",
      color: 0x0,
    });
    this.arrange_window.create_at_group(8, 8, "keyboard_buttons", {
      frame: "l_button",
    });
    this.arrange_window.set_text_in_position("+", 23, 9);
    this.arrange_window.create_at_group(30, 9, "keyboard_buttons", {
      frame: "a_button",
      color: 0x0,
    });
    this.arrange_window.create_at_group(29, 8, "keyboard_buttons", {
      frame: "a_button",
    });
    this.arrange_window.set_text_in_position(": Arrange items", 38, 8);

    this.arrange_window.create_at_group(9, 17, "keyboard_buttons", {
      frame: "r_button",
      color: 0x0,
    });
    this.arrange_window.create_at_group(8, 16, "keyboard_buttons", {
      frame: "r_button",
    });
  ...
  config_gear_window() {
    ...
    this.gear_overview_items = {
      weapon: {
        ...
        item_obj: this.gear_overview_window.make_item_obj("", {
          x: 112,
          y: 8,
        }),
      },
  ...
  shift_item_overview(down: boolean, hide_sub_menus: boolean = true) {
    ...
    this.item_overview_window.update_position({
      y: ITEM_OVERVIEW_WIN_Y + (down ? ITEM_OVERVIEW_Y_SHIFT : 0),
    });
    this.item_overview_window.update_size({
      height:
        ITEM_OVERVIEW_WIN_HEIGHT + (down ? ITEM_OVERVIEW_HEIGHT_SHIFT : 0),
    });
  ...
  char_change() {
    ...
    if (this.choosing_destination) {
      if (this.item_options_window.item.type === item_types.ABILITY_GRANTOR) {
        ...
        this.item_options_window.item.type !== item_types.GENERAL_ITEM
      ) {
        const preview_obj = Object.assign(
          {},
      ...
      this.set_description_window_text(
        this.item_options_window.item.description,
        true
      );
    } else {
      if (
        this.item_choose_window.window_open &&
        !this.item_options_window.window_open
      ) {
  ...
  char_choose() {
    ...
    this.item_choose_window.grant_control(
      ...
      () => {
        ...
        this.selected_item_pos = {
          page: item_win.page_index,
          index: item_win.selected_element_index,
        };

        const selected_item =
          item_win.element_list[
            (item_win.elements[item_win.selected_element_index] as ItemSlot)
              .key_name
          ];
        const selected_item_obj =
          item_win.item_objs[item_win.selected_element_index];
  ...
  set_guide_window_text() {
    if (this.choosing_item) {
      this.guide_window.update_text(
        this.guide_window_msgs.choosing_item,
  ...
  set_item_icons() {
    ...
    for (
      let i = 0;
      i <
      this.data.info.party_data.members[this.selected_char_index].items.length;
      ++i
    ) {
      const item_obj =
        this.data.info.party_data.members[this.selected_char_index].items[i];
  ...
  open_char_select() {
    ...
    if (!this.item_overview_window.open)
  ...
  set_gear_window(show_window: boolean = true) {
    ...
    for (let slot_type in this.gear_overview_items) {
      ...
      if (slot) {
        ...
        this.gear_overview_window.update_text(
          name,
          this.gear_overview_items[slot_type].text_obj
        );
        this.gear_overview_window.set_text_obj_visibility(
          true,
          this.gear_overview_items[slot_type].text_obj
        );
        this.gear_overview_items[slot_type].item_obj.icon.frameName =
          slot.key_name;
  ...
  show_details(item: Item, item_slot: ItemSlot, callback?: () => void) {
    ...
    if (item.type === item_types.ABILITY_GRANTOR) {
      const ability_name =
        this.data.info.abilities_list[item.granted_ability].name;
      details_equip.push(`  Bestows ${ability_name}.`);
    }
    if (item.use_ability) {
      const ability_desciption =
        this.data.info.abilities_list[item.use_ability].description;
      details_use.push(
        `  ${ability_desciption ? ability_desciption : item.description}`
      );
    }
    ...
    if (item.unleash_ability) {
      const ability_name =
        this.data.info.abilities_list[item.unleash_ability].name;
    ...
    if (
      item_slot.additional_details !== undefined &&
      Object.keys(item_slot.additional_details).length
    ) {
    ...
    const text_objs = this.details_window.set_lines_of_text(lines, {
