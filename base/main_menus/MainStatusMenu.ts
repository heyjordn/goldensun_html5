...
export class MainStatusMenu {
  ...
  public constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.chars_menu = new CharsMenu(
      this.game,
      this.data,
      this.on_character_change.bind(this)
    );
    ...
    this.equip_window.add_to_internal_group(
  ...
  private set_battle_effects() {
    ...
    const status_effects = _.sortBy(
      [
        ...this.data.info.main_char_list[this.selected_char.key_name]
          .permanent_status,
      ],
    ...
    for (let index in status_effects) {
      ...
      if (effects.length < MainStatusMenu.MAX_EFFECTS_DISPLAYED)
        effects.push(effect);
    }
  ...
  public update_description(line1: string, line2?: string) {
    ...
    const desc_win_group = this.desc_window.get_internal_group(
      MainStatusMenu.DESC_GUIDE_KEY
    );
    const desc_guide_states = [
      MainStatusStates.STATISTICS,
      MainStatusStates.PSYNERGY,
      MainStatusStates.ITEMS,
    ];
    ...
    if (text2 === "" && desc_guide_states.includes(this.current_state)) {
      ...
      next_state_name =
        next_state_name.charAt(0).toUpperCase() + next_state_name.slice(1);
  ...
  private toggle_guide_win() {
    ...
    const new_text =
      this.select_text.text.text === ": Return" ? ": Djinn  list" : ": Return";
  ...
  private check_main_components() {
    if (this.current_state === MainStatusStates.DJINN) {
      this.main_window.get_internal_group(MainStatusMenu.GROUP_KEY).visible =
        false;
    } else {
      let label_x = 0;
      let label_y = 0;
      ...
      let label = "";
      if (
        [MainStatusStates.CHARACTERS, MainStatusStates.STATISTICS].includes(
          this.current_state
        )
      ) {
      ...
      this.main_window.update_text_position(
        { x: label_x, y: label_y },
        this.level_label
      );
      this.main_window.update_text_position(
        { x: value_end_x, y: value_y },
        this.level_value
      );
  ...
  private initialize() {
    ...
    this.active_sprites.push(
      this.select_button.sprite,
      this.select_button.shadow
    );
    ...
    this.active_sprites.push(
      this.eq_weapon_name.text,
      this.eq_weapon_name.shadow
    );
    ...
    this.active_sprites.push(
      this.eq_shield_name.text,
      this.eq_shield_name.shadow
    );
    ...
    this.active_sprites.push(
      this.eq_chest_name.text,
      this.eq_chest_name.shadow
    );
  ...
  private update_info() {
    ...
    this.equip_window.update_text(
      names[equip_slots.WEAPON],
      this.eq_weapon_name
    );
    this.equip_window.update_text(names[equip_slots.HEAD], this.eq_head_name);
    this.equip_window.update_text(
      names[equip_slots.CHEST],
      this.eq_shield_name
    );
  ...
  private on_character_change(char?: MainChar | string) {
    ...
    if (this.current_state === null || this.current_state === undefined)
      state = MainStatusStates.CHARACTERS;
  ...
  public inner_control() {
    ...
    if (this.current_state !== MainStatusStates.DJINN) {
      controls.push(
        {
          buttons: Button.A,
          on_down: this.trigger_state_change.bind(this),
          sfx: { down: "menu/positive" },
        },
        {
          buttons: Button.B,
          on_down: this.selecting_char.bind(this),
  ...
  private change_state(
    new_state: MainStatusStates,
    reset_pos: boolean = false
  ) {
    let pos = { line: 0, col: 0 };

    if (
      new_state === MainStatusStates.DJINN ||
      this.current_state === MainStatusStates.DJINN
    ) {
    ...
    if (
      ![MainStatusStates.CHARACTERS, MainStatusStates.DJINN].includes(
        new_state
      ) &&
      !this.desc_window.open
    ) {
      this.desc_window.show(undefined, false);
    } else if (
      new_state === MainStatusStates.CHARACTERS &&
      this.desc_window.open
    ) {
    ...
    if (new_state === MainStatusStates.ITEMS && !this.equip_window.open) {
      ...
      !(new_state === MainStatusStates.ITEMS) &&
      this.equip_window.open
    ) {
    ...
    this.current_component =
      this.components[MainStatusMenu.StateComponent[this.current_state]];

    if (this.current_state === MainStatusStates.DJINN)
      this.current_component.initialize();
    else this.current_component.reset(pos);

    if (this.current_state !== MainStatusStates.CHARACTERS)
      this.inner_control();
  ...
  public open_menu(close_callback?: Function, open_callback?: Function) {
    ...
    this.main_window.group.bringToTop(
      this.main_window.get_internal_group(MainStatusMenu.GROUP_KEY)
    );
  ...
  public update_eq_highlight(highlight: {
    x: number;
    y: number;
    width: number;
    height: number;
