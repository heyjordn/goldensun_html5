...
export class InnMenu {
  ...
  constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.coin_number = this.coin_window.set_text_in_position(
  ...
  start(inn_id: string, voice_key?: string, close_callback?: Function) {
    ...
    this.message =
      this.data.info.inn_list[this.inn_id].messages.welcome_message;
  ...
  update_dialogue(callback?: (finished: boolean) => void) {
    if (this.message.includes("${PRICE}"))
      this.message = this.message.replace("${PRICE}", String(this.cost));
  ...
  inn_price() {
    ...
    this.callback = () => {
      ...
      this.choice_menu.open(
        ...
        {
          x:
            this.dialog_manager.current_width / 2 -
            this.choice_menu.menu.title_window_width / 2 -
            2,
  ...
  inn_rest() {
    ...
    if (this.data.info.party_data.coins >= this.cost) {
      ...
      this.message =
        this.data.info.inn_list[this.inn_id].messages.not_enough_coins;
      this.update_dialogue();
  ...
  inn_fade() {
    ...
    fadeout.onComplete.addOnce(() => {
      ...
      this.message =
        this.data.info.inn_list[this.inn_id].messages.goodbye_message;
      this.update_dialogue();
