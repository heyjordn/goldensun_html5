...
const dialog_msgs = {
  ...
  [DialogTypes.RECUSE_DONATION]:
    "The donation is too much? Then weary wanderers you shall remain.",
  [DialogTypes.LEAVE]: "Visit us again anytime you need healing.",

  [DialogTypes.REVIVE_INIT]:
    "Hmm, you were downed in battle and need reviving, do you?",
  [DialogTypes.NO_DOWNED]: "Fear not, none of your companions is down.",
  [DialogTypes.SELECT_REVIVE]: "Who shall I revive?",
  [DialogTypes.DONATION_REVIVE]: (price, char) =>
    `Reviving ${char} requires a donation of ${price} coins. OK?`,
  [DialogTypes.NO_COIN_REVIVE]:
    "I'm sorry, but you must be able to pay the donation.",
  [DialogTypes.ACCEPT_REVIVE]: "Then I call upon my healing powers.",
  [DialogTypes.SUCCESS_REVIVE]: (char) => `${char} has been revived!`,

  [DialogTypes.POISON_INIT]:
    "Hmm, so you need an antidote to poison or deadly poison?",
  [DialogTypes.NO_POISON]:
    "Fear not! None of your companions has been poisoned!",
  [DialogTypes.SELECT_POISON]: "Whom shall I cure?",
  [DialogTypes.DONATION_POISON]: (price, char) =>
    `I require a donation of ${price} coins to cure ${char}, OK?`,
  [DialogTypes.NO_COIN_POISON]:
    "You cannot cover the required donation, so I cannot provide a cure.",
  ...
  [DialogTypes.NO_COIN_HAUNT]:
    "You cannot cover the required donation, so I cannot aid you.",
  ...
  [DialogTypes.CURSE_INIT]:
    "Hmm, so you wish to have the cursed equipment removed, do you?",
  [DialogTypes.NO_CURSE]:
    "Fear not! None of your companions has any cursed gear!",
  [DialogTypes.SELECT_CURSE]: "From whom shall I remove the curse?",
  [DialogTypes.DONATION_CURSE]: (price, char) =>
    `Donate ${price} coins for me to remove ${char}${
      char.endsWith("s") ? "'" : "'s"
    } curse.`,
  [DialogTypes.NO_COIN_CURSE]:
    "You cannot cover the required donation, so I cannot aid you.",
  [DialogTypes.ACCEPT_CURSE]: "Then I shall remove the curse.",
  [DialogTypes.SUCCESS_CURSE]: (char) =>
    `The curse has been removed from ${char}.`,
};
...
export class HealerMenu {
  ...
  private setup_info_window() {
    ...
    this.info_text = this.info_window.set_text_in_position("", 6, 6, {
      italic: true,
    });
  }
  ...
  private set_dialog(options: {
    ...
    const msg =
      options?.custom_msg ?? (dialog_msgs[options?.dialog_type] as string);
  ...
  private party_has_status(perm_status: permanent_status) {
    ...
    this.set_dialog({
      ...
      callback: () => {
        ...
        const first_char_index = this.data.info.party_data.members.findIndex(
          (c) => c.has_permanent_status(perm_status)
        );
        this.char_change(
          this.data.info.party_data.members[first_char_index].key_name
        );
  ...
  private char_select() {
    ...
    if (char.has_permanent_status(this.selected_perm_status)) {
      const input_and_crystal = [
        permanent_status.HAUNT,
        permanent_status.EQUIP_CURSE,
      ].includes(this.selected_perm_status);
      const price = this.get_price(char, this.selected_perm_status);
      const donation_msg = dialog_msgs[
        status_dialogs_map.donation[this.selected_perm_status]
      ](price, char.name);
      this.set_dialog({
    ...
    this.yes_no_menu.open(
      {
        yes: () => {
          if (price > this.data.info.party_data.coins) {
            this.set_dialog({
              dialog_type:
                status_dialogs_map.no_coin[this.selected_perm_status],
              ask_for_input: true,
              show_crystal: true,
              callback: () => {
  ...
  private async remove_status(char: MainChar) {
    ...
    this.coins_window.update_text(
      this.data.info.party_data.coins.toString(),
      this.coins_number
    );
    this.set_dialog({
      ask_for_input: true,
      show_crystal: false,
      custom_msg: dialog_msgs[
        status_dialogs_map.success[this.selected_perm_status]
      ](char.name),
      callback: () => {
        const has_status = this.data.info.party_data.members.some((c) =>
  ...
  private start_particles_emitter(char_sprite: Phaser.Sprite) {
    ...
    const circle_zone = this.data.particle_manager.createCircleZone(
      char_sprite.height << 1
    );
