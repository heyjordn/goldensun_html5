...
export class ShopMenu {
  ...
  constructor(game: Phaser.Game, data: GoldenSun) {
    ...
    this.shops_db = _.mapKeys(
      this.data.dbs.shops_db,
      (shop) => shop.key_name
    ) as { [key_name: string]: Shop };
    ...
    this.inv_win = new InventoryWindow(
      this.game,
      this.data,
      this.on_inv_win_change.bind(this)
    );
    this.buy_select = new BuySelectMenu(
      this.game,
      this.data,
      this.on_buy_select_change.bind(this)
    );
    this.eq_compare = new EquipCompare(this.game, this.data);
    this.quant_win = new ShopItemQuantityWindow(this.game, this.data);
    this.char_display = new CharsMenu(
      this.game,
      this.data,
      this.on_char_display_change.bind(this)
    );
    ...
    this.item_name_text = this.item_price_win.set_text_in_position(
      "",
      ITEM_PRICE_NAME_X,
      ITEM_PRICE_NAME_Y
    );
    ...
    this.item_desc_text = this.item_desc_win.set_text_in_position(
      "",
      ITEM_DESC_TEXT_X,
      ITEM_DESC_TEXT_Y
    );
  ...
  on_inv_win_change(line: number, col: number) {
    if (this.item_price_win.open && this.sell_menu.active) {
      ...
      if (itm) {
        ...
        let important_item =
          this.data.info.items_list[itm.key_name].important_item;
        ...
        if (is_repair) price_val = (item_price * REPAIR_MULTIPLIER) | 0;
          ...
          price_val =
            (item_price *
              (itm.broken ? SELL_BROKEN_MULTIPLIER : SELL_MULTIPLIER)) |
            0;
  ...
  set_item_lists() {
    const shop_visited = this.data.info.party_data.visited_shops.has(
      this.shop_key
    );
    ...
    item_list.forEach((item_data) => {
      if (item_data.quantity && this.items_db[item_data.key_name].rare_item) {
        if (!shop_visited) {
          const global_items = this.data.info.artifacts_global_list.find(
            (global_item_data) => {
    ...
    const artifact_list = this.data.info.artifacts_global_list.filter(
      (item_data) => {
    ...
    this.normal_item_list = _.mapKeys(normal_list, (item) => item.key_name) as {
      [key_name: string]: ShopItem;
    };
    this.artifact_list = _.mapKeys(artifact_list, (item) => item.key_name) as {
      [key_name: string]: ShopItem;
    };
  }

  update_your_coins() {
    this.your_coins_win.update_text(
      String(this.data.info.party_data.coins),
      this.your_coins_text
    );
  }
  ...
  update_item_info(
    ...
    this.item_price_win.update_text(
      String(price_text),
      this.item_price_val_text
    );
  }
  ...
  alternate_window_pos(mode: string) {
    ...
    if (mode === BUY_MODE) {
      this.item_price_win.update_position({
        x: ITEM_PRICE_WIN_X,
        y: ITEM_PRICE_WIN_Y,
      });
      this.item_desc_win.update_position({
        x: ITEM_DESC_WIN_X,
        y: ITEM_DESC_WIN_Y,
      });
      this.your_coins_win.update_position({
        x: YOUR_COINS_WIN_X,
        y: YOUR_COINS_WIN_Y,
      });
      this.windows_mode = BUY_MODE;
  ...
  update_items() {
    let active_menu = this.buy_menu.active
      ? this.buy_menu
      : this.sell_menu.active
      ? this.sell_menu
      : null;

    this.set_item_lists();
    if (active_menu === this.buy_menu) {
      this.buy_menu.item_list = this.buy_menu.is_artifacts_menu
        ? this.artifact_list
        : this.normal_item_list;
  ...
  open_menu(
    ...
    this.shop_type =
      this.shops_db[this.shop_key].shop_type ?? shop_types.GENERAL_SHOP;
