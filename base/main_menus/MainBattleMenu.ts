...
export class MainBattleMenu {
  ...
  constructor(game: Phaser.Game, data: GoldenSun, battle_instance: Battle) {
    ...
    this.chars_status_window = new CharsStatusWindow(
      this.game,
      this.data,
      true,
      true
    ...
    this.avatar_sprite = this.group.create(
      0,
      numbers.GAME_HEIGHT - numbers.AVATAR_SIZE
    );
  ...
  start_button_press() {
    switch (
      this.start_buttons_keys[this.start_horizontal_menu.selected_button_index]
    ) {
      case "fight":
        this.start_horizontal_menu.close(() => {
          ...
          this.data.info.party_data.members
            ...
            .forEach((char: MainChar) => {
              this.abilities[char.key_name] = [];
            });
          this.djinni_already_used = ordered_elements.reduce(
            (a, b) => ((a[b] = 0), a),
            {}
          );
    ...
    const enemies_lvl =
      _.mean(
        this.battle_instance.enemies_info.flatMap((info) => {
          return info.instance.has_permanent_status(permanent_status.DOWNED)
            ? []
            : [info.instance.level];
        })
      ) | 0;
  ...
  inner_button_press() {
    switch (
      this.current_buttons[this.inner_horizontal_menu.selected_button_index]
    ) {
      case "attack":
      ...
      case "psynergy":
        ...
        break;
      case "djinni":
        this.on_ability_choose(
          this.djinn_window,
          true,
          "djinni",
          this.psynergy_window
        );
        break;
      case "summon":
        this.on_ability_choose(
      ...
      (ability: string, item_slot: ItemSlot) => {
        if (ability) {
          ...
          if (
            ...
            const requirements =
              this.data.info.summons_list[ability].requirements;
            summon_used_djinn = _.mapValues(
              this.djinni_already_used,
              (value, elem) => {
            ...
            (targets: Target[]) => {
              if (targets) {
                ...
                if (summon_used_djinn) {
                  this.djinni_already_used = _.mapValues(
                    this.djinni_already_used,
                    (value, elem) => {
  ...
  change_char(step: number, pop_ability: boolean = false) {
    ...
    if (
      ...
      if (pop_ability) {
        ...
        if (
          this.data.info.abilities_list[ability_info.key_name]
            .ability_category === ability_categories.SUMMON
        ) {
          const requirements =
            this.data.info.summons_list[ability_info.key_name].requirements;
          this.djinni_already_used = _.mapValues(
            this.djinni_already_used,
            (value, elem) => {
              return value - requirements[elem];
  ...
  set_avatar() {
    ...
    this.avatar_sprite.loadTexture(
      "avatars",
  ...
  inner_menu_cancel() {
    ...
    if (
      this.current_char_index > 0 ||
      this.abilities[char_key_name].length === 1
    ) {
  ...
  is_active() {
    return (
      this.start_horizontal_menu.menu_active ||
      this.inner_horizontal_menu.menu_active
    );
  }
