...
export class Gamepad {
  ...
  static initialize(data: GoldenSun, input_type: string = "default_inputs") {
    ...
    const gamepad_mapping = Object.entries(
      data.dbs.init_db.gamepad_rinputs as { [code: string]: string }
    ).map(
    ...
    const gamepad_custom_mapping = Object.entries(
      data.dbs.init_db.gamepad_inputs as { [code: string]: string }
    ).map(([game_button, button_code]): KeyMap => {
      const matches = button_code.match(
        /^(?:(?:(\w+) *\+ *)?(\w+) *\+ *)?(\w+)$/
      );
      if (!matches) console.error("Input not recognized " + button_code);
      const get_button_code = (code: string): number =>
        gamepad_mapping.find((km) => km.name === code)?.button_code ??
        Phaser.Gamepad[code];
      const get_game_button = (code: string): number =>
        gamepad_mapping.find(
          (km) => km.name === code || km.button_code === Phaser.Gamepad[code]
        )?.game_button;
      const km: KeyMap = {
        name: game_button,
        as: button_code,
        game_button: EngineButton[game_button],
      };
      if (matches[matches.length - 2]) {
        const buttons = matches.filter((m, i) => i && m);
        km.button_codes = buttons
          .map(get_button_code)
          .filter((bc) => bc !== undefined);
        km.game_buttons = buttons
          .map(get_game_button)
          .filter((bc) => bc !== undefined);
    ...
    const gamepad_stick_mapping = Object.entries(
      data.dbs.init_db.gamepad_rsticks as {
    ...
    Gamepad.keyboard_mapping = Object.entries(
      data.dbs.init_db[input_type] as { [code: string]: string }
    ).map(([game_button, key_code]): KeyMap => {
  ...
  static transcode_gamepad_button(button_code: number): AnyButton[] {
    ...
    return Gamepad.gamepad_mapping
      .filter((km) => km.button_code === button_code)
  ...
  static transcode_keyboard_key(key_code: number): AnyButton[] {
    ...
    return Gamepad.keyboard_mapping
      .filter((km) => km.key_code === key_code)
      .map((km) => km.game_button);
  }
  ...
  constructor(data: GoldenSun) {
    Gamepad.initialize(
      data,
      navigator.language === "fr-FR" ? "azerty_inputs" : "default_inputs"
    );
    this.register_handle_events(data.game);

    // Create a state for every buttons
    AdvanceButtons.forEach((button_name) => {
      this.buttons[Button[button_name]] = new GamepadButton(
        this,
        Button[button_name]
      );
    });
    ControllerButtons.forEach((button_name) => {
      this.buttons[ControllerButton[button_name]] = new GamepadButton(
        this,
        ControllerButton[button_name]
      );
    });
    EngineButtons.filter((bn) => bn).forEach((button_name) => {
      this.buttons[EngineButton[button_name]] = new GamepadButton(
        this,
        EngineButton[button_name]
      );
    });
  ...
  on_gamepad_down(game_button: AnyButton) {
    ...
    if (group_game_buttons.length)
      return group_game_buttons.forEach((km) => this._on_down(km.game_button));
  ...
  register_handle_events(game: Phaser.Game) {
    ...
    game.input.gamepad.onAxisCallback = (
      ...
      const game_buttons = Gamepad.gamepad_stick_mapping.filter(
        (km) => km.button_code === index
      );
      game_buttons.forEach((km) => {
        if (value < -this.stick_dead_zone)
          this.on_gamepad_down(km.game_buttons[0]);
        else if (value > this.stick_dead_zone)
          this.on_gamepad_down(km.game_buttons[1]);
        else {
    ...
    game.input.gamepad.onFloatCallback = (
      button_code: number,
      value: number
    ) => {
      this.is_last_input_gamepad = true;
      ...
      game_buttons.forEach((game_button) =>
        value > this.trigger_dead_zone
          ? this.on_gamepad_down(game_button)
          : this.on_gamepad_up(game_button)
      );
    };
      ...
      const game_buttons = Gamepad.keyboard_mapping
        .filter(
          (km) =>
            ...
            (km.key_modifiers?.alt != undefined
              ? km.key_modifiers.alt === event.altKey
              : true) &&
            (km.key_modifiers?.ctrl != undefined
              ? km.key_modifiers.ctrl === event.ctrlKey
              : true) &&
            (km.key_modifiers?.shift != undefined
              ? km.key_modifiers.shift === event.shiftKey
              : true)
        )
        .map((km) => km.game_button);
