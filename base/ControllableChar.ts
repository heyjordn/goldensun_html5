...
export abstract class ControllableChar {
  ...
  public casting_aura_stop_function: ReturnType<
    typeof FieldAbilities.init_cast_aura
  >;
  ...
  constructor(
    ...
    initial_animation =
      this.storage_keys.animation !== undefined
        ...
        : initial_animation;
    this._current_direction =
      initial_animation in directions ? directions[initial_animation] : null;
    ...
    this._outline_filter = this.game.add.filter(
      "Outline"
    ) as Phaser.Filter.Outline;
    ...
    this._footsteps = this.enable_footsteps
      ? new Footsteps(this.game, this.data, this)
      : null;
      ...
      (!walk_dash_only &&
        ((this._current_action === base_actions.CLIMB && !this.idle_climbing) ||
          (this._current_action === base_actions.ROPE &&
            this.current_animation !== base_actions.IDLE)))
    ) {
      return true;
    }
    return false;
  ...
  set_sprite(
    ...
    this.sprite.x =
      initial_x_px ?? get_centered_pos_in_px(this.tile_x_pos, map.tile_width);
    this.sprite.y =
      initial_y_px ?? get_centered_pos_in_px(this.tile_y_pos, map.tile_width);
  ...
  set_shadow(
    ...
    const key_name =
      options?.key_name ?? ControllableChar.DEFAULT_SHADOW_KEYNAME;
    const shadow_anchor_x =
      options?.shadow_anchor_x ?? ControllableChar.DEFAULT_SHADOW_ANCHOR_X;
    const shadow_anchor_y =
      options?.shadow_anchor_y ?? ControllableChar.DEFAULT_SHADOW_ANCHOR_Y;
  ...
  private look_to_target() {
    ...
    const direction =
      (1 + Math.floor((angle - numbers.degree45_half) / numbers.degree45)) & 7;
  ...
  is_close(target_char: ControllableChar, distance: number) {
    ...
    let lower_limit =
      reference_angle - ControllableChar.INTERACTION_RANGE_ANGLE;
    let upper_limit =
      reference_angle + ControllableChar.INTERACTION_RANGE_ANGLE;
    ...
    const target_angle = range_360(
      -Math.atan2(relative_point.y, relative_point.x) + angle_shift
    );
    const angle_condition =
      target_angle >= lower_limit && target_angle <= upper_limit;
    const distance_condition =
      Math.pow(relative_point.x, 2) + Math.pow(relative_point.y, 2) <=
      distance * distance;
  ...
  protected choose_direction_by_speed() {
    ...
    const angle = range_360(
      Math.atan2(this.current_speed.y, this.current_speed.x)
    );
    this._required_direction =
      (1 + Math.floor((angle - numbers.degree45_half) / numbers.degree45)) & 7;
  ...
  async face_direction(direction: number, time_between_frames: number = 40) {
    ...
    const transition_promise = new Promise(
      (resolve) => (transition_resolve = resolve)
    );
    const timer_function = (next_direction) => {
      next_direction = get_transition_directions(
  ...
  async jump(options: {
    ...
    if (options?.dest !== undefined) {
      ...
      const is_jump_over_x_axis =
        (options.dest.tile_x ??
          get_tile_position(dest_char.x, this.data.map.tile_width)) ===
        this.tile_x_pos;
      ...
      if (this.sprite_info.hasAction(base_actions.JUMP)) {
        ...
        const jump_init_promise = new Promise(
          (resolve) => (jump_init_resolve = resolve)
        );
  ...
  async shake(options?: {
    ...
    this.game.time.events.repeat(repeat_period, total, () => {
      this.sprite.scale[prop] =
        base_scale * scales_mult[counter % scales_mult.length];
      ++counter;
      if (counter === total) {
        promise_resolve();
      }
  ...
  async show_emoticon(
    ...
    const emoticon_sprite = this.game.add.sprite(
      x,
      y,
      "emoticons",
      emoticon_key
  ...
  set_frame(
    ...
    const this_animation =
      typeof animation === "string" ? animation : reverse_directions[animation];
    action = action ?? this.current_action;
  ...
  private rotate() {
    if (this._rotating) {
      if (
        this._rotating_interval === -1 ||
        this._rotating_elapsed > this._rotating_interval
      ) {
  ...
  private set_half_crop_mask(crop: boolean, force: boolean = false) {
    if (crop && (!this.crop_texture || force)) {
      ...
      this.sprite.mask.beginFill(0xffffff, 1);
      this.sprite.mask.drawRect(
        0,
        0,
        this.sprite.width,
        ((this.sprite.height * 3) | 0) >> 2
      );
      this.sprite.mask.endFill();
      this.shadow.visible = false;
  ...
  play_current_action(
    ...
    if (idle_climbing) {
      ...
      animation = this.stop_by_colliding
        ? base_actions.IDLE
    ...
    if (action === base_actions.WALK) {
      if (this.ice_sliding_active) {
        ...
        frame_rate = this.sprite_info.getFrameRate(
          base_actions.WALK,
          animation
        );
      }
  ...
  private tile_able_to_show_footprint() {
    ...
    for (let i = 0; i < tiles.length; ++i) {
      ...
      if (tile.properties.hasOwnProperty("disable_footprint")) {
        const layers = tile.properties.disable_footprint
          .split(",")
  ...
  protected calculate_speed() {
    ...
    const apply_speed = (speed_factor: number) => {
      this.temp_speed.x =
        (delta_time * this.current_speed.x * speed_factor) | 0;
      this.temp_speed.y =
        (delta_time * this.current_speed.y * speed_factor) | 0;
    };
    if (this.ice_sliding_active && this.sliding_on_ice) {
      ...
      const speed_factor =
        ControllableChar.WALK_OVER_ROPE_SPEED + this.extra_speed;
      apply_speed(speed_factor);
      ...
      this.temp_speed.x =
        (delta_time * this.current_speed.x * this.climb_speed) | 0;
      this.temp_speed.y =
        (delta_time * this.current_speed.y * this.climb_speed) | 0;
          ...
          if (
            ...
            if (
              ...
              if (
                ...
                (interactable_object as RollablePillar).check_and_start_rolling(
                  this
                )
              ) {
                break contacts_check;
