...
export type SnapshotData = {
  ...
  internal_map_objs_storage_keys: {
    [map_key: string]: Map["internal_map_objs_storage_keys"];
  };
  map_data: {
    key_name: string;
...
export class Snapshot {
  ...
  generate_snapshot() {
    const snapshot: SnapshotData = {
      storage_data: _.mapValues(
        this.data.storage.internal_storage,
        (record) => record.value
      ),
      engine_storage_data: _.mapValues(
        this.data.storage.engine_storage,
        (record) => record.value
      ),
      ...
      random_battle_extra_rate:
        this.data.info.party_data.random_battle_extra_rate,
      ...
      summons_availability: _.mapValues(
        this.data.info.summons_list,
        (summon) => summon.available
      ),
      ...
      map_data: {
        ...
        npcs: this.data.map.npcs.map((npc, index) => {
          return {
            ...
            anim_is_playing:
              npc.sprite?.animations.currentAnim.isPlaying ?? null,
        ...
        interactable_objects: this.data.map.interactable_objects.map(
          (io, index) => {
            return {
              ...
              anim_is_playing:
                io.sprite?.animations.currentAnim.isPlaying ?? null,
