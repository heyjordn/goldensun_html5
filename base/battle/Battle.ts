...
import {
  ...
  PlayerAbilities,
  PlayerAbility,
} from "../main_menus/MainBattleMenu";
...
export class Battle {
  ...
  public before_fade_finish_callback: (
    victory: boolean,
    party_fled: boolean
  ) => Promise<void>;
  ...
  constructor(
    ...
    this.allies_info = this.data.info.party_data.members
      .slice(0, Battle.MAX_CHARS_IN_BATTLE)
      .map((char) => {
    ...
    const enemies_party_data =
      this.data.dbs.enemies_parties_db[enemy_party_key];
    ...
    enemies_party_data.members.forEach((member_info: EnemyPartyMember) => {
      ...
      for (let i = 0; i < qtd; ++i) {
        ...
        this.enemies_info[counter].battle_key =
          this.enemies_info[counter].sprite_key + battle_key_suffix;
        this.this_enemies_list[this.enemies_info[counter].battle_key] = this
          .enemies_info[counter].instance as Enemy;
  ...
  choose_targets(
    ...
    if (action !== "defend") {
      this.target_window.open(
        action,
        this_ability.name,
        this_ability.element,
        ability_key,
        quantities,
        item_obj
      );
  ...
  async battle_fadein() {
    ...
    const tween = this.game.add
      .tween(circle.scale)
      .to({ x: 1, y: 1 }, 400, Phaser.Easing.Linear.None, true);
  ...
  async flee(flee_succeed: boolean) {
    ...
    await this.battle_log.add(
      `${this.data.info.party_data.members[0].name} and friends run!`
    );
  ...
  async battle_phase_round_start() {
      ...
      for (let i = 0; i < this.allies_abilities[char_key].length; ++i) {
        const this_ability =
          this.data.info.abilities_list[
            this.allies_abilities[char_key][i].key_name
          ];
        const priority_move =
          this_ability !== undefined ? this_ability.priority_move : false;

        this.allies_abilities[char_key][i].speed =
          BattleFormulas.player_turn_speed(this_char.agi, priority_move, i > 0);
    ...
    for (let battle_key in this.enemies_abilities) {
      ...
      for (let i = 0; i < this.enemies_abilities[battle_key].length; ++i) {
        const this_ability =
          this.data.info.abilities_list[
            this.enemies_abilities[battle_key][i].key_name
          ];
        const priority_move =
          this_ability !== undefined ? this_ability.priority_move : false;

        this.enemies_abilities[battle_key][i].speed =
          BattleFormulas.enemy_turn_speed(
            this_enemy.agi,
            i + 1,
            this_enemy.turns,
            priority_move
    ...
    this.turns_actions = _.sortBy(
      Object.values(this.allies_abilities)
        .flat()
        .concat(Object.values(this.enemies_abilities).flat()),
      (action) => {
  ...
  async set_action_animation_settings(action: PlayerAbility, ability: Ability) {
    ...
    let battle_animation_key =
      this.data.info.abilities_list[action.key_name].battle_animation_key;
    if (
      ability.has_animation_variation &&
      action.key_name in action.caster.battle_animations_variations
    ) {
      battle_animation_key =
        action.caster.battle_animations_variations[action.key_name];
    }
    action.battle_animation_key = battle_animation_key;

    const mirrored_animation =
      ability.can_be_mirrored &&
  ...
  async check_downed(target: Enemy | MainChar) {
    if (target.current_hp === 0) {
      ...
      await this.battle_log.add(
        on_catch_status_msg[permanent_status.DOWNED](target)
      );
      await this.wait_for_key();
    }
  }
  ...
  async battle_phase_combat() {
    ...
    if (action.caster.is_paralyzed()) {
      if (action.caster.temporary_status.has(temporary_status.SLEEP)) {
        ...
        await this.battle_log.add(
          `${action.caster.name} is paralyzed and cannot move!`
        );
    ...
    if (
      action.caster.has_permanent_status(permanent_status.EQUIP_CURSE) &&
      Math.random() < 0.25
    ) {
      await this.battle_log.add(
        `${action.caster.name} is bound and cannot move!`
      );
      await this.wait_for_key();
      this.check_phases();
      return;
    ...
    for (let i = 0; i < action.targets.length; ++i) {
      ...
      if (
        target.magnitude &&
        target.target?.instance.has_permanent_status(permanent_status.DOWNED)
      ) {
    ...
    let item_name = action.item_slot
      ? this.data.info.items_list[action.item_slot.key_name].name
      : "";
    ...
    if (
      ...
      if (
        ...
        this.data.info.items_list[caster.equip_slots.weapon.key_name]
          .unleash_ability
      ) {
        const weapon =
          this.data.info.items_list[caster.equip_slots.weapon.key_name];
    ...
    if (ability === undefined) {
      await this.battle_log.add(
        `${action.key_name} ability key not registered.`
      );
      await this.wait_for_key();
      this.check_phases();
      return;
    ...
    const djinn_name = action.djinn_key_name
      ? this.data.info.djinni_list[action.djinn_key_name].name
      : undefined;
    ...
    if (ability.ability_category === ability_categories.DJINN) {
      if (
        ability.effects.some((effect) => effect.type === effect_types.SET_DJINN)
      ) {
        this.data.info.djinni_list[action.djinn_key_name].set_status(
          djinn_status.SET
        );
      } else {
        this.data.info.djinni_list[action.key_name].set_status(
          djinn_status.STANDBY
        );
      }
    } else if (ability.ability_category === ability_categories.SUMMON) {
      //deal with summon ability type
      const requirements =
        this.data.info.summons_list[ability.key_name].requirements;
      ...
      const has_available_djinni = _.every(
        requirements,
        (requirement, element) => {
      ...
      if (!has_available_djinni) {
        await this.battle_log.add(
          `${action.caster.name} summons ${ability.name} but`
        );
    ...
    if (
      ![ability_types.UTILITY, ability_types.EFFECT_ONLY].includes(ability.type)
    ) {
    ...
    await Promise.all([
      this.battle_stage.reset_chars_position(),
    ...
    if (poison_status) {
      let damage = BattleFormulas.battle_poison_damage(
        action.caster,
        poison_status
      );
      ...
      action.caster.current_hp = _.clamp(
        action.caster.current_hp - damage,
        0,
        action.caster.max_hp
      );
      const poison_name =
        poison_status === permanent_status.POISON ? "poison" : "venom";

      await this.battle_log.add(
        `The ${poison_name} does ${damage.toString()} damage to ${
          action.caster.name
        }!`
      );
      this.battle_menu.chars_status_window.update_chars_info();
    ...
    if (action.caster.has_temporary_status(temporary_status.DEATH_CURSE)) {
      ...
      if (action.caster.get_effect_turns_count(this_effect) === 1) {
        ...
        await this.battle_log.add(
          `The Grim Reaper calls out to ${action.caster.name}`
        );
        await this.wait_for_key();
      }
    }
  ...
  async play_battle_animation(action: PlayerAbility, ability: Ability) {
    ...
    if (anim_availability === animation_availability.AVAILABLE) {
      ...
      const caster_sprite =
        caster_targets_sprites.caster[action.caster_battle_key];
      const target_sprites = action.targets.flatMap((info) => {
        return info.magnitude
          ? [caster_targets_sprites.targets[info.target.battle_key]]
          : [];
      });
      const group_caster =
        action.caster.fighter_type === fighter_types.ALLY
      ...
      if (action.cast_animation_type && action.caster_battle_key) {
        const animation_recipe =
          this.data.info.abilities_cast_recipes[action.cast_animation_type];
      ...
      await this.battle_log.add(
        `Animation for ${ability.name} not available...`
      );
      await this.wait_for_key();
    }
  }
    ...
    for (let i = 0; i < action.targets.length; ++i) {
      ...
      if (ability.can_be_evaded) {
        ...
        if (
          ...
          await this.battle_log.add(
            `${target_instance.name} nimbly dodges the blow!`
          );
      ...
      if (ability.type === ability_types.SUMMON) {
        djinn_used = _.sum(
          _.values(this.data.info.summons_list[ability.key_name].requirements)
        );
      }
      ...
      const current_property = ability.affects_pp
        ? main_stats.CURRENT_PP
        : main_stats.CURRENT_HP;
      const max_property = ability.affects_pp
        ? main_stats.MAX_PP
        : main_stats.MAX_HP;
      ...
      for (let j = 0; j < ability.effects.length; ++j) {
        ...
        if (effect_obj.type === effect_types.DAMAGE_INPUT) {
          ...
          const damage_input_effect = player.add_effect(
            effect_obj,
            ability
          ).effect;
          ...
          if (
            [effect_types.CURRENT_HP, effect_types.CURRENT_PP].includes(
              damage_input_effect.sub_effect.type
            )
          ) {
            ...
            if (effect_damage !== 0) {
              if (damage_input_effect.effect_msg) {
                await this.battle_log.add(
                  effect_msg[damage_input_effect.effect_msg](target_instance)
                );
              } else {
                await this.battle_log.add_damage(
                  effect_damage,
...
*/
      ...
      switch (effect.type) {
        case effect_types.PERMANENT_STATUS:
          if (effect.add_status) {
            if (
              target_instance.has_permanent_status(
                effect.status_key_name as permanent_status
              )
            )
              break;
            if (
            ...
            if (added_effect) {
              if (
                !target_instance.has_temporary_status(
                  added_effect.status_key_name as temporary_status
                )
              ) {
              ...
              await this.battle_log.add(
                on_catch_status_msg[effect.status_key_name](target_instance)
              );
            } else {
              await this.battle_log.add(
                `But it has no effect on ${target_instance.name}!`
              );
            }
            await this.wait_for_key();
          } else {
            const removed_effects = Effect.remove_status_from_player(
              effect,
              target_instance
            );
        ...
        case effect_types.CURRENT_HP:
          ...
          if (effect_result.effect.show_msg) {
            const damage =
              effect_result.changes.before - effect_result.changes.after;
        ...
        case effect_types.RESIST:
          ...
          if (effect.remove_buff) {
            this.on_going_effects = this.on_going_effects.filter(
              (effect) =>
                !effect_result.changes.removed_effects.includes(effect)
            );
            if (effect_result.effect.show_msg) {
              await this.battle_log.add(
                `${target_instance.name}'s strength return to normal!`
              );
              await this.wait_for_key();
            }
            break;
            ...
            if (effect_result.effect.show_msg) {
              const diff =
                effect_result.changes.after - effect_result.changes.before;
              const text = diff >= 0 ? "rises" : "drops";
              let element_info = "";

              if (
                [effect_types.POWER, effect_types.RESIST].includes(effect.type)
              ) {
                element_info =
                  element_names[effect_result.effect.element] + " ";
              }
        ...
        case effect_types.PARALYZE:
          ...
          if (target_instance.paralyzed_by_effect) {
            await this.battle_log.add(
              `${target_instance.name} has become unable to move!`
            );
          } else {
            await this.battle_log.add(
              `But it has no effect on ${target_instance.name}!`
        ...
        case effect_types.TURNS:
          await this.battle_log.add(
            `${action.caster.name} readies for action!`
          );
          await this.wait_for_key();

          this.on_going_effects.push(
            target_instance.add_effect(effect, ability, true).effect
          );
          break;

        case effect_types.DAMAGE_MODIFIER:
          await this.battle_log.add(
            effect_msg[effect.effect_msg](target_instance)
          );
          await this.wait_for_key();
      ...
      if (effect.turn_count !== -1) {
        ...
        if (effect.char.get_effect_turns_count(effect) !== null) {
          if (
            ...
            !(
              effect.char.get_effect_turns_key(effect) in
          ...
          if (!effect_groups[effect.char.key_name]) {
            ...
            effect_groups[effect.char.key_name][
              effect.char.get_effect_turns_key(effect)
            ] = effect;
          }
          avoid_msg = true;
        } else {
      ...
      for (let j = 0; j < player_djinni.length; ++j) {
        ...
        if (djinn.status === djinn_status.RECOVERY) {
          if (djinn.recovery_turn === 0) {
            ...
            await this.battle_log.add(
              `${djinn.name} is set to ${player.name}!`
            );
            await this.wait_for_key();
          } else {
            --djinn.recovery_turn;
  ...
  async apply_battle_recovery(party: PlayerInfo[], is_hp_recovery: boolean) {
    ...
    for (let i = 0; i < party.length; i++) {
      ...
      const recovery_stat = is_hp_recovery
        ? player.instance.hp_recovery
        : player.instance.pp_recovery;
      let current_stat = is_hp_recovery
        ? player.instance.current_hp
        : player.instance.current_pp;
      const max_stat = is_hp_recovery
      ...
      if (recovery_stat !== 0) {
        ...
        this.battle_log.add(
          `${player.instance.name} ${recovery} ${Math.abs(
            recovery_stat
          )} ${print_stat}!`
        );
        await this.wait_for_key();
  ...
  async battle_phase_end() {
    ...
    if (this.allies_defeated) {
      this.battle_log.add(
        this.allies_info[0].instance.name + "' party has been defeated!"
      );
    } else {
      ...
      for (let i = 0; i < this.allies_info.length; ++i) {
        ...
        if (!char.has_permanent_status(permanent_status.DOWNED)) {
          ...
          const change = char.add_exp(
            info.entered_in_battle ? total_exp : total_exp >> 1
          );

          //check whether this char evolved
          if (change.before.level !== change.after.level) {
            this.battle_log.add(
              `${char.name} is now a level ${char.level} ${char.class.name}!`
            );
            await this.wait_for_key();

            const gained_abilities = _.difference(
              change.after.abilities,
              change.before.abilities
            );
            for (let j = 0; j < gained_abilities.length; ++j) {
              const ability =
                this.data.info.abilities_list[gained_abilities[j]];
              this.battle_log.add(
                `Mastered the ${char.class.name}'s ${ability.name}!`
              );
              await this.wait_for_key();
            }
            ...
            for (let j = 0; j < change.before.stats.length; ++j) {
              ...
              const diff =
                change.after.stats[j][stat] - change.before.stats[j][stat];
              if (diff !== 0) {
                let stat_text;
                switch (stat) {
                ...
                this.battle_log.add(
                  `${stat_text} rises by ${diff.toString()}!`
                );
                await this.wait_for_key();
              }
            }
      ...
      for (let i = 0; i < this.enemies_info.length; ++i) {
        ...
        if (enemy.item_reward && Math.random() < enemy.item_reward_chance) {
          ...
          if (item !== undefined) {
            if (
              MainChar.add_item_to_party(this.data.info.party_data, item, 1)
            ) {
    ...
    this.battle_stage.unset_stage(
      () => {
        ...
        if (this.before_fade_finish_callback) {
          return this.before_fade_finish_callback(
            !this.allies_defeated,
            this.party_fled
          );
        }
        return null;
