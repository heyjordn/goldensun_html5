...
export class EnemyAI {
  static roll_action(
    ...
    const available_abilities = caster.abilities.filter(
      (info) => info.key_name in data.info.abilities_list
    );
    const ability =
      data.info.abilities_list[_.sample(available_abilities).key_name];
    return {
      key_name: ability.key_name,
      battle_animation_key: ability.battle_animation_key,
      targets: enemies.map((enemy, index) => {
        let available_target = enemy.has_permanent_status(
