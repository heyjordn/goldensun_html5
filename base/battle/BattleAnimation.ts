...
export class BattleAnimation {
  ...
  initialize(
    ...
    this.trails_bmps = [];
    if (
      super_group.getChildIndex(group_caster) <
      super_group.getChildIndex(group_enemy)
    ) {
    ...
    super_group.addChildAt(
      this.ability_sprites_groups.between,
      super_group.getChildIndex(this.front_group)
    );
    super_group.addChildAt(
      this.ability_sprites_groups.behind,
      super_group.getChildIndex(this.back_group)
    );
    if (this.mirrored) {
    ...
    for (let i = 0; i < this.sprites_keys.length; ++i) {
      ...
      if (sprite_info.trails) {
        const trail_bitmap_data = this.game.make.bitmapData(
          numbers.GAME_WIDTH,
          numbers.GAME_HEIGHT
        );
        ...
        for (let j = 0; j < count; ++j) {
          ...
          switch (sprite_type) {
            case sprite_types.SPRITE:
              psy_sprite = this.game.add.sprite(
                this.init_pos.x,
                this.init_pos.y,
                sprite_key
              );
            ...
            case sprite_types.RING:
              psy_sprite = this.game.add.graphics(
                this.init_pos.x,
                this.init_pos.y
              );
              ...
              psy_sprite.arc(
                0,
                0,
                sprite_info.geometry_info.radius,
                0,
                numbers.degree360,
                false
              );
            ...
            case sprite_types.CIRCLE:
              ...
              color = sprite_info.geometry_info.keep_core_white
                ...
                : get_color(sprite_info.geometry_info.color);
              psy_sprite.beginFill(color, 1);
              psy_sprite.drawCircle(
                0,
                0,
                sprite_info.geometry_info.radius << 1
              );
              psy_sprite.endFill();
              break;
          }
          if (psy_sprite instanceof Phaser.Graphics) {
            ...
            psy_sprite = this.game.add.sprite(
              psy_sprite.x,
              psy_sprite.y,
              psy_sprite.generateTexture()
            );
            graphic.destroy();
            psy_sprite.data.color = get_color(sprite_info.geometry_info.color);
            psy_sprite.data.keep_core_white =
              sprite_info.geometry_info.keep_core_white;
          }
          this.ability_sprites_groups[sprite_info.position].addChild(
            psy_sprite
          );
  ...
  manage_filter(
    ...
    sprite: PlayerSprite | PIXI.DisplayObject,
    remove: boolean
  ) {
  ...
  play(finish_callback) {
    ...
    this.play_number_property_sequence(
    ...
    this.play_number_property_sequence(
      this.hue_angle_sequence,
      "available_filters.hue.angle",
      {
        rotational_property: true,
        filter_key: "hue",
      }
    ...
    this.play_number_property_sequence(
      this.grayscale_sequence,
      "available_filters.gray.intensity",
      {
        filter_key: "gray",
      }
    );
  ...
  play_number_property_sequence(
    ...
    const obj_propety =
      dot_index >= 0 ? target_property.slice(0, dot_index) : null;
    const property_to_set =
      dot_index >= 0
        ? target_property.slice(dot_index + 1, target_property.length)
        : target_property;
    for (let i = 0; i < sequence.length; ++i) {
      ...
      _.forEach(sprites, (sprite_info, key) => {
        ...
        if (
          property_uniq_key in chained_tweens &&
          chained_tweens[property_uniq_key].length
        ) {
        ...
        const get_to_value = () => {
          ...
          if (
            ...
            this.sprites_prev_properties[uniq_key][property_to_set] =
              this_sprite[property_to_set];
          }
          const seq_to = Array.isArray(seq.to)
            ? seq.to[sprite_info.index]
            : seq.to;
          ...
          if (["targets", "caster"].includes(seq_to as string)) {
            ...
            if (seq_to === "targets") {
              player_sprite =
                this.targets_sprites[this.targets_sprites.length >> 1];
            }
            let shift_sign = 1;
            if (seq.shift_direction !== undefined) {
              ...
              const shift_direction: DefaultAttr["shift_direction"] =
                Array.isArray(seq.shift_direction)
            ...
            const shift =
              ((Array.isArray(seq.shift)
                ? seq.shift[sprite_info.index]
                : seq.shift) ?? 0) * shift_sign;
        ...
        if (seq.tween === "initial") {
          ...
          if (seq.duration === "instantly") {
            ...
            this.game.time.events.add(start_delay, () => {
              this_sprite[property_to_set] = get_to_value();
              if (
                ["ellipses_semi_major", "ellipses_semi_minor"].includes(
                  property_to_set
                )
              ) {
              ...
              if (seq.is_absolute && options?.rotational_property) {
                this_sprite[property_to_set] = range_360(
                  this_sprite[property_to_set]
                );
              }
            ...
            const tween = this.game.add.tween(this_sprite).to(
              ...
              Array.isArray(seq.duration)
                ? seq.duration[sprite_info.index]
                : seq.duration,
            ...
            if (!promises_set) {
              let resolve_function;
              const this_promise = new Promise(
                (resolve) => (resolve_function = resolve)
              );
              this.promises.push(this_promise);
              tween.onComplete.addOnce(() => {
                if (seq.is_absolute && options?.rotational_property) {
            ...
            if (chained_tweens[property_uniq_key].length) {
              chained_tweens[property_uniq_key][
                chained_tweens[property_uniq_key].length - 1
              ].chain(tween);
            }
  ...
  play_sprite_sequence() {
    const index_promises_resolve = new Array<() => void>(
      this.play_sequence.length
    );
    const index_promises = new Array<Promise<void>>(this.play_sequence.length)
      .fill(null)
      .map((foo, i) => {
        return new Promise<void>(
          (resolve) => (index_promises_resolve[i] = resolve)
        );
      });
      ...
      _.forEach(sprites, (sprite_info) => {
        ...
        const start = async () => {
          ...
          if (anim) {
            ...
            anim.stop(true);
            if (
              sprite instanceof PlayerSprite &&
              (sprite as PlayerSprite).is_ally
            ) {
              sprite.set_action(
                play_seq.animation_key as battle_actions,
                false
  ...
  play_levels_filter(sequence: BattleAnimation["levels_filter_sequence"]) {
    this.play_general_filter(
      ...
      (
        filter_seq: BattleAnimation["levels_filter_sequence"][0],
        filter: Phaser.Filter.Levels
      ) => {
  ...
  play_color_blend_filter(
    sequence: BattleAnimation["color_blend_filter_sequence"]
  ) {
    this.play_general_filter(
      sequence,
      EngineFilters.COLOR_BLEND,
      (
        filter_seq: BattleAnimation["color_blend_filter_sequence"][0],
        filter: Phaser.Filter.ColorBlend
      ) => {
  ...
  play_tint_filter(sequence: BattleAnimation["tint_sequence"]) {
    this.play_general_filter(
      ...
      EngineFilters.TINT,
      (
        filter_seq: BattleAnimation["tint_sequence"][0],
        filter: Phaser.Filter.Tint
      ) => {
        filter.r = filter_seq.r ?? filter.r;
        filter.g = filter_seq.g ?? filter.g;
        filter.b = filter_seq.b ?? filter.b;
  ...
  play_colorize_filter(sequence: BattleAnimation["colorize_sequence"]) {
    this.play_general_filter(
      ...
      (
        filter_seq: BattleAnimation["colorize_sequence"][0],
        filter: Phaser.Filter.Colorize
      ) => {
  ...
  play_stage_angle_sequence() {
    ...
    for (let i = 0; i < this.stage_angle_sequence.length; ++i) {
      ...
      if (stage_angle_seq.to === "default") {
        ...
        to_value = this.mirrored
          ? MIRRORED_CAST_STAGE_POSITION
          : CAST_STAGE_POSITION;
        direction = "closest";
        is_absolute = true;
      }
      ...
      if (stage_angle_seq.tween === "initial") {
        ...
        const tween = this.game.add.tween(this.battle_stage.camera_angle).to(
          ...
          stage_angle_seq.tween
            .split(".")
            .reduce((p, prop) => p[prop], Phaser.Easing),
        ...
        tween.onComplete.addOnce(() => {
          if (is_absolute) {
            this.battle_stage.camera_angle.rad = range_360(
              this.battle_stage.camera_angle.rad
            );
          }
  ...
  static get_angle_by_direction(
    incoming_current_angle,
    incoming_target_angle,
    direction,
    fourth_quadrant = false
  ) {
    let this_direction;
    const target_angle = fourth_quadrant
      ? numbers.degree360 - incoming_target_angle
      : incoming_target_angle;
    const current_angle = fourth_quadrant
      ? numbers.degree360 - incoming_current_angle
      : incoming_current_angle;
    this_direction =
      target_angle > current_angle ? "counter_clockwise" : "clockwise";
    if (this_direction === direction) {
      return target_angle;
    } else if (direction !== "closest") {
      const times_bigger =
        Math.abs(incoming_target_angle / numbers.degree360) | 0;
      return (
        incoming_target_angle +
        times_bigger *
          (incoming_target_angle > incoming_current_angle
            ? -numbers.degree360
            : numbers.degree360)
      );
    } else {
      const range360_target_angle = range_360(incoming_target_angle);
      const towards_target = range_360(
        range360_target_angle - incoming_current_angle
      );
      const towards_current = range_360(
        incoming_current_angle - range360_target_angle
      );
      const diff = _.round(towards_target - towards_current, 4);
      if (
        +(range360_target_angle > incoming_current_angle) ^ +(diff > 0) ||
        diff === 0
      ) {
        return range360_target_angle;
      } else {
        return (
          range360_target_angle +
          (diff > 0 ? -numbers.degree360 : numbers.degree360)
        );
      }
    }
  }
