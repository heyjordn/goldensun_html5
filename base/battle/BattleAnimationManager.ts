...
export class BattleAnimationManager {
  ...
  async load_animation(battle_anim_key, caster_battle_key, mirrored_animation) {
    ...
    this.game.load.json(
      recipe_key,
      `${ANIMATIONS_DB_PATH}${battle_anim_key}_db.json`
    );

    let load_complete_promise_resolve;
    const load_complete_promise = new Promise(
      (resolve) => (load_complete_promise_resolve = resolve)
    );
  ...
  animation_available(battle_anim_key, caster_battle_key) {
    ...
    else if (battle_anim_key === "no_animation")
      return animation_availability.NO_ANIMATION;
  ...
  async play_animation(
    ...
    await play_promise;
    this.render_functions = this.render_functions.filter(
      (f) => f !== render_function
    );
  }
