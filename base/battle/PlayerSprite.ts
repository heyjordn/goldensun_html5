...
export class PlayerSprite {
  ...
  private on_status_change_obs: Observable<
    Player["on_status_change"] extends Subject<infer T> ? T : never
  >;
  ...
  constructor(
    ...
    this.status_sprite_base =
      this.data.info.misc_sprite_base_list[STATUS_SPRITES_KEY_NAME];
  ...
  initialize_player() {
    ...
    this.shadow_sprite = this.group.create(
      0,
      0,
      "battle_shadows",
      this.player_instance.battle_shadow_key
    );
    this.shadow_sprite.anchor.setTo(0.5, 1);
    this.shadow_sprite.scale.setTo(
      this.player_instance.battle_scale,
      this.player_instance.battle_scale
    );

    this.char_sprite = this.group.create(0, 0, this.player_info.sprite_key);
    this.char_sprite.anchor.setTo(0.5, 1);
    this.char_sprite.scale.setTo(
      this.player_instance.battle_scale,
      this.player_instance.battle_scale
    );
    ...
    this.char_sprite.available_filters[color_blend_filter.key] =
      color_blend_filter;
    ...
    if (this.is_ally) {
      ...
      if (weapon_slot !== null) {
        this.current_weapon_type =
          this.data.info.items_list[weapon_slot.key_name].weapon_type;
        if (player.weapons_sprite_base.hasAction(this.current_weapon_type)) {
          const weapon_sprite_key = player.weapons_sprite_base.getSpriteKey(
            this.current_weapon_type
          );
          ...
          player.weapons_sprite_base.setAnimation(
            this.weapon_sprite,
            this.current_weapon_type
          );
        }
    ...
    const status_key = this.status_sprite_base.getSpriteKey(
      STATUS_SPRITES_KEY_NAME
    );
    this.status_sprite = this.group.create(0, 0, status_key);
    this.status_sprite.anchor.setTo(0.5, 0.5);
    this.status_sprite_base.setAnimation(
      this.status_sprite,
      STATUS_SPRITES_KEY_NAME
    );
    ...
    this.on_status_change_obs =
      this.player_instance.on_status_change.asObservable();
    this.on_status_change_subs = this.on_status_change_obs.subscribe(
      this.set_next_status_sprite.bind(this)
    );
  ...
  get_animation_key(action: battle_actions, position: battle_positions) {
    return this.sprite_base.getAnimationKey(
      base_actions.BATTLE,
      `${action}_${position}`
    );
  }
  ...
  async unmount_by_dissolving() {
    ...
    for (let i = 0; i < sprites.length; ++i) {
      ...
      const x_pos =
        ((bmd_width - sprite.texture.crop.width * sprite.scale.x) *
          sprite.anchor.x) |
        0;
      const y_pos =
        ((bmd_height - sprite.texture.crop.height * sprite.scale.y) *
          sprite.anchor.y) |
        0;
    ...
    for (let i = sections_number - 1; i >= 0; --i) {
      ...
      const end =
        i === sections_number - 1 ? bmd.pixels.length : (i + 1) * section_size;
    ...
    for (let i = 0; i < status_sprites.length; ++i) {
      ...
      if (
        ...
        this.status_sprite.y =
          ...
          this.player_instance.status_sprite_shift *
            this.player_instance.battle_scale;
        ...
        if ((status as temporary_status) === temporary_status.DEATH_CURSE) {
          ...
          const remaining_turns = effect
            ? this.player_instance.get_effect_turns_count(effect)
            : 1;
          status_key = `${status}_${remaining_turns - 1}`;
        }
        const animation_key = this.status_sprite_base.getAnimationKey(
          STATUS_SPRITES_KEY_NAME,
          status_key
        );
