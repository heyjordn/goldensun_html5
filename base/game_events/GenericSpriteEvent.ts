  ...
  _fire() {
    switch (this.control_type) {
      ...
      case control_types.UPDATE:
        const generic_sprite =
          this.data.map.generic_sprites[this.generic_sprite_key_name];
        if (!generic_sprite) {
          console.warn(
            `Generic sprite "${this.generic_sprite_key_name}" doesn't exist.`
          );
          break;
        }
        ...
        generic_sprite.base_collision_layer =
          this.collision_layer ?? generic_sprite.base_collision_layer;
        if (this.play) {
