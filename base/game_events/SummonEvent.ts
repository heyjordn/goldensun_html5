...
export class SummonEvent extends GameEvent {
  ...
  config_emitter() {
    ...
    const sprite_key = this.origin_npc.sprite_info.getSpriteKey(
      SummonEvent.ACTION
    );
    ...
    const anim_key = this.origin_npc.sprite_info.getAnimationKey(
      SummonEvent.ACTION,
      "particles"
    );
  ...
  config_letters() {
    ...
    for (let i = 0; i < SummonEvent.PARTICLES_NUMBER; ++i) {
      ...
      const frame = this.origin_npc.sprite_info.getFrameName(
        SummonEvent.ACTION,
        "letters",
        frame_index
      );
  ...
  async _fire() {
    ...
    for (let i = 0; i < 3; ++i) {
      ...
      for (let j = 0; j < 16; ++j) {
        const dest_x =
          x0 + SummonEvent.LETTER_INIT_X + SummonEvent.LETTER_PADDING_X * j;
        const dest_y =
          y0 + SummonEvent.LETTER_INIT_Y + SummonEvent.LETTER_PADDING_Y * i;
        const letter = this.letters[16 * i + j];
        letter.visible = true;
        const angle =
          range_360(Math.atan2(dest_y - letter.y, dest_x - letter.x)) % Math.PI;
    ...
    const delta_time_to_be_in_track =
      time_to_all_be_in_track / SummonEvent.LETTERS_NUMBER;
    const join_timers = new Array(SummonEvent.LETTERS_NUMBER).fill(
      time_to_join_track
    );
    ...
    const letter_spiral_update_callback = () => {
      for (let i = 0; i < SummonEvent.LETTERS_NUMBER; ++i) {
        ...
        if (phi < PI) {
          ...
          new_y =
            2.4 * Math.exp(-0.4 * _phi) * Math.sin(_phi) -
            0.1 -
            Math.pow(6, -1.52 * (-0.95 + _phi));
        ...
        new_y = 1 - new_y;
        new_x =
          numbers.GAME_WIDTH * new_x +
          this.data.hero.sprite.centerX -
          half_width -
          4;
        new_y =
          numbers.GAME_HEIGHT * new_y +
          this.data.hero.sprite.centerY -
          half_height -
          10;
    ...
    reset_map();
    const summon_name =
      this.data.info.abilities_list[this.summon.key_name].name;
    aux_promise = new Promise((resolve) => (aux_resolve = resolve));
    this.dialog.next_dialog(
    ...
    this.dialog.next_dialog(
      `To summon ${summon_name}\${BREAK_LINE}${requirements.join(
        " "
      )}\${BREAK_LINE}Standby Djinn are needed.`,
      () => {
        this.control_enable = true;
      },
      { show_crystal: false }
