...
const ITEM_TEXT = (hero_name: string, item_name: string) =>
  `${hero_name} got ${item_name}.`;
...
export class ChestEvent extends GameEvent {
  ...
  async _fire() {
    ...
    this.dialog_manager.set_dialog(
      this.custom_init_text ?? INIT_TEXT(hero_name)
    );
    this.promise = new Promise((resolve) => (this.resolve = resolve));
    this.next();
    await this.promise;

    const item_sprite = this.game.add.sprite(
      0,
      0,
      "items_icons",
      this.item.key_name
    ...
    const emitter = this.game.add.emitter(
      item_sprite.x,
      item_sprite_pos_above_chest,
      15
    );
    const psynergy_particle_base =
    ...
    if (!this.no_chest) {
      const animation_key = this.origin_npc.sprite_info.getAnimationKey(
        "chest",
        "opening"
      );
      const animation =
        this.origin_npc.sprite.animations.getAnimation(animation_key);
      animation.play();
    ...
    this.game.add
      ...
      .onComplete.addOnce(() => {
        ...
        const anim_key = psynergy_particle_base.getAnimationKey(
          "psynergy_particle",
          "vanish"
        );
        emitter.forEach((particle: Phaser.Sprite) => {
    ...
    const delta_y =
      item_sprite.y - (this.data.hero.sprite.y - this.data.hero.sprite.height);
