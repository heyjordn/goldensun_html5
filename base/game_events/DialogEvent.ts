...
export class DialogEvent extends GameEvent {
  ...
  async finish() {
    if (
      this.origin_npc &&
      this.npc_hero_reciprocal_look &&
      this.reset_reciprocal_look
    ) {
    ...
    this.dialog_manager.next(
      async (finished) => {
        if (finished) {
          ...
          const y_pos =
            this.dialog_manager.window_y > GAME_HEIGHT >> 1 ? 5 : null;
          this.yes_no_menu.open(
            {
              yes: () =>
                this.next(() =>
                  this.yes_no_events.yes.forEach((event) =>
                    event.fire(this.origin_npc)
                  )
                ),
              no: () =>
                this.next(() =>
                  this.yes_no_events.no.forEach((event) =>
                    event.fire(this.origin_npc)
                  )
                ),
            },
  ...
  start_dialog() {
    ...
    this.dialog_manager.set_dialog(this.current_info.text, {
      avatar:
        this.current_info.avatar ??
        (reference_npc ? reference_npc.avatar : this.origin_npc.avatar),
      voice_key:
        this.current_info.voice_key ??
        (reference_npc ? reference_npc.voice_key : this.origin_npc.voice_key),
      hero_direction: this.current_info.consider_hero_direction
