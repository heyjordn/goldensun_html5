...
export class MoveEvent extends GameEvent {
  ...
  constructor(
    ...
    this.final_direction =
      final_direction !== undefined
        ? directions[final_direction as string]
        : null;
    this.is_npc = is_npc;
    this.npc_label = npc_label;
    this.previous_dash_value = null;
    this.camera_follow = camera_follow ?? false;
    this.camera_follow_duration =
      camera_follow_duration ?? MoveEvent.CAMERA_TRANSITION_DURATION;
    this.camera_unfollow_char_on_finish =
      camera_unfollow_char_on_finish ?? false;
  ...
  async on_position_reach(
    ...
    if (this.wait_after) {
      ...
      this.go_to_finish(
        char,
        previous_force_char_stop_in_event,
        previous_move_freely_in_event
      );
    }
  }
  ...
  async go_to_finish(
    ...
    if (this.camera_follow) {
      if (this.reset_previous_camera_target) {
        await this.data.camera.follow(
          this.previous_camera_target,
          this.camera_follow_duration
        );
  ...
  async _fire() {
    ...
    if (this.dest.hasOwnProperty("type")) {
      dest_value = this.data.game_event_manager.get_value(
        this.dest as EventValue
      );
    } else {
      ...
      dest_value = {
        x:
          typeof dest_pos.x === "object"
            ? this.data.game_event_manager.get_value(dest_pos.x)
            : dest_pos.x,
        y:
          typeof dest_pos.y === "object"
            ? this.data.game_event_manager.get_value(dest_pos.y)
            : dest_pos.y,
      };
    }
    const dest = {
      x: this.dest_unit_in_tile
        ? get_centered_pos_in_px(dest_value.x, this.data.map.tile_width)
        : dest_value.x,
      y: this.dest_unit_in_tile
        ? get_centered_pos_in_px(dest_value.y, this.data.map.tile_height)
        : dest_value.y,
    };
    ...
    const udpate_callback = async () => {
      ...
      if (
        this_sqr_dist < minimal_distance_sqr ||
        this_sqr_dist > previous_sqr_dist
      ) {
        this.data.game_event_manager.remove_callback(udpate_callback);
        await this.on_position_reach(
          char,
          previous_force_char_stop_in_event,
          previous_move_freely_in_event
        );
