...
export class GameEventManager {
    ...
    switch (npc.npc_type) {
      ...
      case npc_types.SHOP:
        if (!this.data.shop_open) {
          const previous_npc_direction =
            await this.handle_npc_interaction_start(npc);
          this.data.shop_menu.open_menu(
            npc.shop_key,
            npc.avatar,
            npc.voice_key,
            async () => {
              await this.handle_npc_interaction_end(
                npc,
                previous_npc_direction
              );
            }
          );
        }
   ...
   */
  private async handle_npc_interaction_end(
    npc: NPC,
  ...
  async set_npc_and_hero_directions(npc: NPC) {
    ...
    const hero_promise = this.data.hero.face_direction(
      interaction_directions.hero_direction
    );
    const npc_promise = npc.face_direction(
      interaction_directions.target_direction
    );
  ...
  private async manage_npc_dialog(npc: NPC) {
    ...
    this.fire_next_step = dialog_manager.next.bind(
      dialog_manager,
    ...
    switch (event_value.type) {
      ...
      case event_value_types.STORAGE:
        ...
        return storage.type === storage_types.POSITION
          ? `${(storage.value as StoragePosition).x}/${
              (storage.value as StoragePosition).y
            }`
          : storage.value;
        ...
        switch (detailed_value.type) {
          ...
          case game_info_types.INTERACTABLE_OBJECT:
            const interactable_object = detailed_value.label
              ? this.data.map.interactable_objects_label_map[
                  detailed_value.label
                ]
  ...
  static get_interaction_directions(
    ...
    if (
      ...
      target_direction =
        interaction_pattern === interaction_patterns.TIK_TAK_TOE
