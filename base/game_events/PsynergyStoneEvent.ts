...
const INIT_TEXT = (hero_name: string) =>
  `${hero_name} checked on the ground...`;
...
export class PsynergyStoneEvent extends GameEvent {
  ...
  async _fire() {
    ...
    this.game.add
      ...
      .to(
        { x: stone_sprite_x, y: stone_sprite_y },
        300,
    ...
    const animation_key_shatter = this.origin_npc.sprite_info.getAnimationKey(
      "stone",
      "shatter"
    );
    const animation_shatter = this.origin_npc.sprite.animations.getAnimation(
      animation_key_shatter
    );
