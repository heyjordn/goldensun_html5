...
export class JumpEvent extends TileEvent {
  ...
  fire() {
    ...
    const next_position = get_front_position(
      side_position.x,
      side_position.y,
      jump_direction
    );
    const jump_offset =
      ([directions.left, directions.up].includes(jump_direction) ? -1 : 1) *
      JumpEvent.JUMP_OFFSET;
    ...
    if (
      ...
      const objs = this.data.map.get_tile_bodies(
        next_position.x,
        next_position.y
      );
    ...
    for (let i = 0; i < this.data.map.interactable_objects.length; ++i) {
      ...
      if (
        ...
        interactable_object.base_collision_layer
      ) {
    ...
    for (let i = 0; i < this.data.map.npcs.length; ++i) {
      ...
      if (
        next_npc.tile_x_pos !== next_position.x ||
        next_npc.tile_y_pos !== next_position.y
      ) {
        continue;
      }
    ...
    const next_pos_key = IntegerPairKey.get_key(
      next_position.x,
      next_position.y
    );
    ...
    if (next_pos_key in this.data.map.events) {
      for (let i = 0; i < this.data.map.events[next_pos_key].length; ++i) {
        ...
        if (
          ...
          event.is_active_at_direction(
            get_opposite_direction(jump_direction)
          ) &&
