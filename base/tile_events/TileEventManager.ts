...
class EventQueue {
  ...
  add(
    event: TileEvent,
    this_activation_direction: directions,
    fire_function: Function,
    fire = false
  ) {
    switch (event.type) {
  ...
  process_queue() {
    if (this.climb_event) {
      this.queue = this.queue.filter(
        (item) => item.event.type !== event_types.JUMP
      );
    }
...
export class TileEventManager {
  ...
  private fire_event(
    current_event: TileEvent,
    this_activation_direction: directions
  ) {
    if (
      current_event.type === event_types.ICE_SLIDE &&
    ...
    if (
      current_event.type === event_types.CLIMB &&
      !this.data.hero.idle_climbing
    ) {
      (current_event as ClimbEvent).set_current_activation_direction(
        this_activation_direction
      );
      current_event.fire();
    } else if (
      ![event_types.SPEED, event_types.STEP, event_types.COLLISION].includes(
        current_event.type
      )
    ) {
      current_event.fire();
  ...
  check_tile_events() {
    ...
    for (let i = 0; i < this.data.map.events[location_key].length; ++i) {
      ...
      if (
        !this_event.activation_collision_layers.has(
      ...
      switch (this_event.type) {
        ...
        case event_types.ROPE:
          if (
            ...
            const right_direction = this_event.is_active_at_direction(
              this.data.hero.current_direction
            );
            ...
            if (
              ...
              [
                ...
                base_actions.CLIMB,
                base_actions.ROPE,
              ].includes(this.data.hero.current_action as base_actions)
            ) {
              ...
              const timer_key = IntegerPairKey.get_key(
                this_event.id,
                this.data.hero.current_direction
              );
              ...
              if (
                ...
                this.event_timers[timer_key].timer.running
              ) {
                continue;
              }
              this.event_queue.add(
                ...
                () => {
                  ...
                  this.event_timers[timer_key].timer.add(
                    ...
                    () => {
                      ...
                      if (
                        this.event_timers[timer_key].activation_direction ===
                          this.data.hero.current_direction &&
                        this_event.is_active_at_direction(
                          this.data.hero.current_direction
                        ) &&
                        this_event.activation_collision_layers.has(
                          this.data.hero.collision_layer
                        )
                      ) {
                        this.fire_event(
                          this_event,
