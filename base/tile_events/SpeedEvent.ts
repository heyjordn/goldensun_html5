...
export class SpeedEvent extends TileEvent {
  ...
  constructor(
    ...
    if (
      speed_activate_directions === undefined ||
      speed_activate_directions === "all"
    ) {
      get_directions(true).forEach((d) =>
        this._speed_activate_directions.add(d)
      );
    } else if (Array.isArray(directions)) {
      speed_activate_directions.forEach((d) =>
        this._speed_activate_directions.add(directions[d as string])
      );
    } else {
      this._speed_activate_directions.add(
        directions[speed_activate_directions as string]
      );
    }
  }
