...
export abstract class TileEvent {
  ...
  constructor(
    ...
    const snapshot_info =
      this.data.snapshot_manager.snapshot?.map_data.tile_events[this._id];
    ...
    activation_collision_layers =
      snapshot_info?.activation_collision_layers ?? activation_collision_layers;
    ...
    this._initial_activation_directions = new Set(
      TileEvent.format_activation_directions(activation_directions)
    );
    if (snapshot_info?.activation_directions) {
      activation_directions = new Set(snapshot_info?.activation_directions);
    } else if (
      this.active_storage_key &&
      !this.data.storage.get(this.active_storage_key)
    ) {
      activation_directions = new Set();
    } else {
      activation_directions = new Set(
        [...this._initial_activation_directions].filter(
          (x) => !this._initial_disabled_directions.has(x)
        )
      );
    }
  ...
  is_active_at_direction(direction?: directions) {
    if (direction === undefined) {
      ...
      const possible_directions = this.allow_active_in_diagonal
  ...
  check_position() {
    return (
      this.data.hero.tile_x_pos === this.x &&
      this.data.hero.tile_y_pos === this.y
    );
  }
  ...
  set_activation_collision_layers(...collision_layers_indexes: number[]) {
    ...
    collision_layers_indexes.forEach(
      this.activation_collision_layers.add,
      this.activation_collision_layers
    );
  }
  ...
  static get_labeled_event(key_name: string): TileEvent {
    return key_name in TileEvent.labeled_events
      ? TileEvent.labeled_events[key_name]
      : null;
  }
