...
export class NPC extends ControllableChar {
  ...
  constructor(
    ...
    if (this.storage_keys.move_freely_in_event !== undefined) {
      move_freely_in_event = this.data.storage.get(
        this.storage_keys.move_freely_in_event
      );
    }
    ...
    this._interaction_pattern =
      interaction_pattern ?? interaction_patterns.NO_INTERACTION;
    if (this.storage_keys.affected_by_reveal !== undefined) {
      affected_by_reveal = this.data.storage.get(
        this.storage_keys.affected_by_reveal
      );
    }
  ...
  check_storage_keys() {
    ...
    if (this.storage_keys.movement_type !== undefined) {
      const storage_value = this.data.storage.get(
        this.storage_keys.movement_type
      );
  ...
  change_collision_layer(
    ...
    this.sprite.body.removeCollisionGroup(
      this.data.collision.npc_collision_groups[this.base_collision_layer]
    );
    this.sprite.body.setCollisionGroup(
      this.data.collision.npc_collision_groups[destination_collision_layer]
    );
    if (update_on_map) {
      ...
      if (this._stepping && this._step_frame_counter < this._step_duration) {
        if (!this.set_speed_factors()) {
          ...
          if (
            get_sqr_distance(
              this.x,
              this._step_destination.x,
              this.y,
              this._step_destination.y
        ...
        !this._stepping &&
        this._step_frame_counter < this._wait_duration
      ) {
        this.stop_char(true);
      } else if (
        this._step_frame_counter >=
        (this._stepping ? this._step_duration : this._wait_duration)
      ) {
  ...
  private set_speed_factors() {
    for (
      let i = 0;
      i < this.game.physics.p2.world.narrowphase.contactEquations.length;
      ++i
    ) {
      const contact =
        this.game.physics.p2.world.narrowphase.contactEquations[i];
      if (
        contact.bodyB === this.body.data &&
        contact.bodyA === this.data.hero.body.data
      ) {
  ...
  private update_random_walk(flip_direction: boolean = false) {
    if (
      flip_direction ||
      get_sqr_distance(this.x, this._initial_x, this.y, this._initial_y) <=
      ...
      for (let tries = 0; tries < NPC.MAX_DIR_GET_TRIES; ++tries) {
        ...
        const next_pos: { x: number; y: number } = next_px_step(
          this.x,
          this.y,
          step_size,
          desired_direction
        ...
        const wall_avoiding_dist =
          _.mean([this.data.map.tile_width, this.data.map.tile_height]) / 2;

        const surrounding_constraints = [
          { direction: desired_direction, step_size: step_size },
          {
            direction: desired_direction,
            step_size: step_size + wall_avoiding_dist,
          },
          ...(flip_direction
            ? []
            : [
                {
                  direction: desired_direction + numbers.degree45,
                  step_size: step_size,
                },
              ]),
          ...(flip_direction
        ...
        surrounding_tests: {
          for (let i = 0; i < surrounding_constraints.length; ++i) {
            ...
            const test_pos_x_tile = get_tile_position(
              test_pos.x,
              this.data.map.tile_width
            );
            const test_pos_y_tile = get_tile_position(
              test_pos.y,
          ...
          const speed_vector = new Phaser.Point(
            next_pos.x - this.x,
            next_pos.y - this.y
          ).normalize();
  ...
  is_pos_colliding(tile_x_pos: number, tile_y_pos: number) {
    ...
    const instances_in_tile = this.data.map.get_tile_bodies(
      tile_x_pos,
      tile_y_pos
    );
    ...
    return (
      tile_x_pos === this.data.hero.tile_x_pos &&
      tile_y_pos === this.data.hero.tile_y_pos
    );
  }
  ...
  toggle_active(active: boolean) {
    if (active) {
      ...
      if (this.sprite) {
        if (this.storage_keys.visible !== undefined) {
          this.sprite.visible = this.data.storage.get(
            this.storage_keys.visible
          ) as boolean;
        } else {
  ...
  init_npc(map: Map, snapshot_info?: SnapshotData["map_data"]["npcs"][0]) {
    ...
    if (!this.no_shadow) {
      ...
      this.shadow_following =
        this.snapshot_info?.shadow_following ?? this.shadow_following;
    ...
    if (this.snapshot_info?.active_filters) {
      ...
      if (active_filters[EngineFilters.OUTLINE]) {
        ...
        this.outline_filter.texture_width =
          this.snapshot_info.filter_settings.outline.texture_width;
        this.outline_filter.texture_height =
          this.snapshot_info.filter_settings.outline.texture_height;
        ...
        this.outline_filter.keep_transparent =
          this.snapshot_info.filter_settings.outline.keep_transparent;
      }
    }
  }
