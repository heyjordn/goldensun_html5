...
export class InteractableObjects {
  ...
  constructor(
    ...
    if (this.storage_keys.position !== undefined) {
      const position = this.data.storage.get(
        this.storage_keys.position
      ) as StoragePosition;
      x = position.x;
      y = position.y;
    }
    ...
    if (this.storage_keys.base_collision_layer !== undefined) {
      base_collision_layer = this.data.storage.get(
        this.storage_keys.base_collision_layer
      );
    }
    ...
    if (this.storage_keys.entangled_by_bush !== undefined) {
      entangled_by_bush = this.data.storage.get(
        this.storage_keys.entangled_by_bush
      );
    }
    ...
    this._levels_filter = this.game.add.filter(
      "Levels"
    ) as Phaser.Filter.Levels;
    this._color_blend_filter = this.game.add.filter(
      "ColorBlend"
    ...
    if (this.storage_keys.anchor !== undefined) {
      const anchor = this.data.storage.get(
        this.storage_keys.anchor
      ) as StoragePosition;
      anchor_x = anchor.x;
      anchor_y = anchor.y;
    }
    ...
    if (this.storage_keys.scale !== undefined) {
      const scale = this.data.storage.get(
        this.storage_keys.scale
      ) as StoragePosition;
      scale_x = scale.x;
      scale_y = scale.y;
    }
  ...
  position_allowed(x: number, y: number) {
    ...
    for (let i = 0; i < this.allowed_tiles.length; ++i) {
      ...
      if (
        tile.x === x &&
        tile.y === y &&
        tile.collision_layer === this.data.map.collision_layer
      ) {
        return true;
      }
    }
  ...
  private creating_blocking_stair_block() {
    const target_layer =
      this.base_collision_layer + this.block_climb_collision_layer_shift;
    ...
    body.clearShapes();
    const width =
      this.data.dbs.interactable_objects_db[this.key_name].body_radius * 2;
    body.setRectangle(width, width, 0, 0);
    if (
      !(target_layer in this.data.collision.interactable_objs_collision_groups)
    ) {
  ...
  initial_config(
    map: Map,
    snapshot_info: SnapshotData["map_data"]["interactable_objects"][0]
  ) {
    this._sprite_info =
      this.data.info.iter_objs_sprite_base_list[this.key_name];
    if (this.psynergies_info) {
      for (let psynergy_key in this.psynergies_info) {
        ...
        if (
          ...
          this.psynergy_casted[psynergy_key] =
            snapshot_info?.psynergy_casted[psynergy_key] ?? false;
        }
        if (psynergy_properties.after_psynergy_cast_events) {
          ...
          psynergy_properties.after_psynergy_cast_events.forEach(
            (event_info) => {
        ...
        if (psynergy_properties.before_psynergy_cast_events) {
          ...
          psynergy_properties.before_psynergy_cast_events.forEach(
            (event_info) => {
    ...
    if (this.sprite_info) {
      const interactable_object_db =
        this.data.dbs.interactable_objects_db[this.key_name];
      const interactable_object_key = this.sprite_info.getSpriteKey(
        this.current_action
      );
      const interactable_object_sprite = this.data.middlelayer_group.create(
        0,
        0,
        interactable_object_key
      );
      ...
      if (this.storage_keys.position_px) {
        const position_px = this.data.storage.get(
          this.storage_keys.position_px
        ) as StoragePosition;
      ...
      if (this.snapshot_info?.active_filters) {
        ...
        if (active_filters[EngineFilters.COLORIZE]) {
          ...
          this.colorize_filter.intensity =
            this.snapshot_info.filter_settings.colorize.intensity;
          this.colorize_filter.color =
            this.snapshot_info.filter_settings.colorize.color;
        }
        if (active_filters[EngineFilters.LEVELS]) {
          this.manage_filter(this.levels_filter, true);
          this.levels_filter.min_input =
            this.snapshot_info.filter_settings.levels.min_input;
          this.levels_filter.max_input =
            this.snapshot_info.filter_settings.levels.max_input;
          this.levels_filter.gamma =
            this.snapshot_info.filter_settings.levels.gamma;
        }
        if (active_filters[EngineFilters.COLOR_BLEND]) {
          this.manage_filter(this.color_blend_filter, true);
          this.color_blend_filter.r =
            this.snapshot_info.filter_settings.color_blend.r;
          this.color_blend_filter.g =
            this.snapshot_info.filter_settings.color_blend.g;
          this.color_blend_filter.b =
            this.snapshot_info.filter_settings.color_blend.b;
        }
        ...
        if (active_filters[EngineFilters.GRAY]) {
          ...
          this.gray_filter.intensity =
            this.snapshot_info.filter_settings.gray.intensity;
        }
  ...
  set_shadow(
    ...
    const key_name =
      options?.key_name ?? InteractableObjects.DEFAULT_SHADOW_KEYNAME;
    const shadow_anchor_x =
      options?.shadow_anchor_x ?? InteractableObjects.DEFAULT_SHADOW_ANCHOR_X;
    const shadow_anchor_y =
      options?.shadow_anchor_y ?? InteractableObjects.DEFAULT_SHADOW_ANCHOR_Y;
    ...
    const scale_x = options?.is_world_map
      ? numbers.WORLD_MAP_SPRITE_SCALE_X
      : 1;
    const scale_y = options?.is_world_map
  ...
  init_bush(map: Map) {
    this._bush_sprite = this.data.middlelayer_group.create(
      0,
      0,
      InteractableObjects.BUSH_KEY
    );
    ...
    if (this.sprite) {
      ...
      this._bush_sprite.x =
        get_px_position(this.tile_x_pos, map.tile_width) +
        (map.tile_width >> 1);
      this._bush_sprite.y =
        get_px_position(this.tile_y_pos, map.tile_height) + map.tile_height;
    }
  ...
  initialize_related_events(map: Map) {
    ...
    for (
      let i = 0;
      i < this.data.dbs.interactable_objects_db[this.key_name].events.length;
      ++i
    ) {
      ...
      const collision_layer_shift =
        this.tile_events_info[i]?.collision_layer_shift ?? 0;
  ...
  play(
    ...
    const anim_key = this.sprite_info.getAnimationKey(
      this.current_action,
  ...
  private set_jump_type_event(
    ...
    const this_event_location_key = IntegerPairKey.get_key(
      new_event.x,
      new_event.y
    );
    ...
    const collision_layer_shift =
      this.tile_events_info[event_index]?.collision_layer_shift ?? 0;
    new_event.collision_layer_shift_from_source = collision_layer_shift;
    this.collision_change_functions.push(() => {
      new_event.set_activation_collision_layers(
        this.base_collision_layer + collision_layer_shift
      );
    });
  ...
  private set_rope_event(
    ...
    const activation_directions =
      event_info.activation_directions ??
      get_directions(false).map((dir) => reverse_directions[dir]);
      ...
      switch (direction) {
        ...
        case null:
          activation_direction = activation_directions.map(
            (dir) =>
              reverse_directions[
                get_opposite_direction(directions[dir as string])
              ]
          );
  ...
  private set_stair_event(
    ...
    events_data.forEach((event_data) => {
      ...
      const this_location_key = IntegerPairKey.get_key(
        new_event.x,
        new_event.y
      );
      ...
      this.insert_event(new_event.id);
      new_event.collision_layer_shift_from_source =
        event_data.collision_layer_shift_from_source;
      this.collision_change_functions.push(
        event_data.collision_change_function.bind(null, new_event)
      );
    });
  }
  ...
  toggle_active(active: boolean) {
    if (active) {
      ...
      if (this._blocking_stair_block) {
        this._blocking_stair_block.collides(
      ...
      if (this._blocking_stair_block) {
        this._blocking_stair_block.removeCollisionGroup(
          this.data.collision.hero_collision_group
        );
      }
      if (this.sprite) {
  ...
  config_body() {
    ...
    if (db.custom_body_polygon) {
      ...
      polygon = mount_collision_polygon(
        width,
        -(width >> 1),
        db.collision_body_bevel
      );
    }
    ...
    this.sprite.body.setCollisionGroup(
      collision_groups[this.base_collision_layer]
    );
    ...
    this.sprite.body.collides(this.data.collision.hero_collision_group);
    this._shapes_collision_active =
      this.snapshot_info?.shapes_collision_active ?? true;
  ...
  shift_events(event_shift_x: number, event_shift_y: number) {
    ...
    for (let i = 0; i < object_events.length; ++i) {
      ...
      this.data.map.events[event.location_key] = this.data.map.events[
        event.location_key
      ].filter((e) => {
  ...
  unset(remove_from_middlelayer_group: boolean = true) {
    ...
    for (let psynergy_key in this.before_psynergy_cast_events) {
      this.before_psynergy_cast_events[psynergy_key].forEach((e) =>
        e.destroy()
      );
    }
