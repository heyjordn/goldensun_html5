...
export class Pushable extends InteractableObjects {
  ...
  fire_push_movement(
    ...
    if (!target_only) {
      ...
      if (
        -char.sprite.y >= positive_limit &&
        -char.sprite.y >= negative_limit
      ) {
            ...
            this.object_drop_tiles.forEach((drop_tile) => {
              if (
                ...
                this.game.add
                  ...
                  .onComplete.addOnce(() => {
                    this.change_collision_layer(
                      drop_tile.destination_collision_layer
                    );
                    this.shift_events_and_check_collision(
                      0,
                      drop_tile.dest_y - this.tile_y_pos
                    );
                    this.set_tile_position({
        ...
        if (
          ...
          if (
            ...
            this.base_collision_layer ===
              this.dock_tile_position.collision_layer
          ) {
            ...
            if (pillar_into_the_water) {
              ...
              if (storage_key_position_px) {
                ...
                storage_key_position_px =
                  this.create_engine_storage_key("position_px");
                this.data.storage.add(
                  storage_key_position_px,
                  storage_types.POSITION,
                  value_position_px,
   ...
   */
  private shift_events_and_check_collision(
    ...
    for (let i = 0; i < object_events.length; ++i) {
      ...
      if (event.type === event_types.JUMP) {
        const events_in_map =
          this.data.map.events[IntegerPairKey.get_key(event.x, event.y)];
  ...
  private dust_animation(on_animation_end: () => void) {
    ...
    const dust_sprite_base =
      this.data.info.misc_sprite_base_list[Pushable.DUST_KEY];
    const dust_key = dust_sprite_base.getSpriteKey(Pushable.DUST_KEY);
    for (let i = 0; i < Pushable.DUST_COUNT; ++i) {
      const this_angle =
        ((Math.PI + numbers.degree60) * i) / (Pushable.DUST_COUNT - 1) -
        numbers.degree30;
