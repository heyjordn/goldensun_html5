...
export class RopeDock extends InteractableObjects {
  ...
  get rope_fragments_group(): Phaser.Group {
    return this._is_starting_dock
      ? this._rope_fragments_group
      : this._dest_rope_dock.rope_fragments_group;
  }
  ...
  initialize_rope(map: Map) {
    ...
    this.sprite.sort_function_end = () => {
      ...
      if (
        this.data.middlelayer_group.getChildIndex(back) >
        this.data.middlelayer_group.getChildIndex(front)
      ) {
  ...
  destroy_overlapping_fragments() {
    ...
    this._extra_sprites = this._extra_sprites.filter(
      (obj) => obj !== this._frag_overlap_group
    );
  }
  ...
  set_rope_fragments(map: Map) {
    ...
    const this_y_px =
      ...
      RopeDock.ROPE_Y_SHIFT;
    const dest_x_px = get_centered_pos_in_px(this._dest_x, map.tile_width);
    const dest_y_px =
      get_centered_pos_in_px(this._dest_y, map.tile_height) +
      RopeDock.ROPE_Y_SHIFT;
    ...
    this._fragment_angle = range_360(
      Math.atan2(dest_y_px - this_y_px, dest_x_px - this_x_px)
    );
    ...
    this._rope_fragments_group = this.game.add.group(
    ...
    if (!this.tied) {
      this._frag_overlap_group = this.game.add.group(
        this.data.middlelayer_group
      );
      this._frag_overlap_group.x = this._rope_fragments_group.x;
      this._frag_overlap_group.y = this._rope_fragments_group.y;
      this._frag_overlap_group.base_collision_layer =
        this._rope_fragments_group.base_collision_layer;
      this._frag_overlap_group.useHeightWhenSorting =
        this._rope_fragments_group.useHeightWhenSorting;
    ...
    for (let i = 0; i < fragments_number; ++i) {
      ...
      const frame_name = this.sprite_info.getFrameName(
        RopeDock.ROPE_DOCK_KEY,
        RopeDock.ROPE_FRAGMENT
      );
      const sprite: Phaser.Sprite = this._rope_fragments_group.create(
        0,
        0,
        sprite_key,
        frame_name
      ...
      if (this.tied) {
        ...
        sprite.x =
          RopeDock.SPIRAL_VELOCITY *
          t *
          Math.cos(RopeDock.SPIRAL_ANG_VELOCITY * t);
        sprite.y =
          RopeDock.SPIRAL_VELOCITY *
          t *
          Math.sin(RopeDock.SPIRAL_ANG_VELOCITY * t);
        sprite.rotation =
          Math.atan2(sprite.y - RopeDock.SPIRAL_Y_SHIFT, sprite.x) + degree90;
        sprite.x += RopeDock.SPIRAL_X_SHIFT;

        if (sprite.y >= 3) {
          const sprite_over: Phaser.Sprite = this._frag_overlap_group.create(
            0,
            0,
            sprite_key,
            frame_name
  ...
  find_dest_dock(map: Map) {
    for (let i = 0; i < map.interactable_objects.length; ++i) {
      ...
      if (
        io.is_rope_dock &&
        io.tile_x_pos === this._dest_x &&
        io.tile_y_pos === this._dest_y
      ) {
  ...
  intialize_swing(force: boolean = false) {
    ...
    const y_shift = this.data.hero.climbing_rope
      ? RopeDock.HERO_CLIMB_Y_SHIFT
      : 0;
    this.swing_tween.onUpdateCallback(() => {
      ...
      if (this.data.hero.in_movement() || force) {
        this._hero_walking_factor = _.clamp(
          this._hero_walking_factor + RopeDock.HERO_WALKING_DELTA,
          0,
          1
        );
      } else {
      ...
      if (force) {
        ...
        relative_pos =
          Math.abs(this.data.hero.sprite.x - this.x) / this.rope_width;
      }
      const position_ratio =
        position_ratio_formula(relative_pos) * this._hero_walking_factor;

      if (!force) {
        this.data.hero.sprite.y =
          this.data.hero.sprite.body.y +
          y_shift +
          swing_object.y * position_ratio;
      }
      ...
      for (let i = 0; i < rope_fragments_group.children.length; ++i) {
        ...
        if (!force) {
          const hero_frag_dist =
            Math.abs(rope_frag_x - this.data.hero.sprite.x) | 0;
          distance_ratio = 1 - hero_frag_dist / this.rope_width;
        }

        const relative_frag_pos =
          Math.abs(rope_frag_x - this.x) / this.rope_width;
        const position_penalty = position_ratio_formula(relative_frag_pos);

        const variation =
          swing_object.y * distance_ratio * position_ratio * position_penalty;
