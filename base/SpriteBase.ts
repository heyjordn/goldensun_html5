...
export class SpriteBase {
  ...
  setActionAlias(game: Phaser.Game, alias, reference_action) {
    if (reference_action in this.actions) {
      ...
      game.cache.setCacheAlias(
        this.getSpriteKey(alias),
        this.getSpriteKey(reference_action),
        Phaser.Cache.IMAGE
      );
    }
  }
  ...
  getFrameName(action, animation, index = 0) {
    const formatted_index = index.toLocaleString("en-US", {
  ...
  static getSpriteAction(sprite) {
    ...
    return key.substring(
      key.lastIndexOf(SpriteBase.ACTION_ANIM_SEPARATOR) + 1,
      key.length
    );
  }
