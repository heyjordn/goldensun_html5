...
export class Debug {
  ...
  initialize_controls() {
    const debug_controls = [
      {
        buttons: Button.DEBUG_PHYSICS,
        on_down: this.toggle_debug_physics.bind(this),
      },
    ...
    if (!this.data.electron_app) {
      debug_controls.push(
        ...[
          {
            buttons: Button.DEBUG_GRID,
            on_down: this.toggle_grid.bind(this),
          },
          {
            buttons: Button.DEBUG_KEYS,
            on_down: this.toggle_keys.bind(this),
          },
          {
            buttons: Button.DEBUG_STATS,
            on_down: this.toggle_stats.bind(this),
          },
          {
            buttons: Button.DEBUG_SLIDERS,
            on_down: this.toggle_sliders.bind(this),
          },
        ]
      );
    }
    this.data.control_manager.add_controls(debug_controls, {
      persist: true,
    });
  }
  ...
  update_debug_physics(flag) {
    ...
    for (let i = 0; i < this.data.middlelayer_group.children.length; ++i) {
      let sprite: Phaser.Sprite = this.data.middlelayer_group.children[
        i
      ] as Phaser.Sprite;
  ...
  toggle_debug_physics() {
    ...
    for (let i = 0; i < this.data.map.interactable_objects.length; ++i) {
      ...
      if (interactable_object.blocking_stair_block) {
        interactable_object.blocking_stair_block.debug =
          !interactable_object.blocking_stair_block.debug;
      }
    }
  ...
  fill_key_debug_table() {
    ...
    document.querySelector("#key_debug table .action").innerHTML =
    ...
    document.querySelector("#key_debug table .sliding_on_ice").innerHTML =
      this.data.hero.sliding_on_ice.toString();
  ...
  set_debug_info() {
    ...
    if (this.show_fps) {
      this.game.debug.text(
        "RPS: " + this.game.time.rps || "RPS: --",
        5,
        15,
        "#00ff00"
      );
      this.game.debug.text(
        "UPS: " + this.game.time.ups || "UPS: --",
        5,
        27,
        "#00ff00"
      );
    ...
    if (this.grid) {
      ...
      for (let y = 0; y < this.game.world.height; y += tile_height) {
        this.game.debug.geom(
          new Phaser.Line(0, y, this.game.world.width, y),
          "rgba(0,255,255,0.35)",
          false,
          4
        );
      ...
      this.game.debug.geom(
        new Phaser.Rectangle(x_pos, y_pos, tile_width, tile_height),
        "rgba(255,0,0,0.5)"
      );
      this.game.debug.geom(
      ...
      if (this.game.input.mousePointer.withinGame) {
        const mouse_x =
          this.game.camera.x +
          this.game.input.mousePointer.x / this.data.scale_factor;
        const mouse_y =
          this.game.camera.y +
          this.game.input.mousePointer.y / this.data.scale_factor;
        const mouse_x_tile = (mouse_x / this.data.map.tile_width) | 0;
        const mouse_y_tile = (mouse_y / this.data.map.tile_height) | 0;
        this.game.debug.text(
          `x: ${mouse_x_tile}/${mouse_x | 0}, y: ${mouse_y_tile}/${
            mouse_y | 0
          }`,
          60,
          15,
          ...
          const events = this.data.map.events[event_key].map((event) => {
            return _.omitBy(
              Object.assign({}, event, {
                ...
                x: event.x,
                y: event.y,
                activation_directions: Array.from(
                  event.activation_directions
                ).map((dir) => reverse_directions[dir]),
                activation_collision_layers: Array.from(
                  event.activation_collision_layers
                ),
                ...(event.origin_interactable_object && {
                  origin_interactable_object: `[${
                    event.origin_interactable_object.key_name
                  }${
  ...
  toggle_stats() {
    ...
    if (this.debug_stats) {
      ...
      if (this.debug_stats_info) {
        select_element.removeEventListener(
          "change",
          this.debug_stats_info.listener
        );
  ...
  fill_stats_debug_table() {
    ...
    document.querySelector("#stats_debug table .class").innerHTML =
      char.class.name;
    document.querySelector("#stats_debug table .level").innerHTML =
      char.level.toString();
    document.querySelector("#stats_debug table .exp").innerHTML =
      char.current_exp.toString();
    document.querySelector("#stats_debug table .current_hp").innerHTML =
      char.current_hp.toString();
    document.querySelector("#stats_debug table .max_hp").innerHTML =
      char.max_hp.toString();
    document.querySelector("#stats_debug table .current_pp").innerHTML =
      char.current_pp.toString();
    document.querySelector("#stats_debug table .max_pp").innerHTML =
      char.max_pp.toString();
    document.querySelector("#stats_debug table .atk").innerHTML =
      char.atk.toString();
    document.querySelector("#stats_debug table .def").innerHTML =
      char.def.toString();
    document.querySelector("#stats_debug table .agi").innerHTML =
      char.agi.toString();
    document.querySelector("#stats_debug table .luk").innerHTML =
      char.luk.toString();
    ordered_elements.forEach((element) => {
      ...
      document.querySelector(
        `#stats_debug table .${element}_resist`
      ).innerHTML = char.current_resist[element].toString();
      document.querySelector(`#stats_debug table .${element}_level`).innerHTML =
        char.current_level[element].toString();
    });
    document.querySelector("#stats_debug table .turns").innerHTML =
      char.turns.toString();
    document.querySelector("#stats_debug table .temp_statuses").innerHTML = [
      ...char.temporary_status,
    ].join(" ");
    document.querySelector("#stats_debug table .perm_statuses").innerHTML = [
      ...char.permanent_status,
    ].join(" ");
    let buff_html = "";
    ...
    document.querySelector("#stats_debug table .effect_count").innerHTML =
      char.effects.length.toString();
  }
}
